...
using media_m96::AudioLatency;
using media_m96::ChannelLayout;
...
bool ParamTraits<AudioParameters>::Read(const base::Pickle* m,
  ...
  std::vector<media_m96::Point> mic_positions;
  AudioLatency::LatencyType latency_tag;
  absl::optional<media_m96::AudioParameters::HardwareCapabilities>
...
template <>
struct ParamTraits<media_m96::EncryptionPattern> {
  typedef media_m96::EncryptionPattern param_type;
...
void ParamTraits<media_m96::EncryptionPattern>::Write(base::Pickle* m,
                                                  const param_type& p) {
  WriteParam(m, p.crypt_byte_block());
  WriteParam(m, p.skip_byte_block());
}

bool ParamTraits<media_m96::EncryptionPattern>::Read(const base::Pickle* m,
                                                 base::PickleIterator* iter,
                                                 param_type* r) {
  ...
  *r = media_m96::EncryptionPattern(crypt_byte_block, skip_byte_block);
  return true;
}

void ParamTraits<media_m96::EncryptionPattern>::Log(const param_type& p,
                                                std::string* l) {
