...
// Set the following variable to false to force it Flush() MediaCodec
// during Seek().
constexpr bool kForceRecreateDecoderDuringReset = false;
...
class PlayerComponentsFactory : public starboard::shared::starboard::player::
  ...
  scoped_ptr<PlayerComponents> CreateComponents(
    ...
    bool force_recreate_decoder_during_reset =
        !enable_flush_during_seek && kForceRecreateDecoderDuringReset;
    ...
    audio_renderer.reset(new AudioRendererPassthrough(
        ...
        creation_parameters.drm_system(), force_recreate_decoder_during_reset));
  ...
  bool CreateSubComponents(
    ...
    bool force_recreate_decoder_during_reset =
        !video_mime_type.GetParamBoolValue("enableflushduringseek", false) &&
        kForceRecreateDecoderDuringReset;
    SB_LOG(INFO) << "Recreate MediaCodec during Reset(): "
                 << (force_recreate_decoder_during_reset ? "enabled."
                                                         : "disabled.");
    ...
    if (creation_parameters.audio_codec() != kSbMediaAudioCodecNone) {
      ...
      auto decoder_creator = [force_recreate_decoder_during_reset](
  ...
  scoped_ptr<VideoDecoder> CreateVideoDecoder(
    ...
    scoped_ptr<VideoDecoder> video_decoder(new VideoDecoder(
        ...
        max_video_input_size, force_recreate_decoder_during_reset,
        error_message));
