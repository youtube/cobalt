...
void MojoVideoEncodeAcceleratorService::Encode(
  ...
  if (frame->coded_size() != input_coded_size_ &&
      frame->storage_type() != media_m96::VideoFrame::STORAGE_GPU_MEMORY_BUFFER) {
    DLOG(ERROR) << __func__ << " wrong input coded size, expected "
                << input_coded_size_.ToString() << ", got "
                << frame->coded_size().ToString();
    NotifyError(::media_m96::VideoEncodeAccelerator::kInvalidArgumentError);
    std::move(callback).Run();
    return;
  }

  frame->AddDestructionObserver(media_m96::base::BindPostTaskToCurrentDefault(std::move(callback)));
...
void MojoVideoEncodeAcceleratorService::
    RequestEncodingParametersChangeWithBitrate(const media_m96::Bitrate& bitrate,
                                               uint32_t framerate) {
  DCHECK_CALLED_ON_VALID_SEQUENCE(sequence_checker_);
...
void MojoVideoEncodeAcceleratorService::NotifyEncoderInfoChange(
    const ::media_m96::VideoEncoderInfo& info) {
  DVLOG(4) << __func__;
  DCHECK_CALLED_ON_VALID_SEQUENCE(sequence_checker_);
  if (!vea_client_)
    return;
