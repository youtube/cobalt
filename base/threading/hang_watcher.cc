  ...
  switch (thread_type) {
    ...
    case HangWatcher::ThreadType::kThreadPoolThread:
      return g_threadpool_log_level.load(std::memory_order_relaxed) >=
             logging_level;
    case HangWatcher::ThreadType::kRendererThread:
      return g_browser_process_renderer_thread_log_level.load(std::memory_order_relaxed) >=
             logging_level;
  }
}
  ...
  switch (process_type) {
    ...
    case HangWatcher::ProcessType::kBrowserProcess:
      ...
      g_browser_process_renderer_thread_log_level.store(
          static_cast<LoggingLevel>(kBrowserProcessRendererThreadLogLevel.Get()),
          std::memory_order_relaxed);
      break;
...
void HangWatcher::UnitializeOnMainThreadForTesting() {
  ...
  g_browser_process_renderer_thread_log_level.store(LoggingLevel::kNone, std::memory_order_relaxed);
}
...
bool HangWatcher::IsCrashReportingEnabled() {
  ...
  if (g_browser_process_renderer_thread_log_level.load(std::memory_order_relaxed) ==
      LoggingLevel::kUmaAndCrash) {
    return true;
  }
  return false;
