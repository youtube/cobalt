...
void* DecoderBufferAllocator::Allocate(DemuxerStream::Type type,
  ...
  if (starboard::common::Allocator::ExtraLogEnabled()) {
    ++pending_allocation_log_count_;
    pending_allocation_log_ << " a " << p << " " << type << " " << size << " "
                            << alignment;
...
void DecoderBufferAllocator::TryFlushAllocationLog_Locked() {
  ...
  // The allocation operations generate a few hundred log lines per second and
  // leads to missing entries on some platforms.  Grouping them helps avoid
  // missing entries, and the log index will be verified in the processing
  // script.
  if ((allocation_log_index_ + pending_allocation_log_count_) % 80 == 0 ||
      reuse_allocator_->GetAllocated() == 0) {
    SB_LOG(INFO) << " Media Allocation Log: " << allocation_log_index_
                 << pending_allocation_log_.str();

    allocation_log_index_ += pending_allocation_log_count_;
    pending_allocation_log_count_ = 0;
    // Reset the pending log
    pending_allocation_log_.str("");
    pending_allocation_log_.clear();
  }
}
#endif  // !defined(COBALT_BUILD_TYPE_GOLD)
