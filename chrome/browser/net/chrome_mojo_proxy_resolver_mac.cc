...
SystemProxyResolver* GetProxyResolver(const base::TimeDelta& idle_timeout) {
  ...
  if (!remote->is_bound()) {
    ...
    // `SystemProxyResolver` instances. This will happen pretty frequently, so
    // we wait for `idle_timeout` before dropping the `Remote` to initiate
    // service process termination. Any subsequent call to `GetProxyForUrl()`
