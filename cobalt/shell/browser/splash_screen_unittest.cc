...
class SplashScreenTest : public testing::Test {
 ...
 protected:
  ...
  Shell::State GetState(Shell* shell) { return shell->state_; }
  ...
  void ExpectStateUninitialized(Shell* shell) {
    EXPECT_EQ(GetState(shell), Shell::STATE_SPLASH_SCREEN_UNINITIALIZED);
  }

  void ExpectStateInitialized(Shell* shell) {
    EXPECT_EQ(GetState(shell), Shell::STATE_SPLASH_SCREEN_INITIALIZED);
  }

  void ExpectStateStarted(Shell* shell) {
    EXPECT_EQ(GetState(shell), Shell::STATE_SPLASH_SCREEN_STARTED);
  }

  void ExpectStateEnded(Shell* shell) {
    EXPECT_EQ(GetState(shell), Shell::STATE_SPLASH_SCREEN_ENDED);
  }
};
