...
void AOWSLogInit() {
  ...
  char* log_level_env = std::getenv("SB_AOWS_LOG_LEVEL") ? std::getenv("SB_AOWS_LOG_LEVEL") : const_cast<char*>("ERROR");
  switch (log_level_env[0]) {
    case 'I':
    case 'i':
      log_level = kInfo;
      break;
    ...
    default:
      log_level = kError;
      SB_LOG(WARNING) << logtag << ": Unrecognized SB_AOWS_LOG_LEVEL='" << log_level_env
                      << "', defaulting to ERROR level.";
  }
}
...
void AOWSLog(int level, const char* format, ...) {
  ...
  va_list args;
  // prepend with a common tag to make it easier to filter in logs
  va_start(args, format);
  fprintf(stderr, "[sbMicPOC] ");
...
int ParseAowsPort() {
  ...
  if (end == value || (end && *end != '\0') || parsed_port <= 0 ||
    ...
    AOWSLog(kWarning, "Ignoring invalid SB_AOWS_PORT='%s', using default port %d\n", value, kDefaultAowsPort);
    return kDefaultAowsPort;
  }
...
uint32_t GetAowsBindAddress() {
  if (IsAowsNetworkModeEnabled()) {
    ...
    AOWSLog(kInfo, "AOWS network mode enabled (SB_AOWS_LISTEN_ALL=1)\n");
...
bool ReadWebSocketFrame(int socket_fd,
  ...
  *opcode = frame_header[0] & 0x0F;
  ...
  AOWSLog(kInfo, "ReadWS: opcode=0x%X, fin=%d, masked=%d\n", static_cast<int>(*opcode), fin ? 1 : 0, masked ? 1 : 0);
  ...
  AOWSLog(kInfo, "ReadWS complete: got %zu bytes\n", payload->size());
 ...
 private:
  void Run() {
    ...
    if (listen_socket_fd < 0) {
      ...
      AOWSLog(kError, "Failed to create AOWS server socket, errno=%d\n", errno);
      return;
    }
    ...
    if (setsockopt(listen_socket_fd, SOL_SOCKET, SO_REUSEADDR, &reuse_address,
      ...
      SB_LOG(WARNING) << logtag << ": Failed to enable SO_REUSEADDR for local AOWS server, errno="
                      << errno;
      AOWSLog(kWarning, "Failed to enable SO_REUSEADDR for local AOWS server, errno=%d\n", errno);
    }
    ...
    if (bind(listen_socket_fd, reinterpret_cast<const sockaddr*>(&address),
      ...
      SB_LOG(ERROR) << logtag << ": Failed to bind local AOWS server to " << kAowsHost << ':'
                    << port_ << ", errno=" << errno;
       AOWSLog(kError, "Failed to bind local AOWS server to %s:%d, errno=%d\n", kAowsHost, port_, errno);
      close(listen_socket_fd);
      return;
    }
    ...
    if (listen(listen_socket_fd, kSocketBacklog) != 0) {
      SB_LOG(ERROR) << logtag << ": Failed to listen on local AOWS server socket, errno="
                    << errno;
      AOWSLog(kError, "Failed to listen on local AOWS server socket, errno=%d\n", errno);
      close(listen_socket_fd);
      return;
    }
    ...
    while (!stop_requested_.load()) {
      ...
      if (connection_fd < 0) {
        ...
        AOWSLog(kWarning, "Local AOWS accept failed, errno=%d\n", errno);
        continue;
      }
      ...
      if (!IsAowsNetworkModeEnabled()) {
        ...
        if (!is_loopback_peer) {
          ...
          AOWSLog(kWarning, "Rejected non-loopback AOWS connection (enable SB_AOWS_LISTEN_ALL=1 for network mode).\n");
          close(connection_fd);
          continue;
        }
      } else if (peer_address.sin_family != AF_INET) {
        SB_LOG(WARNING) << logtag << ": Rejected non-IPv4 AOWS connection.";
        AOWSLog(kWarning, "Rejected non-IPv4 AOWS connection.\n");
        close(connection_fd);
        continue;
      }
  ...
  void AppendAudio(const std::vector<uint8_t>& payload) {
    ...
    if (payload.size() >= kMaxBufferedAudioBytes) {
      ...
      AOWSLog(kInfo, "AppendAudio: overflow, kept %zu bytes\n", audio_buffer_.size());
      return;
    }
    ...
    AOWSLog(kInfo, "AppendAudio: %zu bytes added, buffer %zu -> %zu\n", payload.size(), prior_size, audio_buffer_.size());
  }
