...
#include "third_party/chromium/media/base/wait_and_replace_sync_token_client.h"
  ...
  switch (format) {
    ...
    case PIXEL_FORMAT_UNKNOWN:
      NOTREACHED() << "Only YUV formats and Y16 are supported, got: "
                   << media_m96::VideoPixelFormatToString(format);
  }
  done->Run();
}
...
void PaintCanvasVideoRenderer::Paint(
  ...
  if (!video_frame.get() || video_frame->natural_size().IsEmpty() ||
      !(media_m96::IsYuvPlanar(video_frame->format()) ||
...
void PaintCanvasVideoRenderer::Copy(
  ...
  Paint(std::move(video_frame), canvas, dest_rect, flags,
        media_m96::kNoTransformation, raster_context_provider);
}
...
bool PaintCanvasVideoRenderer::CopyVideoFrameTexturesToGLTexture(
  ...
  if (video_frame->NumTextures() > 1 ||
    ...
    // accurate.
    if ((media_m96::IsOpaque(video_frame->format()) || premultiply_alpha) &&
