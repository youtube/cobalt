...
class RBSPStreamTest : public testing::Test {
 protected:
  ...
  void EncodeSEV(int32_t num, std::list<bool>& bits) {
    bool is_negative = (num < 0);
    uint32_t unum = 0;
    if (is_negative) {
      unum = static_cast<uint32_t>(num * -1);
    } else {
      unum = static_cast<uint32_t>(num);
    }
  ...
  void EncodeUEV(uint32_t num, std::list<bool>& bits) {
  ...
  // for deserialization.
  std::unique_ptr<uint8_t[]> SerializeToBuffer(const std::list<bool>& bitlist,
    ...
    if (add_sequence_bytes) {
      int num_zeros = 0;
      for (std::list<uint8_t>::iterator it = bytelist.begin();
           it != bytelist.end(); ++it) {
    ...
    std::unique_ptr<uint8_t[]> buf(new uint8_t[bytelist.size()]);
    int index = 0;
    for (std::list<uint8_t>::iterator it = bytelist.begin();
         it != bytelist.end(); it++) {
      buf[index] = *it;
...
TEST_F(RBSPStreamTest, ReadUEV) {
  ...
  uint32_t f_n_minus_2 = 0;
  EncodeUEV(f_n_minus_2, fibbits);
  uint32_t f_n_minus_1 = 1;
  EncodeUEV(f_n_minus_1, fibbits);
  for (int i = 2; i < 47; i++) {
    uint32_t f_n = f_n_minus_1 + f_n_minus_2;
...
TEST_F(RBSPStreamTest, ReadSEV) {
  ...
  int32_t l_n_minus_2 = 1;
  EncodeSEV(l_n_minus_2, lucasbits);
  int32_t l_n_minus_1 = 2;
  EncodeSEV(-l_n_minus_1, lucasbits);
  for (int i = 2; i < 44; ++i) {
    int32_t l_n = l_n_minus_1 + l_n_minus_2;
    if (i % 2) {
...
static const uint8_t kTestRBSPExpGolombTooBig[] = {
...
TEST_F(RBSPStreamTest, ReadBit) {
  ...
  std::unique_ptr<uint8_t[]> sequence_buff =
  ...
  std::unique_ptr<uint8_t[]> desequence_buff =
...
TEST_F(RBSPStreamTest, ReadBits) {
  ...
  std::unique_ptr<uint8_t[]> seventeen_ones_buff =
  ...
  size_t pows_size = 0;
  std::unique_ptr<uint8_t[]> pows_buff =
      SerializeToBuffer(pows, true, pows_size);
  ...
  uint32_t dont_touch = 0xfeedfeed;
...
TEST_F(RBSPStreamTest, SkipBytes) {
  ...
  size_t nines_size = 0;
  std::unique_ptr<uint8_t[]> nines_buff =
      SerializeToBuffer(nines, true, nines_size);
  size_t nines_deseq_size = 0;
  std::unique_ptr<uint8_t[]> nines_deseq_buff =
  ...
  std::unique_ptr<uint8_t[]> run_length_buff =
      SerializeToBuffer(run_length, true, run_length_size);
  size_t run_length_deseq_size = 0;
  std::unique_ptr<uint8_t[]> run_length_deseq_buff =
...
TEST_F(RBSPStreamTest, SkipBits) {
  ...
  std::unique_ptr<uint8_t[]> skip_ones_buff =
      SerializeToBuffer(one_ohs, true, skip_ones_size);
  size_t skip_ohs_size = 0;
  std::unique_ptr<uint8_t[]> skip_ohs_buff =
