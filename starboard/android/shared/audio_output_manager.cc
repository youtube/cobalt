...
bool AudioOutputManager::GetAndResetHasAudioDeviceChanged(JNIEnv* env) {
  SB_DCHECK(env);
  jboolean has_audio_device_changed_java =
      Java_AudioOutputManager_getAndResetHasAudioDeviceChanged(
          env, j_audio_output_manager_);
  return has_audio_device_changed_java == JNI_TRUE;
}
