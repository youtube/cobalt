...
using media_m96::v4l2_test::Vp9Decoder;
...
std::unique_ptr<Vp9Decoder> CreateDecoder(
  ...
  VLOG(1) << "Creating decoder with codec "
          << media_m96::FourccToString(file_header.fourcc);

  LOG_ASSERT(file_header.fourcc == v4l2_fourcc('V', 'P', '9', '0'))
      << "Codec " << media_m96::FourccToString(file_header.fourcc)
      << " not supported.\n"
      << kUsageMsg;
  ...
  CHECK_EQ(driver_codec_fourcc, V4L2_PIX_FMT_VP9_FRAME)
      ...
      << media_m96::FourccToString(driver_codec_fourcc);
