...
1.  Update documents and images. Note that files in `cobalt/site/gen` and
    `cobalt/site/reference` are autogenerated by the `generate_site.py` script
    (executed within the Docker container) and changes in these directories
    will be overwritten by the steps below. These directories are created
