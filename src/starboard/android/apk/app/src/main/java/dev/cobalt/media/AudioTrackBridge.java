...
public class AudioTrackBridge {
  ...
  public AudioTrackBridge(
      ...
      int tunnelingAudioSessionIdVal) {
    int channelConfig;
    switch (channelCount) {
      case 1:
      ...
      try {
        audioTrack =
            new AudioTrack(
                ...
                (tunnelingAudioSessionIdVal != -1)
                    ? tunnelingAudioSessionIdVal
  ...
  @SuppressWarnings("unused")
  private int writeWithAvSync(byte[] audioData, int sizeInBytes, long presentationTimeNs) {
    if (audioTrack == null) {
      Log.e(TAG, "Unable to write with NULL audio track.");
      return 0;
    }
