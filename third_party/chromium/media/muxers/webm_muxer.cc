...
#include "third_party/chromium/media/formats/common/opus_constants.h"
...
void WriteOpusHeader(const media_m96::AudioParameters& params, uint8_t* header) {
...
bool WebmMuxer::OnEncodedAudio(const media_m96::AudioParameters& params,
                               std::string encoded_data,
                               base::TimeTicks timestamp) {
  DVLOG(2) << __func__ << " - " << encoded_data.size() << "B";
  DCHECK_CALLED_ON_VALID_SEQUENCE(sequence_checker_);
...
void WebmMuxer::AddAudioTrack(const media_m96::AudioParameters& params) {
