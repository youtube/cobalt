...
jobject MediaCodecBridge::GetInputBuffer(jint index) {
  SB_DCHECK(index >= 0);
  JNIEnv* env = AttachCurrentThread();
  return Java_MediaCodecBridge_getInputBuffer(env, j_media_codec_bridge_, index)
      .obj();
}
...
void MediaCodecBridge::ReleaseOutputBuffer(jint index, jboolean render) {
  JNIEnv* env = AttachCurrentThread();
  Java_MediaCodecBridge_releaseOutputBufferV_I_Z(env, j_media_codec_bridge_,
                                                 index, render);
}
...
void MediaCodecBridge::ReleaseOutputBufferAtTimestamp(
  ...
  JNIEnv* env = AttachCurrentThread();
  Java_MediaCodecBridge_releaseOutputBufferV_I_J(env, j_media_codec_bridge_,
