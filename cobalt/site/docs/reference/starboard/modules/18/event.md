...
` [===========] ` ` ` ` Focus Blur Preload ` ` ` ` V ` - – [===========] ` – > [
BLURRED ] ` ` [===========] ` ` ` ` Reveal Conceal ` ` ` ` ` V ` ` [===========]
` - – [ CONCEALED ] < – – > [===========] ` ` Unfreeze Freeze ` ` ` V `
[===========] - – [ FROZEN ] [===========] ` Stop ` V [===========] [ STOPPED ]
[===========]

A Starboard application receives either `Start` Start (`kSbEventTypeStart`) or
`Preload` Preload (`kSbEventTypePreload`) as its first event. `Start` Start
transitions the application to the `STARTED` STARTED state, while `Preload`
Preload transitions it to the `CONCEALED` CONCEALED state.

In the `STARTED` STARTED state, the application runs in the foreground and can
perform all standard operations. From the `STARTED` STARTED state, the
application can transition to the `BLURRED` BLURRED state upon receiving a
`Blur` Blur event.

In the `BLURRED` BLURRED state, the application remains visible but has lost
focus, is partially obscured by a modal dialog, or is preparing to shut down.
The application should reduce activity in this state. From the `BLURRED` BLURRED
state, the application can receive a `Focus` Focus event to return to the
`STARTED` STARTED state, or a `Conceal` Conceal event to transition to the
`CONCEALED` CONCEALED state.

In the `CONCEALED` CONCEALED state, the application behaves like an invisible
background process. It can still access the network and play audio, though the
platform might restrict its CPU and memory resources. The platform can
transition the application from `CONCEALED` CONCEALED to `FROZEN` FROZEN at any
time.

In the `FROZEN` FROZEN state, the application is invisible. It must immediately
release all graphics and video resources, and stop all background activity (such
as timers and rendering). The application must also flush storage to ensure data
is preserved if the process is terminated. Although the platform can terminate
the process in this state, it ideally sends a `Stop` Stop event first for a
graceful shutdown.

Note that the application always transitions through `BLURRED` BLURRED and
`CONCEALED` CONCEALED to `FROZEN` FROZEN before receiving `Stop` Stop or being
terminated.
...
Each event includes a `void*` void* data argument and must define the type of
...
*   `kSbEventTypePreload`
    ...
    The system can send `kSbEventTypePreload` while in the `UNSTARTED` UNSTARTED
    state to push the app into a lower resource consumption state. Applications
    call `SbSystemRequestConceal()` SbSystemRequestConceal() to request this.
    Only `Reveal` Reveal or `Freeze` Freeze events can follow a `Preload`
    Preload event. This event has no data argument.
*   `kSbEventTypeStart`
    ...
    `SbSystemRequestStop()` SbSystemRequestStop() . The system sends this event
    only once per process launch. The data argument contains `SbEventStartData`.
*   `kSbEventTypeBlur`

    Sent when a dialog is raised or the application enters a background-but-
    visible or partially-obscured state ( `BLURRED` BLURRED ). Graphics and
    ...
    sent only after a `Start` Start event. Only `Focus` Focus or `Conceal`
    Conceal events can follow a `Blur` Blur event. This event has no data
    argument.
*   `kSbEventTypeFocus`

    Sent when the application returns to the foreground ( `STARTED` STARTED )
    from the `BLURRED` BLURRED state. The application should resume foreground
    activity (such as animations and video playback). This event is only sent
    after a `Blur` Blur or `Reveal` Reveal event. This event has no data
    argument.
*   `kSbEventTypeConceal`

    The platform transitions the application to the `CONCEALED` CONCEALED state
    after handling this event. The application must become invisible, though
    background tasks like audio playback and recommendation updates can continue
    running. This event is sent only after a `Blur` Blur or `Reveal` Reveal
    event. Only `Freeze` Freeze or `Reveal` Reveal events can follow a `Conceal`
    Conceal event. On some platforms, the platform might terminate the process
    after a `Conceal` Conceal event without sending a `Freeze` Freeze event.
*   `kSbEventTypeReveal`

    The platform restores the application to the `BLURRED` BLURRED state from
    the `CONCEALED` CONCEALED state. This is the first event the application
    receives when leaving the `CONCEALED` CONCEALED state, following a `Conceal`
    Conceal or `Unfreeze` Unfreeze event. This event has no data argument.
*   `kSbEventTypeFreeze`

    The platform transitions the application to the `FROZEN` FROZEN state after
    ...
    SbStorage writes. This event can follow a `Conceal` Conceal or `Unfreeze`
    Unfreeze event. Only `Unfreeze` Unfreeze or `Stop` Stop events can follow a
    `Freeze` Freeze event. Some platforms terminate the application if it
    performs work or retains resources after freezing. Also, on some platforms,
    the platform might terminate the process after a `Freeze` Freeze event
    without sending a `Stop` Stop event. This event has no data argument.
*   `kSbEventTypeUnfreeze`

    The platform restores the application to the `CONCEALED` CONCEALED state
    from the `FROZEN` FROZEN state. This is the first event the application
    receives when leaving the `FROZEN` FROZEN state. This event can be received
    only after a `Freeze` Freeze event. This event has no data argument.
*   `kSbEventTypeStop`

    The platform shuts down the application after handling this event. This
    event is sent only in the `FROZEN` FROZEN state, following a `Freeze` Freeze
...
*   `kSbEventTypeScheduled`
    ...
    response to `SbEventSchedule()` SbEventSchedule() , which invokes the
    callback directly. Consequently, `SbEventHandle` never receives this event.
    The data type is an internally-defined structure.
...
*   `callback`: The callback function to call. Must not be `NULL` NULL .

*   `context`: The context passed to the `callback` function.

*   `delay`: The minimum number of microseconds to wait before calling the
    `callback` function. Set to `0` 0 to call the callback as soon as possible.
