...
void MediaAgency::UpdatePlayerClient(int audio_session_id,
  ...
  client_map_[audio_session_id].media_algorithm_ =
...
bool MediaAgency::IsTunnelModeEnabled(void* VideoRenderAlgorithmImpl) {
  ...
  for (std::pair<int32_t, ClientInfo > element : client_map_) {
      if (element.second.media_algorithm_ ==
              VideoRenderAlgorithmImpl) {
...
void MediaAgency::SetDroppedFramesTunnel(int audio_session_id, int dropped_frames) {
  starboard::ScopedLock lock(mutex_);
  auto iter = client_map_.find(audio_session_id);
  if (iter != client_map_.end()) {
...
int MediaAgency::GetAudioConfigByAudioSinkContext(
        void* audio_renderer_sink_android, SbTime *seek_time) {
  starboard::ScopedLock lock(mutex_);
  for (std::pair<int32_t, ClientInfo > element : client_map_) {
...
int MediaAgency::GetAudioSessionIdByVideoDecoder(
        void* video_decoder) {
  starboard::ScopedLock lock(mutex_);
  for (std::pair<int32_t, ClientInfo > element : client_map_) {
      if (element.second.video_decoder_ ==
              video_decoder) {
          if (element.second.disable_tunnel_) {
              return -1;
          }
...
void MediaAgency::SetPlaybackStatus(int audio_session_id, SbTime seek_to_time,
  ...
  if (iter->second.playback_status_cb_ != nullptr &&
      action != kSbPlaybackStatusUpdateActionNone) {
    iter->second.playback_status_cb_(action, playback_rate, playTime, seek_to_time);
  }
  ...
  /* There are three approaches for tunnel mode playback rate
   Method 1: Adjust audio sync time by playback rate
          ...
          b, Android HAL handle the audio timestamp specially since the
             ...
             rate for tunnel mode. If audio timestamp does not match with written
             audio frames, it would report incorrect audio timestamp.
             Hence it causes A/V sync problem
          c, Maintain a table consist of audio timestamp and written frames, it is
             similar as AudioFrameTracker. We calculate the real played audio
             time stamp by interpolate the table with the consumed frames.
             Note: there is problem that we can't the elapsed audio timestamp like
             GetFutureFramesPlayedAdjustedToPlaybackRate(). Maybe we can send the
             playback rate to low level. Anyway it would be complicated since video
             pipeline and audio pipeline is different process

   Method 2: Adjust video timestamp by playback rate
          a, Modify the video input buffer timestamp when there is playback rate changed.
             Note: there is limitation because video and audio pipeline run in
             different thread. Maybe video input buffer is prior to playback rate
             changed so that it has no chance to modify timestamp
             we can modify output buffer timestamp. However tunnel mode do not notify
             output buffer to cobalt.

   Method 3: In Android Video HAL to maintain AudioFrameTracker. Currently patch use it
          a, send the playback rate and corresponding audio written frames to video HAL
          b, When do A/V sync, we adjust audio timestamp by this
             AudioFrameTracker that has playback rate information.

  */
#if 0
  // we would set below parameter to android MediaCodec by setParameter like vendor.tunnel.playback.rate
  std::string s = std::to_string(static_cast<int32_t>(playback_rate));
  int ret = __system_property_set("tunnel.playback.rate", s.c_str());

  SbTime playTime = seek_to_time + frames_sent_to_sink_in_time;
  std::string s2 = std::to_string(static_cast<int32_t>(playTime / 1000));
  ret = __system_property_set("tunnel.playback.rate.timeat", s2.c_str());
#endif
}
