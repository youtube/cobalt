...
#include "third_party/chromium/media/capture/video/shared_memory_buffer_tracker.h"
...
class SharedMemoryBufferTrackerHandle : public media_m96::VideoCaptureBufferHandle {
 public:
...
size_t CalculateRequiredBufferSize(
    ...
    const media_m96::mojom::PlaneStridesPtr& strides) {
  if (strides) {
    size_t result = 0u;
    for (size_t plane_index = 0;
         plane_index < media_m96::VideoFrame::NumPlanes(format); plane_index++) {
      result +=
          strides->stride_by_plane[plane_index] *
          media_m96::VideoFrame::Rows(plane_index, format, dimensions.height());
    }
    return result;
  } else {
    const auto& frame_format =
        media_m96::VideoCaptureFormat(dimensions, 0.0f, format);
    return media_m96::VideoFrame::AllocationSize(frame_format.pixel_format,
