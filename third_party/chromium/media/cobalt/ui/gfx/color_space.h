...
class COLOR_SPACE_EXPORT ColorSpace {
 public:
  ...
  enum class TransferID : uint8_t {
    ...
    PQ,
    SMPTEST428_1,
    HLG,  // AKA hybrid-log gamma, HLG.
    // The same as SRGB on the interval [0, 1], with the nonlinear
  ...
  static constexpr ColorSpace CreateDisplayP3D65() {
    return ColorSpace(PrimaryID::P3, TransferID::SRGB,
  ...
  static constexpr ColorSpace CreateExtendedSRGB() {
    return ColorSpace(PrimaryID::BT709, TransferID::SRGB_HDR,
                      MatrixID::RGB, RangeID::FULL);
  }
  ...
  // HLG uses the BT.2020 primaries with the HLG transfer function.
  ...
  static constexpr ColorSpace CreateJpeg() {
    ...
    // intended here.
    return ColorSpace(PrimaryID::BT709, TransferID::SRGB,
