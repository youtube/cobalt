...
//       return static_cast<SbKey>(kSbKeyNumpad0 + (keycode -
//       AKEYCODE_NUMPAD_0));
...
// // For a left joystick, AMOTION_EVENT_AXIS_X reports the absolute X position
// of
// // the joystick. The value is normalized to a range from -1.0 (left) to 1.0
// // (right).
// //
// // For a left joystick, AMOTION_EVENT_AXIS_Y reports the absolute Y position
// of
// // the joystick. The value is normalized to a range from -1.0 (up or far)
// to 1.0
// // (down or near).
// //
...
//   events->push_back(
//       CreateMoveEventWithKey(device_id, window_, key, location,
//       input_vector));
// }

// namespace {

// // Generate a Starboard event from an Android motion event, with the SbKey
// and
...
// // Some helper enumerations to index into the
// InputEventsGenerator::hat_value_
// // array.
// enum HatAxis {
//   kHatX,
//   kHatY,
...
//     ProcessJoyStickEvent(kLeftX, AMOTION_EVENT_AXIS_X, android_event,
//     events); ProcessJoyStickEvent(kLeftY, AMOTION_EVENT_AXIS_Y,
//     android_event, events); ProcessJoyStickEvent(kRightX,
//     AMOTION_EVENT_AXIS_Z, android_event, events);
//     ProcessJoyStickEvent(kRightY, AMOTION_EVENT_AXIS_RZ, android_event,
//     events);
...
//     // Direction pad events are all assumed to be coming from the hat
//     controls
...
//       PushKeyEvent(key, kSbInputEventTypePress, window_, android_event,
//       events);
//     }
//     return;
//   }

//   // If we get this far, then we are exclusively dealing with thumbstick
//   events,
...
//   PossiblySynthesizeHatKeyEvents(kHatX, hat_value_[kHatX], new_hat_x,
//   window_,
...
//   PossiblySynthesizeHatKeyEvents(kHatY, hat_value_[kHatY], new_hat_y,
//   window_,
//                                  android_motion_event, events);
