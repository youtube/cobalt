...
TEST(NativeStabilityTest, VerifyNativeStabilityExtension) {
  ...
  // Since Evergreen platforms are expected to just wire up the shared
  // implementation of the extension, it's reasonable to expect them to be on
  // the latest version of the extension API available at the commit from which
  // this test is built.
  ASSERT_EQ(extension->version, 1u)
      << "Please update your build to use the latest version of the shared "
