...
extern "C" int LLVMFuzzerTestOneInput(const uint8_t* data, size_t size) {
  ...
  media_m96::H264Parser parser;
  parser.SetStream(data, base::checked_cast<off_t>(size));

  // Parse until the end of stream/unsupported stream/error in stream is
  // found.
  while (true) {
    media_m96::H264NALU nalu;
    media_m96::H264Parser::Result res = parser.AdvanceToNextNALU(&nalu);
    if (res != media_m96::H264Parser::kOk)
      break;

    switch (nalu.nal_unit_type) {
      case media_m96::H264NALU::kIDRSlice:
      case media_m96::H264NALU::kNonIDRSlice: {
        media_m96::H264SliceHeader shdr;
        res = parser.ParseSliceHeader(nalu, &shdr);
        break;
      }

      case media_m96::H264NALU::kSPS: {
        int id;
        res = parser.ParseSPS(&id);
        if (res != media_m96::H264Parser::kOk)
          break;
        const media_m96::H264SPS* sps = parser.GetSPS(id);
        if (!sps)
          break;
      ...
      case media_m96::H264NALU::kPPS: {
        int id;
        res = parser.ParsePPS(&id);
        break;
      }

      case media_m96::H264NALU::kSEIMessage: {
