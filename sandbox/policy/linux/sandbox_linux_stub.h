...
class SANDBOX_POLICY_EXPORT SandboxLinux {
 public:
  //   // This is a list of sandbox IPC methods which the renderer may send to
  //   the
  //   // sandbox host. See
  //   //
  //   https://chromium.googlesource.com/chromium/src/+/main/docs/linux/sandbox_ipc.md
  //   // This isn't the full list, values < 32 are reserved for methods called
  //   from
  ...
  // These form a bitmask which describes the conditions of the Linux
  // sandbox.
  ...
  struct Options : public SandboxSeccompBPF::Options {
    ...
    // Allow starting the sandbox with multiple threads already running.
    //   This
    //   // will enable TSYNC for seccomp-BPF, which syncs the seccomp-BPF
    //   policy
    // across all running threads.
    bool allow_threads_during_sandbox_init = false;

    // Enables the CHECK for open directories. The open directory check is
    //   only
    // useful for the chroot jail (from the semantic layer of the sandbox),
    //   and
    // can safely be disabled if we are only enabling the seccomp-BPF
    // layer.
  ...
  //   // Do some initialization that can only be done before any of the
  //   sandboxes
  //   // are enabled. If using the setuid sandbox, this should be called
  //   manually
  //   // before the setuid sandbox is engaged.
  //   // Security: When this runs, it is imperative that either
  //   InitializeSandbox()
  //   // runs as well or that all file descriptors returned in
  //   // GetFileDescriptorsToClose() get closed.
  //   // Otherwise file descriptors that bypass the security of the setuid
  //   sandbox
  //   // would be kept open. One must be particularly careful if a process
  //   performs
  //   // a fork().
  ...
  //   // In order for this sandbox to be effective, it must be "sealed" by
  //   calling
  //   // InitializeSandbox().
  //   // Terminates the process in case the sandboxing operations cannot
  //   complete
  //   // successfully.
  ...
  //   // benefit from extra sandboxing but is not strictly required on systems
  //   that
  ...
  //   // Return a list of file descriptors to close if PreinitializeSandbox()
  //   ran
  //   // but InitializeSandbox() won't. Avoid using.
  //   // TODO(jln): get rid of this hack.class SANDBOX_POLICY_EXPORT
  //   SandboxLinux {
  ...
  // be queried after going through PreinitializeSandbox(). This is a
  // bitmask
  ...
  //   // Returns true if the current process is single-threaded or if the
  //   number
  ...
  //   // Simple accessor for our instance of the setuid sandbox. Will never
  //   return
  //   // NULL.
  //   // There is no StartSetuidSandbox(), the SetuidSandboxClient instance
  //   should
  //   // be used directly.
  SetuidSandboxClient* setuid_sandbox_client() const;

  //   // Check the policy and eventually start the seccomp-bpf sandbox. Fine to
  //   be
  ...
  //   // Limit the address space of the current process (and its children) to
  //   make
  //   // some vulnerabilities harder to exploit. Writes the errno due to
  //   setrlimit
  //   // (including 0 if no error) into |error|.
  //   bool LimitAddressSpace(int* error);

  //   // Returns a file descriptor to proc. The file descriptor is no longer
  //   valid
  ...
  //   // A BrokerProcess is a helper that is started before the sandbox is
  //   engaged,
  //   // typically from a pre-sandbox hook, that will serve requests to access
  //   // files over an IPC channel. The client  of this runs from a SIGSYS
  //   handler
  //   // triggered by the seccomp-bpf sandbox.
  //   //
  //   // |policy| is the policy being run by the client, and is used to derive
  //   the
  //   // equivalent broker-side policy.
  //   //
  //   // |broker_side_hook| is an alternate pre-sandbox hook to be run before
  //   the
  ...
  //   // continuation is impossible (and presumably unsafe). This should never
  //   be
  ...
  //   // Returns true if the broker should handle a particular syscall
  //   indicated by
 ...
 private:
  ...
  //   // The last part of the initialization is to make sure any temporary
  //   "hole"
  ...
  //   // after transitioning to a new user namespace we don't allow this
  //   function
  //   // to fail.
  //   bool EngageNamespaceSandboxInternal(bool from_zygote);

  //   // A file descriptor to /proc. It's dangerous to have it around as it
  //   could
  ...
  //   bool initialize_sandbox_ran_;            // InitializeSandbox() was
  //   called. std::unique_ptr<SetuidSandboxClient> setuid_sandbox_client_;
  // #if BUILDFLAG(USING_SANITIZER)
  //   std::unique_ptr<__sanitizer_sandbox_arguments> sanitizer_args_;
  // #endif
  //   raw_ptr<syscall_broker::BrokerProcess> broker_process_;  // Leaked as
  //   global.
};
