...
#if BUILDFLAG(IS_ANDROID)
  LOG(ERROR) << "Cobalt: " << __func__;
  if (decode_target_graphics_context_provider_.texture_native_id) {
    LOG(ERROR) << "Cobalt: " << __func__;
    base::WaitableEvent done_event(
        base::WaitableEvent::ResetPolicy::MANUAL,
        base::WaitableEvent::InitialState::NOT_SIGNALED);
    LOG(ERROR) << "Cobalt: " << __func__;
    gpu_factory_.AsyncCall(&StarboardGpuFactory::RunDeleteTaskOnGpuThread)
        .WithArgs(decode_target_graphics_context_provider_.texture_native_id,
                  &done_event);
    LOG(ERROR) << "Cobalt: " << __func__;
...
void StarboardRendererWrapper::OnGpuChannelTokenReady(
  ...
  auto get_command_buffer_stub_cb = base::BindRepeating(
      get_starboard_command_buffer_stub_cb_, command_buffer_id->channel_token,
...
void StarboardRendererWrapper::OnGpuFactoryInitialized() {
  ...
  decode_target_graphics_context_provider_.texture_native_id =
      texture_native_id_;
