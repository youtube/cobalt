...
#include "third_party/chromium/media/cdm/cbcs_decryptor.h"
...
extern "C" int LLVMFuzzerTestOneInput(const uint8_t* data, size_t size) {
  ...
  std::vector<media_m96::SubsampleEntry> subsamples;
  if (clear_bytes <= size) {
  ...
  media_m96::EncryptionPattern pattern(1, encryption_pattern % 10);

  auto encrypted_buffer = media_m96::DecoderBuffer::CopyFrom(data, size);

  // Key_ID is never used.
  encrypted_buffer->set_decrypt_config(media_m96::DecryptConfig::CreateCbcsConfig(
      "key_id", std::string(std::begin(kIv), std::end(kIv)), subsamples,
      pattern));

  media_m96::DecryptCbcsBuffer(*encrypted_buffer, *key);
  return 0;
}
