...
class CobaltMemoryThresholdBrowserTest : public content::ContentBrowserTest, 
 ...
 public:
  ...
  void OnMemoryDumpReceived(
    ...
    // ScopedClosureRunner guarantees quit.Run() is called when this function ends,
    // no matter how it exits (failure, etc.)
    ...
    // print to stdout
    perf_test::PrintResult("PrivateFootprint", "", scenario_name, static_cast<size_t>(private_footprint_kb), "kb", true);
    perf_test::PrintResult("PeakResidentSet", "", scenario_name, static_cast<size_t>(peak_resident_set_kb), "kb", true);
    perf_test::PrintResult("V8HeapSize", "", scenario_name, static_cast<size_t>(v8_heap_size_kb), "kb", true);
 
  }

  void CollectMemoryMetrics(base::HistogramTester& histogram_tester, const std::string& scenario_name) {
    base::RunLoop run_loop;

    // Gather metrics from memory dump.
    memory_instrumentation::MemoryInstrumentation::GetInstance()
        ->RequestGlobalDump(
            {"v8"}, base::BindOnce(&CobaltMemoryThresholdBrowserTest::OnMemoryDumpReceived,
    ...
    if (!buckets.empty()) {
      ...
      testing::Test::RecordProperty("gpu_peak_memory_on_page_load_mb", gpu_peak_memory_page_load);
      perf_test::PrintResult("GpuPeakMemoryOnPageLoad", "", scenario_name,static_cast<size_t>(gpu_peak_memory_page_load), "mb", true);
    }
  }
};

IN_PROC_BROWSER_TEST_P(CobaltMemoryThresholdBrowserTest, BlankPageMemoryFootprint) {
...
IN_PROC_BROWSER_TEST_P(CobaltMemoryThresholdBrowserTest, YouTubeHomepageMemoryFootprint) {
  base::HistogramTester histogram_tester;
