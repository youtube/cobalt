...
#include "third_party/chromium/media/base/mock_filters.h"
...
#include "third_party/chromium/media/gpu/chromeos/mailbox_video_frame_converter.h"
...
MATCHER_P(MatchesStatusCode, status_code, "") {
  // media_m96::Status doesn't provide an operator==(...), we add here a simple one.
...
class VideoDecoderPipelineTest
 ...
 public:
  ...
  media_m96::CallbackRegistry<CdmContext::EventCB::RunType> event_callbacks_;
#endif  // BUILDFLAG(IS_CHROMEOS_ASH)
...
TEST_F(VideoDecoderPipelineTest, TranscryptKeyAddedDuringTranscrypt) {
  ...
  EXPECT_CALL(decryptor_, Decrypt(Decryptor::kVideo, encrypted_buffer_, _))
      .WillOnce([&saved_decrypt_cb](Decryptor::StreamType stream_type,
                                    scoped_refptr<DecoderBuffer> encrypted,
                                    Decryptor::DecryptCB decrypt_cb) {
        saved_decrypt_cb = base::BindPostTaskToCurrentDefault(std::move(decrypt_cb));
      });
  decoder_->Decode(encrypted_buffer_,
                   base::BindOnce(&VideoDecoderPipelineTest::OnDecodeDone,
                                  base::Unretained(this)));
...
TEST_F(VideoDecoderPipelineTest, RebuildFramePoolsOnStateLost) {
  ...
  EXPECT_CALL(*this, OnWaiting(media_m96::WaitingReason::kDecoderStateLost));
  InvokeWaitingCB(media_m96::WaitingReason::kDecoderStateLost);
  task_environment_.RunUntilIdle();
