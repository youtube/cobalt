  ...
  switch (status) {
    case DemuxerStream::kConfigChanged:
      if (next_audio_decoder_config_.IsValidConfig()) {
        ...
        next_audio_decoder_config_ = media_m96::AudioDecoderConfig();
      }
      if (next_video_decoder_config_.IsValidConfig()) {
        video_decoder_config_ = next_video_decoder_config_;
        next_video_decoder_config_ = media_m96::VideoDecoderConfig();
      }
...
std::string StreamProvider::GetDisplayName() const {
  return "media_m96::remoting::StreamProvider";
}
...
void default_delete<media_m96::remoting::StreamProvider>::operator()(
    media_m96::remoting::StreamProvider* ptr) const {
  ptr->Destroy();
}
