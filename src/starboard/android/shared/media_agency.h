...
class MediaAgency {
 public:
  typedef std::function<void(int, int64_t)> PlaybackStatusCB;

  void RegisterPlayerClient(int tunneling_audio_session_id,
                            void* video_decoder,
                            const PlaybackStatusCB& playback_status_cb);
  int UnregisterPlayerClient(int tunneling_audio_session_id);

  void UpdatePlayerClient(int tunneling_audio_session_id,
                          void* audio_renderer_sink_android);

  void SetPlaybackStatus(int tunneling_audio_session_id, SbTime seek_to_time);
