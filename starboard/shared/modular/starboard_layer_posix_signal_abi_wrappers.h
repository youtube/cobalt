...
// Musl's sigset_t is typically 128 bytes.
// We define it as an array of unsigned long to match.
#define _MUSL_SIGSET_SIZE (1024 / (8 * sizeof(long)))
typedef struct {
  unsigned long val[_MUSL_SIGSET_SIZE];
} musl_sigset_t;
...
// Wraps the platform's sigprocmask() call.
// It translates between musl_sigset_t and the platform's native sigset_t.
SB_EXPORT int __abi_wrap_sigprocmask(int how,
                                     const musl_sigset_t* set,
                                     musl_sigset_t* oldset);

// The following are simple wrappers that may be needed if their underlying
// types (like sigset_t) differ from the platform.
