...
#include "third_party/chromium/media/capture/video/mac/video_capture_device_decklink_mac.h"
...
class DeckLinkCaptureDelegate
 ...
 public:
  DeckLinkCaptureDelegate(
      ...
      media_m96::VideoCaptureDeviceDeckLinkMac* frame_receiver);
 ...
 private:
  ...
  void SendErrorString(media_m96::VideoCaptureError error,
  ...
  const media_m96::VideoCaptureDeviceDescriptor device_descriptor_;
  ...
  media_m96::VideoCaptureDeviceDeckLinkMac* frame_receiver_;
...
void DeckLinkCaptureDelegate::AllocateAndStart(
  ...
  if (decklink_input_local->GetDisplayModeIterator(
    ...
    SendErrorString(
        media_m96::VideoCaptureError::kMacDeckLinkErrorCreatingDisplayModeIterator,
  ...
  if (!chosen_display_mode.get()) {
    SendErrorString(
        media_m96::VideoCaptureError::kMacDeckLinkCouldNotFindADisplayMode,
        FROM_HERE, "Could not find a display mode");
    return;
  }
#if !defined(NDEBUG)
  DVLOG(1) << "Requested format: "
           << media_m96::VideoCaptureFormat::ToString(params.requested_format);
  ...
  if (decklink_input_local->EnableVideoInput(
    ...
    SendErrorString(media_m96::VideoCaptureError::
  ...
  if (decklink_input_local->StartStreams() != S_OK)
    SendErrorString(
        media_m96::VideoCaptureError::kMacDeckLinkCouldNotStartCapturing, FROM_HERE,
...
HRESULT DeckLinkCaptureDelegate::VideoInputFrameArrived(
  ...
  media_m96::VideoPixelFormat pixel_format =
      media_m96::PIXEL_FORMAT_UNKNOWN;
  switch (video_frame->GetPixelFormat()) {
    case bmdFormat8BitYUV:  // A.k.a. '2vuy';
      pixel_format = media_m96::PIXEL_FORMAT_UYVY;
      break;
    case bmdFormat8BitARGB:
      pixel_format = media_m96::PIXEL_FORMAT_ARGB;
      break;
    default:
      SendErrorString(
          media_m96::VideoCaptureError::kMacDeckLinkUnsupportedPixelFormat,
          FROM_HERE, "Unsupported pixel format");
      break;
  }

  const media_m96::VideoCaptureFormat capture_format(
...
void DeckLinkCaptureDelegate::SendErrorString(media_m96::VideoCaptureError error,
                                              const base::Location& from_here,
                                              const std::string& reason) {
  base::AutoLock lock(lock_);
  if (frame_receiver_)
