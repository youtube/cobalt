...
#if !BUILDFLAG(IS_APPLE) && !BUILDFLAG(IS_STARBOARD) && !BUILDFLAG(IS_FUCHSIA)  DCHECK(broker_host_handle.is_valid());
...
NodeChannel::NodeChannel(
    ...
    : base::RefCountedDeleteOnSequence<NodeChannel>(io_task_runner),
      delegate_(delegate),
process_error_callback_(process_error_callback),      channel_(Channel::Create(this,
...
#if !BUILDFLAG(IS_APPLE) && !BUILDFLAG(IS_STARBOARD) && !BUILDFLAG(IS_FUCHSIA)  // Self-owned.
  ConnectionParams connection_params(
...
#if !BUILDFLAG(IS_NACL) && !BUILDFLAG(IS_STARBOARD)  base::AutoLock lock(channel_lock_);
