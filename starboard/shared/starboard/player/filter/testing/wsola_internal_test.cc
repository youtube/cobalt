...
int ReferenceDecimatedSearch(int decimation,
  ...
  std::unique_ptr<float[]> dot_prod = std::make_unique<float[]>(channels);
  float similarity[3];
...
int ReferenceOptimalIndex(const DecodedAudio* search_block,
  ...
  std::unique_ptr<float[]> energy_target_block =
      std::make_unique<float[]>(channels);
...
TEST(WsolaInternalTest, ExtensiveRandomTesting) {
  ...
  const int kChannelCounts[] = {1, 2, 6};  // Mono, Stereo, 5.1
  ...
  for (int channels : kChannelCounts) {
    for (int sample_rate : kSampleRates) {
      ...
      if (channels == 1) {
        // For Mono, Old Path should ALSO match Reference
        EXPECT_EQ(old_index, ref_index)
