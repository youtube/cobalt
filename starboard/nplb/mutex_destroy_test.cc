...
// Destroy is mostly tested in Create.

TEST(SbMutexDestroyTest, DestroyInvalid) {
  SbMutex mutex = kSbMutexInvalid;
  EXPECT_FALSE(SbMutexIsValid(mutex));
  bool result = SbMutexDestroy(mutex);
  EXPECT_FALSE(result);
}

TEST(SbMutexDestroyTest, DestroyHeld) {
