...
#else  return new NodeChannel(delegate, std::move(connection_params),
...
#if !BUILDFLAG(IS_APPLE) && !BUILDFLAG(IS_NACL) && !BUILDFLAG(IS_STARBOARD) && !BUILDFLAG(IS_FUCHSIA)  DCHECK(broker_host_handle.is_valid());
...
#if !BUILDFLAG(IS_APPLE) && !BUILDFLAG(IS_NACL) && !BUILDFLAG(IS_FUCHSIA)  // Self-owned.
  ConnectionParams connection_params(
