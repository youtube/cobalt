...
void ActorLoginPasswordCredentialsFetcher::Fetch(FetchResultCallback callback) {
...
void ActorLoginPasswordCredentialsFetcher::OnEligibleLoginFormManagersRetrieved(
  ...
  // `ActorLoginPasswordCredentialsFetcher` being destroyed).
  form_fetcher_->AddConsumer(this);
}

void ActorLoginPasswordCredentialsFetcher::OnFetchCompleted() {
  std::unique_ptr<BrowserSavePasswordProgressLogger> logger =
  ...
  if (mqls_logger_) {
    BuildGetCredentialsOutcome(credentials);
  }
