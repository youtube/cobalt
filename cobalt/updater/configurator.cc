...
base::flat_map<std::string, std::string> Configurator::ExtraRequestParams()
  ...
  // the cases where update is requested by client but channel stays the same,
  // but the flag name sent to Omaha remains "updaterchannelchanged" to isolate
  // the changes to Omaha configs. If it's decided to change this flag name
  // on Omaha, it'll be done in a separate PR after the Omaha configs are
  // updated.
