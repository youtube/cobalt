...
TEST(MediaCodecVideoDecoderHelpersTest, GetLowerResolution) {
  ...
  EXPECT_EQ(GetLowerResolution(size_1080p, size_720p), size_720p);
  EXPECT_EQ(GetLowerResolution(size_720p, size_1080p), size_720p);

  // Equal resolutions.
  EXPECT_EQ(GetLowerResolution(size_720p, size_720p), size_720p);

  // One present, one nullopt: returns the present resolution.
  EXPECT_EQ(GetLowerResolution(size_720p, std::nullopt), size_720p);
  EXPECT_EQ(GetLowerResolution(std::nullopt, size_720p), size_720p);

  // Both nullopt: returns nullopt.
  EXPECT_EQ(GetLowerResolution(std::nullopt, std::nullopt), std::nullopt);
  ...
  EXPECT_EQ(GetLowerResolution(narrow_tall, wide_short), size_720p);
}
