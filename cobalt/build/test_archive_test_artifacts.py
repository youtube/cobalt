...
class TestArchiveTestArtifacts(unittest.TestCase):
  ...
  @mock.patch('archive_test_artifacts._make_tar')
  @mock.patch('shutil.which', return_value='/depot/tools/vpython3')
  def test_create_archive_depot_tools(self, unused_mock_which, mock_make_tar):
    # Setup mock runtime_deps
    target_name = 'my_test'
    deps_file = os.path.join(self.out_dir, f'{target_name}.runtime_deps')
    with open(deps_file, 'w', encoding='utf-8') as f:
    ...
    self.assertEqual(depot_tools_entry[0], ['tools'])
    self.assertEqual(depot_tools_entry[1], '/depot')
