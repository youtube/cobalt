...
int  pthread_cond_broadcast(pthread_cond_t * cond) {
  return SbConditionVariableBroadcast((SbConditionVariable*)cond->cond_buffer)? 0: -1;
}

int  pthread_cond_destroy(pthread_cond_t * cond) {
  return SbConditionVariableDestroy((SbConditionVariable*)cond->cond_buffer)? 0: -1;
}

int  pthread_cond_init(pthread_cond_t* cond, const pthread_condattr_t* attr) {
  return SbConditionVariableCreate((SbConditionVariable*)cond->cond_buffer, NULL /* mutex */)? 0: -1;
}

int  pthread_cond_signal(pthread_cond_t* cond) {
  return SbConditionVariableSignal((SbConditionVariable*)cond->cond_buffer)? 0: -1;
}
...
int  pthread_cond_timedwait(pthread_cond_t* cond, pthread_mutex_t* mutex, const struct timespec* t) {
  ...
  SbConditionVariableResult result = SbConditionVariableWaitTimed((SbConditionVariable*)cond->cond_buffer, (SbMutex*)mutex->mutex_buffer, timeout_duration_microsec);
...
int  pthread_cond_wait(pthread_cond_t* cond, pthread_mutex_t* mutex) {
  SbConditionVariableResult result = SbConditionVariableWait((SbConditionVariable*)cond->cond_buffer, (SbMutex*)mutex->mutex_buffer);
  if (result == kSbConditionVariableSignaled) {
    return 0;
  }
  return -1;
