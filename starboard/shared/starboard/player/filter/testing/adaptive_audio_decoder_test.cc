 ...
 protected:
  ...
  AdaptiveAudioDecoderTest() {
    ...
    for (auto filename : params) {
      dmp_readers_.emplace_back(ResolveTestFileName(filename).c_str());
    }
  ...
  void WriteSingleInput(const VideoDmpReader& dmp_reader, size_t buffer_index) {
    ...
    audio_decoder_->Decode(
        dmp_reader.GetAudioInputBuffer(buffer_index),
...
TEST_P(AdaptiveAudioDecoderTest, SingleInput) {
  ...
  for (auto& dmp_reader : dmp_readers_) {
    ...
    auto input_buffer = dmp_reader.GetAudioInputBuffer(buffer_index);
    ...
    auto next_input_buffer = dmp_reader.GetAudioInputBuffer(buffer_index);
