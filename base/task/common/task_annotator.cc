...
void InitThreadLocalHashKey() {
  pthread_key_create(&s_thread_local_hash_key , NULL);
  DCHECK(s_thread_local_hash_key);
}
...
void InitThreadLocalTrackerKey() {
  pthread_key_create(&s_thread_local_tracker_key , NULL);
  DCHECK(s_thread_local_tracker_key);
}
