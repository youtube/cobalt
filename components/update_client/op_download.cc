...
void HandleAvailableSpace(
  ...
  cancellation->OnCancel(crx_downloader->StartDownload(
      urls, hash,
base::BindOnce(&DownloadComplete, id, crx_downloader, cancellation,                     event_adder, std::move(callback))));
#endif
}
...
base::OnceClosure DownloadOperation(
  ...
  base::ThreadPool::PostTaskAndReplyWithResult(
      ...
      base::BindOnce(&HandleAvailableSpace, config, id, cancellation,
                     ...
                     crx_str,
#endif
                     std::move(callback)));  return base::BindOnce(&Cancellation::Cancel, cancellation);
}
