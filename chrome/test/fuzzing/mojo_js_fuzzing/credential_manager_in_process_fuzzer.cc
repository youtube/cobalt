...
#include "chrome/test/fuzzing/mojo_js_fuzzing/credential_manager_fuzzer_grammar.h"
#include "chrome/test/fuzzing/mojo_js_fuzzing/credential_manager_fuzzer_grammar.pb.h"
...
// rule.
class CredentialManagerInProcessFuzzer
    : public InProcessBinaryProtoFuzzer<
          domatolpm::generated::credential_manager_fuzzer_grammar::fuzzcase> {
 public:
  using FuzzCase =
      domatolpm::generated::credential_manager_fuzzer_grammar::fuzzcase;
  CredentialManagerInProcessFuzzer();
...
REGISTER_BINARY_PROTO_IN_PROCESS_FUZZER(CredentialManagerInProcessFuzzer)

void CredentialManagerInProcessFuzzer::SetUpOnMainThread() {
  ...
  CHECK(ui_test_utils::NavigateToURL(
      browser(), embedded_https_test_server().GetURL(
                     "/CredentialManager_in_process_fuzzer.html")));
...
CredentialManagerInProcessFuzzer::CredentialManagerInProcessFuzzer()
...
int CredentialManagerInProcessFuzzer::Fuzz(const FuzzCase& fuzz_case) {
  static Environment env;
  domatolpm::Context ctx;
  CHECK(domatolpm::credential_manager_fuzzer_grammar::handle_fuzzer(&ctx,
                                                                    fuzz_case));
