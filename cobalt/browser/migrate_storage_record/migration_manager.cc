...
ItemPtrs<net::CanonicalCookie> ToCanonicalCookies(
...
ItemPtrs<StringPair> ToLocalStorageItems(
    const url::Origin& page_origin,
    const cobalt::storage::Storage& storage) {
  ItemPtrs<StringPair> entries;
  for (const auto& local_storages : storage.local_storages()) {
    ...
    for (const auto& local_storage_entry :
      ...
      entries.emplace_back(new StringPair(local_storage_entry.key(),
...
// static
void MigrationManager::DoTasks(Tasks tasks, base::OnceClosure callback) {
...
// static
Task MigrationManager::GroupTasks(Tasks tasks) {
  return base::BindOnce(&DoTasks, std::move(tasks));
}
...
// static
Task MigrationManager::CookieTask(content::WeakDocumentPtr weak_document_ptr,
                                  ItemPtrs<net::CanonicalCookie> cookies) {
  Tasks tasks;
  ...
  for (auto& cookie : cookies) {
    tasks.push_back(base::BindOnce(
        [](content::WeakDocumentPtr weak_document_ptr,
           ItemPtr<net::CanonicalCookie> cookie, base::OnceClosure callback) {
...
Task MigrationManager::LocalStorageTask(
  ...
  for (auto& pair : pairs) {
    tasks.push_back(base::BindOnce(
        [](content::WeakDocumentPtr weak_document_ptr, ItemPtr<StringPair> pair,
           base::OnceClosure callback) {
          RenderFrameHost(weak_document_ptr)
              ->ExecuteJavaScriptMethod(
                  base::ASCIIToUTF16(std::string("localStorage")),
