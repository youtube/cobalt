...
- [ ] **Next Steps (Halt)**
    ...
    > [!IMPORTANT]
    > **HALT** and ask the user to approve next steps. Do NOT attempt to investigate or fix the issues yourself.
    *   **Triage Hotlist Bugs**: Suggest creating bugs for new issues (marked as `Bug: NEW`) and adding them to the [Gardener Hotlist](https://b.corp.google.com/hotlists/7245259), or adding existing matching bugs to the hotlist if they are not already there.
    *   **Investigate Fixes**: Create a list of investigative tasks (e.g. "Check if PR X is merged to branch Y") for the user to approve.
    *   **Restart Jobs**: Suggest which jobs to restart.
