...
std::unique_ptr<icu::TimeZone> TimeZone::CreateIcuTimezoneFromParsedData(
  ...
  const int32_t raw_offset_ms = -tz_data.std_offset * 1000;
  ...
  if (!tz_data.start_rule) {
    return std::make_unique<icu::SimpleTimeZone>(
        raw_offset_ms, icu::UnicodeString::fromUTF8(tz_data.std_name));
  }

  // icu::SimpleTimeZone only supports the Month/Week/Day rule format.
  // This is the most common format, e.g., "M3.2.0". It does not support
  // Julian day rules (e.g., "J60" or "n59").
  if (tz_data.start_rule->date_rule.format !=
          tz::DateRule::Format::MonthWeekDay ||
      !tz_data.end_rule ||
      tz_data.end_rule->date_rule.format !=
          tz::DateRule::Format::MonthWeekDay) {
    // Unsupported rule format. Return null to indicate failure.
    return nullptr;
  }

  // At this point, we know we have Mm.w.d rules for both start and end.
  const auto& start = *tz_data.start_rule;
  const auto& end = *tz_data.end_rule;
  ...
  const int32_t dst_savings_ms =
      tz_data.dst_offset ? (tz_data.std_offset - *tz_data.dst_offset) * 1000
                         : 3600 * 1000;  // Default to 1 hour (3600 seconds).

  // --- Convert POSIX rules to ICU SimpleTimeZone parameters ---

  // Month: POSIX is 1-12, ICU is 0-11 (e.g., UCAL_JANUARY)
  const int32_t start_month = start.date_rule.month - 1;
  const int32_t end_month = end.date_rule.month - 1;

  // Day of Week: POSIX is 0=Sun, ICU is 1=Sun (e.g., UCAL_SUNDAY)
  const int32_t start_day_of_week = start.date_rule.day + 1;
  const int32_t end_day_of_week = end.date_rule.day + 1;

  // Week in Month: POSIX 1-4 is Nth week, 5 is last. ICU uses -1 for last.
  const int32_t start_day_in_month =
      (start.date_rule.week == 5) ? -1 : start.date_rule.week;
  const int32_t end_day_in_month =
      (end.date_rule.week == 5) ? -1 : end.date_rule.week;

  // Transition Time: POSIX is seconds from midnight, ICU is milliseconds.
  const int32_t start_time_ms = start.time * 1000;
  const int32_t end_time_ms = end.time * 1000;

  UErrorCode status = U_ZERO_ERROR;
  auto custom_tz = std::make_unique<icu::SimpleTimeZone>(
      raw_offset_ms, icu::UnicodeString::fromUTF8(tz_data.std_name),
      start_month, start_day_in_month, start_day_of_week, start_time_ms,
      icu::SimpleTimeZone::WALL_TIME,  // POSIX times are local wall times.
      end_month, end_day_in_month, end_day_of_week, end_time_ms,
      icu::SimpleTimeZone::WALL_TIME, dst_savings_ms, status);

  if (U_FAILURE(status)) {
    return nullptr;
  }

  // The custom timezone is now fully defined by its rules and offsets.
  return custom_tz;
}

void TimeZone::MaybeApplyCorrection(const icu::UnicodeString& time_zone_id) {
  // Corrections for zones where ICU data doesn't match test expectations.
  static constexpr std::array<std::pair<std::string_view, Correction>, 24>
      kCorrections = {{
          ...
          // "GB-Eire" was {_, "GMT+1"}
          {"GB-Eire", {nullptr, "BST"}},
          // "HST" was {"GMT-10", "GMT-10", 0}
          {"HST", {"HST", "HPT", 1}},
          // "MET" was {"GMT+1", "GMT+2"}
          {"MET", {"CET", "CEST"}},
          // "MST" was {"GMT-7", "GMT-7", 0}
          {"MST", {"MST", "MDT", 1}},
          // "WET" was {"GMT", "GMT+1"}
          {"WET", {"WET", "WEST"}},
      }};
...
bool TimeZone::ExtractZoneName(const icu::TimeZone& tz,
  ...
  if (date_for_name) {
    ...
    calendar.set(2023, is_daylight ? UCAL_JULY : UCAL_JANUARY, 1);
...
void TimeZone::TZSet() {
  ...
  std::optional<tz::TimezoneData> tz_data_optional =
      tz::ParsePosixTz(timezone_name);
  if (tz_data_optional) {
    const auto& tz_data = *tz_data_optional;
    ...
    std::unique_ptr<icu::TimeZone> custom_tz =
        TimeZone::CreateIcuTimezoneFromParsedData(tz_data);
    ...
    timezone = tz_data.std_offset;
    SafeCopyToName(tz_data.std_name, std_name_);
    tzname[0] = std_name_.data();

    daylight = tz_data.dst_name.has_value() ? 1 : 0;
    SafeCopyToName(daylight ? *tz_data.dst_name : tz_data.std_name, dst_name_);
    tzname[1] = dst_name_.data();
    return;
  }
