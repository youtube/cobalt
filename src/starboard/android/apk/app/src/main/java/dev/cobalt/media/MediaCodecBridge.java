...
class MediaCodecBridge {
  ...
  private MediaCodecBridge(
    ...
    // TODO: support for non tunneled mode
  ...
  public static void createVideoMediaCodecBridge(
    ...
    boolean findTunneledDecoder = tunnelingAudioSessionId != -1;
    ...
    if (findVideoDecoderResult.name.equals("") && findHDRDecoder) {
      ...
      findVideoDecoderResult =
          MediaCodecUtil.findVideoDecoder(mime, isSecure, 0, 0, 0, 0, false, requireSoftwareCodec, findTunneledDecoder);
    }
    try {
    ...
    if (tunnelingAudioSessionId != -1) {
      ...
      Log.d(TAG, String.format("Enabled tunneled mode playback on audio session %d",
                               tunnelingAudioSessionId));
    } else {
      Log.d(TAG, "Playback is not tunneled.");
    }
