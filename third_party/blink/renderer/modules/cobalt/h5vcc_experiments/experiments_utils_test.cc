...
TEST(ExperimentsUtilsTest, AllFieldsPresent) {
  ...
  std::optional<base::Value::Dict> result = ToBaseValueDict(config);
...
TEST(ExperimentsUtilsTest, ToBaseValueDict_MissingFeatures) {
  auto* config = MakeGarbageCollected<ExperimentConfiguration>();
...
TEST(ExperimentsUtilsTest, ToBaseValueDict_MissingFeatureParams) {
  auto* config = MakeGarbageCollected<ExperimentConfiguration>();
...
TEST(ExperimentsUtilsTest, ToBaseValueDict_MissingExperimentIds) {
  auto* config = MakeGarbageCollected<ExperimentConfiguration>();
  config->setFeatures(Vector<std::pair<String, bool>>());
  config->setFeatureParams(
      HeapVector<std::pair<String, Member<V8UnionBooleanOrLongOrString>>>());
...
TEST(ExperimentsUtilsTest, ToBaseValueDict_AllFieldsPresentButEmpty) {
  auto* config = MakeGarbageCollected<ExperimentConfiguration>();
