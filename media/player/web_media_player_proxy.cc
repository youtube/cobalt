...
void WebMediaPlayerProxy::KeyError(const std::string& key_system,
                                   ...
                                   int system_code) {
  render_loop_->PostTask(FROM_HERE,
                         base::Bind(&WebMediaPlayerProxy::KeyErrorTask,
                                    this,
                                    key_system,
                                    session_id,
                                    error_code,
...
void WebMediaPlayerProxy::KeyMessage(const std::string& key_system,
  ...
  render_loop_->PostTask(FROM_HERE,
                         base::Bind(&WebMediaPlayerProxy::KeyMessageTask,
                                    this,
                                    key_system,
                                    session_id,
                                    message,
...
void WebMediaPlayerProxy::NeedKey(const std::string& key_system,
                                  ...
                                  int init_data_size) {
  render_loop_->PostTask(FROM_HERE,
                         base::Bind(&WebMediaPlayerProxy::NeedKeyTask,
                                    this,
                                    key_system,
                                    session_id,
                                    type,
                                    base::Passed(&init_data),
                                    init_data_size));
}
...
void WebMediaPlayerProxy::KeyMessageTask(const std::string& key_system,
  ...
  if (webmediaplayer_) {
    ...
    webmediaplayer_->OnKeyMessage(
        key_system, session_id, message, default_url_gurl);
  }
}
