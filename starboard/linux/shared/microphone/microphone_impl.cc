...
const int kSampleRateInHz = 48000;
const int kChannels = 1;
const int kFramesPerBuffer = 480;
const int kBufferSizeInBytes = kFramesPerBuffer * kChannels * sizeof(int16_t);
...
class SbMicrophoneImpl : public SbMicrophonePrivate {
 public:
  ...
  bool Open() override {
    ...
    error =
        snd_pcm_hw_params_set_rate_near(handle_, hw_params, &sample_rate, NULL);
    if (error < 0) {
      Close();
      return false;
    }
    ...
    error = snd_pcm_nonblock(handle_, 1);
    if (error < 0) {
      SB_LOG(ERROR) << "Failed to set non-blocking mode: "
                    << snd_strerror(error);
      Close();
      return false;
    }
  ...
  for (void** n = hints; *n != NULL; n++) {
    ...
    if (name != nullptr && (ioid == nullptr || strcmp(ioid, "Input") == 0)) {
      if (out_info_array && count < info_array_size) {
        ...
        info->min_read_size = kFramesPerBuffer;
...
SbMicrophone SbMicrophonePrivate::CreateMicrophone(SbMicrophoneId id,
  ...
  if (!SbMicrophoneIdIsValid(id) ||
      ...
      buffer_size_bytes > kBufferSizeInBytes || buffer_size_bytes < 0) {
  ...
  s_microphone = new SbMicrophoneImpl();
  return s_microphone;
}
