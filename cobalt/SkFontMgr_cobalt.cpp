...
#include "cobalt/SkFontMgr_cobalt.h"
...
int SkFontMgr_Cobalt::onCountFamilies() const { return family_names_.size(); }
...
sk_sp<SkFontStyleSet> SkFontMgr_Cobalt::onCreateStyleSet(int index) const {
  ...
  if (family_iterator != name_to_family_map_.end()) {
    return sk_make_sp<SkFontStyleSet>(SkRef(family_iterator->second));
  }
...
sk_sp<SkTypeface> SkFontMgr_Cobalt::onMatchFamilyStyleCharacter(
  ...
  for (int bcp47_index = bcp47_count; bcp47_index-- > 0;) {
    ...
    while (!language.GetTag().isEmpty()) {
      sk_sp<SkTypeface> matching_typeface = font_manager->FindFamilyStyleCharacter(
...
sk_sp<SkTypeface> SkFontMgr_Cobalt::onMakeFromData(sk_sp<SkData> data,
  ...
  return this->makeFromStream(std::make_unique<SkMemoryStream>(std::move(data)),
                        face_index);
}
...
sk_sp<SkTypeface> SkFontMgr_Cobalt::FindFamilyStyleCharacter(
  ...
  if (default_families_.size() > 1) {
    for (int i = 1; i < default_families_.size(); ++i) {
      sk_sp<SkFontStyleSet_Cobalt> family = default_families_[i];
      if (family->ContainsCharacter(style, character)) {
        sk_sp<SkTypeface> matching_typeface = family->MatchStyleWithoutLocking(style);
        if (matching_typeface) {
          return matching_typeface;
        }
      }
  ...
  for (int i = 0; i < fallback_families->size(); ++i) {
    sk_sp<SkFontStyleSet_Cobalt> family = (*fallback_families)[i];
    if (family->ContainsCharacter(style, character)) {
      sk_sp<SkTypeface> matching_typeface = family->MatchStyleWithoutLocking(style);
      if (matching_typeface) {
        return matching_typeface;
...
SkFontMgr_Cobalt::StyleSetArray* SkFontMgr_Cobalt::GetMatchingFallbackFamilies(
  ...
  if (language_fallback_families == NULL) {
    ...
    for (StyleSetArray::iterator iter = fallback_families_.begin();
      ...
      sk_sp<SkFontStyleSet_Cobalt> fallback_family = *iter;
