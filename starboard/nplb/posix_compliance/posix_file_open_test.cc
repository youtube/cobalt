...
#define SB_FILE_OPEN_TEST_CONTEXT                                      \
  ...
  if (!existing) {
    ...
    // EXPECT_FALSE(stat(filename.c_str(), &file_info) == 0) <<
    // SB_FILE_OPEN_TEST_CONTEXT; if (stat(filename.c_str(), &file_info) == 0) {
  ...
  // int file = open(filename.c_str(), open_flags, &created, &error);
  if (!expected_success) {
