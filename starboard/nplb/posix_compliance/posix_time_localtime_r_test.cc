...
TEST(PosixLocaltimeRTests, HandlesSpecificKnownPositiveTimeInDifferentTimezone) {
...
TEST(PosixLocaltimeRTests, HandlesNegativeTimeBeforeEpoch) {
  ScopedTZ tz("America/New_York");
...
TEST(PosixLocaltimeRTests, HandlesTimeTHighValueOverflow) {
  ...
  // tm_year (year - 1900) would exceed INT_MAX. Thus, localtime_r is expected to
  // return nullptr.
  EXPECT_EQ(local_result_ptr, nullptr)
