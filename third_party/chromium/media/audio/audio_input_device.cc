...
class AudioInputDevice::AudioThreadCallback
 ...
 private:
  ...
  std::vector<std::unique_ptr<const media_m96::AudioBus>> audio_buses_;
...
void AudioInputDevice::DetectedDeadInputStream() {
  callback_->OnCaptureError(media_m96::AudioCapturerSource::ErrorCode::kUnknown,
...
void AudioInputDevice::AudioThreadCallback::MapSharedMemory() {
  ...
  for (uint32_t i = 0; i < total_segments_; ++i) {
    const media_m96::AudioInputBuffer* buffer =
        reinterpret_cast<const media_m96::AudioInputBuffer*>(ptr);
    audio_buses_.push_back(
        media_m96::AudioBus::WrapReadOnlyMemory(audio_parameters_, buffer->audio));
    ptr += segment_length_;
  }
...
void AudioInputDevice::AudioThreadCallback::Process(uint32_t pending_data) {
  ...
  if (buffer->params.id != last_buffer_id_ + 1) {
    ...
    capture_callback_->OnCaptureError(
        media_m96::AudioCapturerSource::ErrorCode::kUnknown, message);
  }
  ...
  const media_m96::AudioBus* audio_bus = audio_buses_[current_segment_id_].get();
