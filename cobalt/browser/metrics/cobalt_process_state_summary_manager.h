...
inline constexpr uint8_t kProcessStateSummaryVersion = 2;
// Fixed payload length in bytes (strictly <= 128 bytes limit in Android OS).
inline constexpr size_t kProcessStateSummaryPayloadSize = 40;
// Legacy version 1 payload size.
inline constexpr size_t kProcessStateSummaryV1PayloadSize = 28;

// Flag bits packed into snapshot flags byte.
enum ProcessStateFlags : uint8_t {
  kFlagForeground = 1 << 0,
  kFlagMediaPlaying = 1 << 1,
  kFlagStartupGuardArmed = 1 << 2,
  kFlagStartupGuardTriggeredKill = 1 << 3,
};
inline constexpr uint8_t kProcessStateKnownFlagsMask =
    kFlagForeground | kFlagMediaPlaying | kFlagStartupGuardArmed |
    kFlagStartupGuardTriggeredKill;
...
struct ProcessStateSnapshot {
  ...
  bool is_startup_guard_armed() const {
    return (flags & kFlagStartupGuardArmed) != 0;
  }
  bool was_killed_by_startup_guard() const {
    return (flags & kFlagStartupGuardTriggeredKill) != 0;
  }
...
// Standard Android process exit reasons matching Chromium UMA enum values.
enum class AndroidExitReason {
  kAnr = 0,
  kCrash = 1,
  kCrashNative = 2,
  kDependencyDied = 3,
  kExcessiveResourceUsage = 4,
  kExitSelf = 5,
  kInitializationFailure = 6,
  kLowMemory = 7,
  kOther = 8,
  kPermissionChange = 9,
  kSignaled = 10,
  ...
  kApiFailed = 14,
  kFreezer = 15,
  kPackageStateChange = 16,
  kPackageUpdated = 17,
  kMaxValue = kPackageUpdated,
};
...
// Manages capturing, serializing, and synchronizing compact process state
// summaries to the Android OS via ActivityManager.setProcessStateSummary() (API
// 30+) during a session, and recovering and reporting prior-session state and
// exit reasons on startup.
//
// Lifetime / Ownership:
//   Singleton managed via base::NoDestructor; created on first call to
//   GetInstance() and persists for the lifetime of the browser process.
//
// Threading Model:
//   Thread-safe. All mutable internal state is guarded by an internal
//   base::Lock, allowing methods such as UpdateMemoryFootprint(),
//   UpdateTrimMemoryLevel(), and SetFlags() to be invoked safely from any
//   thread (e.g. Memory Monitor thread, main UI thread, or background tasks).
class CobaltProcessStateSummaryManager {
 public:
  ...
  // Records a StartupGuard milestone (0..63) in the startup milestone bitmask.
  void SetStartupMilestone(uint8_t milestone);

  // Updates whether StartupGuard is currently armed.
  void SetStartupGuardArmed(bool is_armed);

  // Marks that StartupGuard triggered a watchdog self-kill and synchronizes
  // to the Android OS.
  void SetStartupGuardTriggeredKill();
 ...
 private:
  ...
  std::optional<std::vector<uint8_t>> PreparePayloadLocked()
      EXCLUSIVE_LOCKS_REQUIRED(lock_);
  void SyncToSystemServer(const std::vector<uint8_t>& payload);
  ...
  uint64_t startup_milestones_ GUARDED_BY(lock_) = 0;
  uint8_t highest_milestone_ GUARDED_BY(lock_) = 0;
