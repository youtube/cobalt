...
scoped_refptr<UDPSocket> UdpSocketFactory::CreateAndBind(
    const IPEndPoint& address) {
  SocketDescriptor s = NativeCreateUdpSocket(address.GetFamily());
  if (s == UDPSocket::kInvalidSocket)
    return NULL;
  ...
  scoped_refptr<UDPSocket> sock(new UDPSocket(s));
  return sock;
}
...
void DialUdpSocketFactory::SetupSocketAfterBind(SocketDescriptor s) {
  ...
  if (::setsockopt(s, IPPROTO_IP, IP_ADD_MEMBERSHIP, &imreq, sizeof(imreq)) <
      0) {
