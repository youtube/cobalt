...
bool OSMetrics::FillDetailedMetrics(base::ProcessHandle handle,
...
#if BUILDFLAG(IS_COBALT)
  // If the delegate already has stats (e.g. from coalesced RequestDetailedDump),
  // we can use them directly. Otherwise we need to scan.
  // Note: DetailedMetricsDelegate usually adds a "total_pss" key, so size is at least 1
  // if it ran, but it might be empty if it didn't run at all or failed.
  // We check if size <= 1 to cover the case where only "total_pss" is present (no categories).
  delegate->GetAndResetStats(stats);
  if (stats.size() <= 1) {
    if (!SmapsCategorizer::ScanSmaps(std::move(file_to_scan), delegate)) {
  ...
  for (const auto& entry : stats) {
    categories_kb[entry.first] = base::saturated_cast<uint32_t>(entry.second);
  }
