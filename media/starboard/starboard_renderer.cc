...
void StarboardRenderer::ApplyPendingVideoConfig() {
  ...
  LOG(INFO) << "Applying pending Video config change. from_changeType="
            << pending_video_config_->from_changeType();
...
void StarboardRenderer::ApplyPendingAudioConfig() {
  ...
  LOG(INFO) << "Applying pending Audio config change. from_changeType="
            << pending_audio_config_->from_changeType();
...
void StarboardRenderer::OnDemuxerStreamRead(
  ...
  if (status == DemuxerStream::kOk) {
    ...
    if (stream == audio_stream_) {
      ...
      if (config.from_changeType()) {
        pending_audio_config_ = config;
        LOG(INFO)
            << "Pending Audio config change recorded. from_changeType=true";
      } else {
      ...
      if (config.from_changeType()) {
        ...
        LOG(INFO)
            << "Pending Video config change recorded. from_changeType=true";
      } else {
