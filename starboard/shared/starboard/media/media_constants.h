...
inline int g_video_buffer_budget_1080p = 30 * 1024 * 1024;
inline int g_video_buffer_budget_4k_sdr = 100 * 1024 * 1024;
inline int g_video_buffer_budget_4k_hdr = 160 * 1024 * 1024;
...
#if BUILDFLAG(IS_ANDROID)
inline int g_video_buffer_budget_above_4k = 200 * 1024 * 1024;
#else
inline int g_video_buffer_budget_above_4k = 300 * 1024 * 1024;
#endif

constexpr int k1080pArea = 1920 * 1080;
constexpr int k4KArea = 3840 * 2160;

inline int GetAreaBasedVideoBufferBudget(int resolution_width,
  ...
  int64_t resolution_area =
      (resolution_width == kSbMediaVideoResolutionDimensionInvalid ||
       resolution_height == kSbMediaVideoResolutionDimensionInvalid)
          ? 0
          : static_cast<int64_t>(resolution_width) * resolution_height;
...
inline int GetLegacyVideoBufferBudget(int resolution_width,
                                      int resolution_height,
                                      int bits_per_pixel) {
...
inline int GetDefaultVideoBufferBudget(int resolution_width,
                                       int resolution_height,
                                       int bits_per_pixel) {
#if BUILDFLAG(IS_ANDROID)
  if (starboard::features::FeatureList::IsEnabled(
          starboard::features::kEnableAreaBasedVideoBufferBudget)) {
    return GetAreaBasedVideoBufferBudget(resolution_width, resolution_height,
                                         bits_per_pixel);
  } else {
    return GetLegacyVideoBufferBudget(resolution_width, resolution_height,
                                      bits_per_pixel);
  }
#else
