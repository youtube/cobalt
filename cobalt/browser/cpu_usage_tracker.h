...
class CpuUsageTracker : base::MessageLoopCurrent::DestructionObserver {
 public:
  static CpuUsageTracker* GetInstance();

  void Initialize(persistent_storage::PersistentSettings*);
  void UpdateIntervalsDefinition(std::unique_ptr<base::Value>);
 ...
 private:
  ...
  void TotalIntervalTask(uint64_t id);
  void PerThreadIntervalTask(uint64_t id);
  ...
  using IdIntervalContextPair =
      std::pair<const uint64_t, std::unique_ptr<IntervalContext>>;
  ...
  std::map<uint64_t, std::unique_ptr<IntervalContext>> id_to_interval_context_;
  base::Lock lock_;
