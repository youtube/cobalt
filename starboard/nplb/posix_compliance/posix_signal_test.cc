...
void PipeWritingSignalHandler(int signum) {
  ...
  ssize_t result = write(g_signal_pipe_write_fd, &signal_byte, 1);
...
class PosixSignalTest : public ::testing::Test {
 protected:
  ...
  bool WaitForSignal(int* signum, int timeout_ms) {
    struct epoll_event event;
...
TEST_F(PosixSignalTest, SignalLegacyHandler) {
  ...
  EXPECT_TRUE(WaitForSignal(&received_signal, 1000));
...
TEST_F(PosixSignalTest, PthreadSigmaskBlocksAndUnblocksSignal) {
  ...
  ASSERT_EQ(pthread_sigmask(SIG_SETMASK, &old_mask, NULL), 0);
...
TEST_F(PosixSignalTest, PthreadKillSendsSignalToSpecificThread) {
  ...
  pthread_t thread;
  ASSERT_EQ(pthread_create(&thread, NULL, PauseThread, &args), 0);
