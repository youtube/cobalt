...
TEST_F(UrlPlayerRendererWrapperIntegrationTest,
  ...
  // Phase 1: InitializeWithUrl triggers GetSbWindowHandle via Mojo.
  base::RunLoop window_handle_loop;
  base::MockOnceCallback<void(PipelineStatus)> init_cb;
  EXPECT_CALL(client_extension_, GetSbWindowHandle())
      .WillOnce(RunOnceClosure(window_handle_loop.QuitClosure()));
  renderer_wrapper_->InitializeWithUrl(&renderer_client_, GURL(kSourceUrl),
                                       init_cb.Get());
  window_handle_loop.Run();

  // Phase 2: Window handle arrives, player is created.
  base::RunLoop player_created_loop;
  ...
  EXPECT_CALL(client_extension_,
      ...
      .WillOnce(RunOnceClosure(player_created_loop.QuitClosure()));
  renderer_extension_->OnSbWindowHandleReady(kWindowHandle);
  player_created_loop.Run();

  // Phase 3: Player reports initialized.
  base::RunLoop init_loop;
  ASSERT_TRUE(player_status_cb);
  EXPECT_CALL(init_cb, Run(HasStatusCode(PIPELINE_OK)))
      .WillOnce(RunOnceClosure(init_loop.QuitClosure()));
  player_status_cb(player, player_context, kSbPlayerStateInitialized,
                   SB_PLAYER_INITIAL_TICKET);
  init_loop.Run();

  // Phase 4: Player reports presenting with video dimensions.
  base::RunLoop presenting_loop;
  ...
  EXPECT_CALL(client_extension_, PaintVideoHoleFrame(gfx::Size(1920, 1080)))
      .WillOnce(RunOnceClosure(presenting_loop.QuitClosure()));
