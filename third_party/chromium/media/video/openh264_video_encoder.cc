...
#include "third_party/chromium/media/video/openh264_video_encoder.h"
...
void OpenH264VideoEncoder::Initialize(VideoCodecProfile profile,
                                      ...
                                      OutputCB output_cb,
                                      StatusCB done_cb) {
  done_cb = base::BindPostTaskToCurrentDefault(std::move(done_cb));
  if (codec_) {
  ...
  options_ = options;
  output_cb_ = base::BindPostTaskToCurrentDefault(std::move(output_cb));
