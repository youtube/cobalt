...
#include "third_party/chromium/media/gpu/vaapi/vaapi_video_decoder.h"
#elif BUILDFLAG(USE_V4L2_CODEC)
#include "third_party/chromium/media/gpu/v4l2/v4l2_video_decoder.h"
#else
...
// number of frames in media_m96::Pipeline plus the current processing frame.
...
void VideoDecoderPipeline::OnFrameConverted(scoped_refptr<VideoFrame> frame) {
  ...
  // task runner, even though media_m96::VideoDecoder states frames should be output
...
void VideoDecoderPipeline::OnDecoderWaiting(WaitingReason reason) {
  DVLOGF(3);
  DCHECK_CALLED_ON_VALID_SEQUENCE(decoder_sequence_checker_);
  if (reason == media_m96::WaitingReason::kDecoderStateLost)
...
VideoDecoderPipeline::PickDecoderOutputFormat(
  ...
  std::unique_ptr<ImageProcessor> image_processor =
      ImageProcessorFactory::CreateWithInputCandidates(
          ...
          base::BindPostTaskToCurrentDefault(base::BindRepeating(&VideoDecoderPipeline::OnError,
                                                decoder_weak_this_,
