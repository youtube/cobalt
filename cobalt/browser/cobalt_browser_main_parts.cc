...
int CobaltBrowserMainParts::PreEarlyInitialization() {
  global_features_ = std::make_unique<GlobalFeatures>(
      TakePrefService(exp_config_), TakePrefService(local_state_),
      TakeMetricsServicesManager(), metrics_services_manager_client_);
...
std::unique_ptr<PrefService> CobaltBrowserMainParts::TakePrefService(
    std::unique_ptr<PrefService>& pref_service) {
  return std::move(pref_service);
}

std::unique_ptr<metrics_services_manager::MetricsServicesManager>
CobaltBrowserMainParts::TakeMetricsServicesManager() {
...
void CobaltBrowserMainParts::SetExperimentConfig(
    std::unique_ptr<PrefService> exp_config) {
  exp_config_ = std::move(exp_config);
}

void CobaltBrowserMainParts::SetLocalState(
