// Copyright 2020 The Chromium Authors. All rights reserved.
// Use of this source code is governed by a BSD-style license that can be
// found in the LICENSE file.

#ifndef THIRD_PARTY_CHROMIUM_MEDIA_BASE_SPEECH_RECOGNITION_CLIENT_H_
#define THIRD_PARTY_CHROMIUM_MEDIA_BASE_SPEECH_RECOGNITION_CLIENT_H_

#include <memory>

#include "base/callback.h"
#include "third_party/chromium/media/base/audio_buffer.h"
#include "third_party/chromium/media/base/audio_bus.h"
#include "third_party/chromium/media/base/media_export.h"

namespace media_m96 {

// The interface for the speech recognition client used to transcribe audio into
// captions.
class MEDIA_EXPORT SpeechRecognitionClient {
 public:
  using OnReadyCallback = base::OnceCallback<void()>;

  virtual ~SpeechRecognitionClient() = default;

  virtual void AddAudio(scoped_refptr<AudioBuffer> buffer) = 0;

  virtual void AddAudio(std::unique_ptr<media_m96::AudioBus> audio_bus,
                        int sample_rate,
                        media_m96::ChannelLayout channel_layout) = 0;

  virtual bool IsSpeechRecognitionAvailable() = 0;

  virtual void SetOnReadyCallback(OnReadyCallback callback) = 0;
};

}  // namespace media_m96

#endif  // THIRD_PARTY_CHROMIUM_MEDIA_BASE_SPEECH_RECOGNITION_CLIENT_H_
