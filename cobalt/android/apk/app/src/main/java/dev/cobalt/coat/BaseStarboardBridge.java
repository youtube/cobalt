...
public class BaseStarboardBridge {
  ...
  protected CobaltSystemConfigChangeReceiver mSysConfigChangeReceiver;
  protected CobaltTextToSpeechHelper mTtsHelper;
  protected AudioOutputManager mAudioOutputManager;
  protected AudioPermissionRequester mAudioPermissionRequester;
  protected AdvertisingId mAdvertisingId;
  protected final Context mAppContext;
  protected final Holder<Activity> mActivityHolder;
  protected final Holder<Service> mServiceHolder;
  private final String[] mArgs;
  protected final long mNativeApp;
  protected final Runnable mStopRequester =
      new Runnable() {
        @Override
        public void run() {
          ...
          // https://source.corp.google.com/piper///depot/google3/third_party/cobalt/app/android/coat/branch_25_lts/java/dev/cobalt/coat/BaseStarboardBridge.java;l=96
          afterStopped();
        }
      };
  ...
  public BaseStarboardBridge(
    ...
    BaseStarboardBridgeJni.get().setYoutubeCertificationScope(getSystemProperty(YTS_CERT_SCOPE_SYSTEM_PROPERTY));
  ...
  // The Natives interface must mirror chrobalt's BaseStarboardBridge @NativeMethods:
  // the apk's JNI companion + the loader's jni_registration are generated from
  // //cobalt/android:jni_headers (chrobalt BaseStarboardBridge), and jni_zero asserts
  // the Java-side generate_jni matches the native lib's. So currentMonotonicTime/
  // initializePlatformAudioSink/isDevelopmentBuild stay here even though this apk
  // never calls them (same constraint as the eager @CalledByNative no-op stubs).
  @NativeMethods
  interface Natives {
  ...
  /** Shuts down the TTS + advertising-ID helpers on teardown. */
  ...
  // setWebContents() (chrobalt's chromium-content path) intentionally omitted
  // from the cobalt/aosp port — there's no chromium content / WebContents in
  // this apk. CobaltActivity's chromium-init path that would have called this
  // (LibraryLoader.ensureInitialized → setWebContents → media session bridging)
  // is replaced by the plain-Activity onCreate flow (CobaltActivity calls
  ...
  // No-op @CalledByNative stubs, like the accessor block below — kept because
  // starboard_bridge.cc calls them unconditionally. These three are additionally
  // bound eagerly at initJNI, so they'd SIGILL there (not just on first call) if
  // absent.
  @CalledByNative
  ...
  // raisePlatformError / isPlatformErrorShowing stay no-op on the base: PlatformError
  // is content-only (its dialog needs the chromium-content Activity) and isn't compiled
  // into the aosp apk. The other @CalledByNative accessors below are implemented here
  // for both platforms; their backing helpers are created in the ctor above.
  @CalledByNative
