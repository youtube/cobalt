...
/*!
 * @brief if glGetString returns a space separated list of elements, prints each
 * one on a new line
 *
 * This works by creating an istringstream of the input c-style string. Then
 * that is used to create a vector -- each element of the vector is a new
 * element in the input string. Finally a foreach loop consumes this and outputs
 * it to logcat using @a aout
 */
...
/*!
 * Half the height of the projection matrix. This gives you a renderable area of
 * height 4 ranging from -2 to 2
 */
 ...
 * sorting (and avoiding z-fighting at 0).
 */
static constexpr float kProjectionNearPlane = -1.f;

/*!
 * The far plane distance for the projection matrix. Since this is an
 * orthographic projection matrix, it's convenient to have the far plane
 * equidistant from 0 as the near plane.
 */
...
void Renderer::render() {
  ...
  if (shaderNeedsNewProjectionMatrix_) {
    ...
    Utility::buildOrthographicMatrix(
        projectionMatrix, kProjectionHalfHeight,
        float(width_) / height_,  // NOLINT(readability/casting)
  ...
  // Render all the models. There's no depth testing in this sample so they're
  // accepted in the order provided. But the sample EGL setup requests a 24 bit
  // depth buffer so you could configure it at the end of initRenderer
  if (!models_.empty()) {
...
void Renderer::initRenderer() {
  ...
  // Note: there's only one shader in this demo, so I'll activate it here. For a
  // more complex game you'll want to track the active shader and
  // activate/deactivate it as necessary
  shader_->activate();
...
void Renderer::createModels() {
  ...
  auto spAndroidRobotTexture =
      TextureAsset::loadAsset(assetManager, "android_robot.png");
