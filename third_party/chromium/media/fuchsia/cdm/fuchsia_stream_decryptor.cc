...
std::string GetEncryptionScheme(EncryptionScheme mode) {
  switch (mode) {
    case EncryptionScheme::kCenc:
      return fuchsia::media_m96::ENCRYPTION_SCHEME_CENC;
    case EncryptionScheme::kCbcs:
      return fuchsia::media_m96::ENCRYPTION_SCHEME_CBCS;
    default:
...
std::vector<fuchsia::media_m96::SubsampleEntry> GetSubsamples(
    const std::vector<SubsampleEntry>& subsamples) {
  std::vector<fuchsia::media_m96::SubsampleEntry> fuchsia_subsamples(
      subsamples.size());
...
fuchsia::media_m96::EncryptionPattern GetEncryptionPattern(
    EncryptionPattern pattern) {
  fuchsia::media_m96::EncryptionPattern fuchsia_pattern;
...
fuchsia::media_m96::FormatDetails GetClearFormatDetails() {
  fuchsia::media_m96::EncryptedFormat encrypted_format;
  encrypted_format.set_scheme(fuchsia::media_m96::ENCRYPTION_SCHEME_UNENCRYPTED)
      .set_subsamples({})
      .set_init_vector({});

  fuchsia::media_m96::FormatDetails format;
  format.set_format_details_version_ordinal(0);
  format.mutable_domain()->crypto().set_encrypted(std::move(encrypted_format));
  return format;
}

fuchsia::media_m96::FormatDetails GetEncryptedFormatDetails(
...
void FuchsiaStreamDecryptor::OnStreamProcessorAllocateOutputBuffers(
    const fuchsia::media_m96::StreamBufferConstraints& stream_constraints) {
  DCHECK_CALLED_ON_VALID_SEQUENCE(sequence_checker_);
...
void FuchsiaStreamDecryptor::OnStreamProcessorOutputFormat(
    fuchsia::media_m96::StreamOutputFormat format) {
  DCHECK_CALLED_ON_VALID_SEQUENCE(sequence_checker_);
}
