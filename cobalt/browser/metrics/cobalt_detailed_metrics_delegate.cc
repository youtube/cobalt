...
CobaltDetailedMetricsDelegate::GetAndResetStats() {
  ...
  auto add_stats = [&](const char* name, Stats& stats) {
    result[base::StrCat({kDetailedMetricPssPrefix, name})] =
        base::saturated_cast<uint32_t>(stats.pss_kb);
    result[base::StrCat({kDetailedMetricRssPrefix, name})] =
        base::saturated_cast<uint32_t>(stats.rss_kb);
    stats.pss_kb = 0;
    stats.rss_kb = 0;
  };

  add_stats(kDetailedMetricCobaltCore, cobalt_core_);
  add_stats(kDetailedMetricV8, v8_);
  add_stats(kDetailedMetricPartitionAlloc, partition_alloc_);
  add_stats(kDetailedMetricMalloc, malloc_);
  add_stats(kDetailedMetricAndroidRuntime, android_runtime_);
  add_stats(kDetailedMetricGraphics, graphics_);
  add_stats(kDetailedMetricFonts, fonts_);
  add_stats(kDetailedMetricAshmemJit, ashmem_jit_);
  add_stats(kDetailedMetricStacks, stacks_);
  add_stats(kDetailedMetricCodeOther, code_other_);
  add_stats(kDetailedMetricAnonymousOther, anonymous_other_);
  add_stats(kDetailedMetricOther, other_);
