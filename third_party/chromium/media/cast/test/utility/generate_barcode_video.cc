...
void DumpPlane(scoped_refptr<media_m96::VideoFrame> frame,
               int plane) {
...
int main(int argc, char **argv) {
  ...
  scoped_refptr<media_m96::VideoFrame> frame =
      media_m96::VideoFrame::CreateBlackFrame(gfx::Size(width, height));
  printf("YUV4MPEG2 W%d H%d F%d:1 Ip C420mpeg2\n", width, height, fps);
  for (uint16_t frame_id = 1; frame_id <= wanted_frames; frame_id++) {
    CHECK(media_m96::cast::test::EncodeBarcode(frame_id, frame));
    printf("FRAME\n");
    DumpPlane(frame, media_m96::VideoFrame::kYPlane);
    DumpPlane(frame, media_m96::VideoFrame::kUPlane);
    DumpPlane(frame, media_m96::VideoFrame::kVPlane);
  }
}
