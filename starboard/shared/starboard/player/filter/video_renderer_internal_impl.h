...
class VideoRendererImpl : public VideoRenderer, private JobQueue::JobOwner {
 ...
 private:
  ...
  std::atomic_bool end_of_stream_written_;
  std::atomic_bool end_of_stream_decoded_;
  std::atomic_bool ended_cb_called_;

  std::atomic_bool need_more_input_;
  std::atomic_bool seeking_;
  int64_t seeking_to_time_ = 0;  // microseconds

  // |number_of_frames_| = decoder_frames_.size() + sink_frames_.size()
  std::atomic<int32_t> number_of_frames_;
