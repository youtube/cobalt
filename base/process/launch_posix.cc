...
#if BUILDFLAG(ENABLE_COBALT_HERMETIC_HACKS) && !BUILDFLAG(IS_NATIVE_TARGET_BUILD)
  COBALT_LINKER_STUB();
#if BUILDFLAG(COBALT_IS_RELEASE_BUILD)
  return -1;
#endif
#else  // BUILDFLAG(ENABLE_COBALT_HERMETIC_HACKS) && !BUILDFLAG(IS_NATIVE_TARGET_BUILD)
  return syscall(SYS_rt_sigaction, sig, act, oact, sizeof(kernel_sigset_t));
#endif  // BUILDFLAG(ENABLE_COBALT_HERMETIC_HACKS) && !BUILDFLAG(IS_NATIVE_TARGET_BUILD)
}
