...
class CourierRendererTest : public testing::Test {
 ...
 protected:
  void InitializeRenderer() {
    // Register media_m96::RendererClient implementation.
...
TEST_F(CourierRendererTest, SetVolume) {
  ...
  // will not call SetVolume() until after the media_m96::Renderer is initialized.
  InitializeRenderer();
  RunPendingTasks();
  ASSERT_EQ(0, ReceivedRpcMessageCount());
...
TEST_F(CourierRendererTest, SetPlaybackRate) {
  // Initialize Renderer because, as of this writing, the pipeline guarantees it
  // will not call SetPlaybackRate() until after the media_m96::Renderer is
  // initialized.
  InitializeRenderer();
  RunPendingTasks();
  ASSERT_EQ(0, ReceivedRpcMessageCount());
