...
#include "cobalt/network/custom/url_fetcher_core.h"
...
const int64_t kInformDownloadProgressIntervalUsec =
...
URLFetcherCore::URLFetcherCore(
    URLFetcher* fetcher, const GURL& original_url,
    URLFetcher::RequestType request_type, URLFetcherDelegate* d,
...
void URLFetcherCore::Stop() {
  ...
  delegate_ = NULL;
  fetcher_ = NULL;
  if (!network_task_runner_.get()) return;
...
void URLFetcherCore::AppendChunkToUpload(const std::string& content,
  ...
  DCHECK(is_chunked_upload_);
  network_task_runner_->PostTask(
      FROM_HERE, base::Bind(&URLFetcherCore::CompleteAddingUploadDataChunk,
                            this, content, is_last_chunk));
}

void URLFetcherCore::SetLoadFlags(int load_flags) { load_flags_ = load_flags; }
...
int URLFetcherCore::GetLoadFlags() const { return load_flags_; }
...
int URLFetcherCore::GetMaxRetriesOn5xx() const { return max_retries_on_5xx_; }
...
bool URLFetcherCore::WasCached() const { return was_cached_; }
...
const GURL& URLFetcherCore::GetOriginalURL() const { return original_url_; }

const GURL& URLFetcherCore::GetURL() const { return url_; }

Error URLFetcherCore::GetStatus() const { return status_.ToNetError(); }

int URLFetcherCore::GetResponseCode() const { return response_code_; }
...
bool URLFetcherCore::GetResponseAsLargeString(
  ...
  if (!large_string_writer) return false;
...
void URLFetcherCore::OnResponseStarted(URLRequest* request, int net_error) {
  ...
  if (net_error == OK) {
    ...
    response_headers_ = request_->response_headers();
    socket_address_ =
        HostPortPair::FromIPEndPoint(request_->GetResponseRemoteEndpoint());
    proxy_server_ = request_->proxy_server();
    was_fetched_via_proxy_ = request_->response_info().was_fetched_via_proxy;
    was_cached_ = request_->was_cached();
...
void URLFetcherCore::OnCertificateRequested(
    URLRequest* request, SSLCertRequestInfo* cert_request_info) {
  DCHECK_EQ(request, request_.get());
  DCHECK(network_task_runner_->RunsTasksInCurrentSequence());

  if (g_ignore_certificate_requests) {
    request->ContinueWithCertificate(scoped_refptr<X509Certificate>(),
                                     scoped_refptr<SSLPrivateKey>());
  } else {
    request->Cancel();
  }
}
...
void URLFetcherCore::OnReadCompleted(URLRequest* request, int bytes_read) {
  ...
  if (!stopped_on_redirect_) url_ = request->url();
  ...
  if (bytes_read != ERR_IO_PENDING || request_type_ == URLFetcher::HEAD) {
    ...
    if (result != ERR_IO_PENDING) DidFinishWriting(result);
  }
}
...
void URLFetcherCore::CancelAll() { g_registry.Get().CancelAll(); }

int URLFetcherCore::GetNumFetcherCores() { return g_registry.Get().size(); }
...
void URLFetcherCore::StartOnIOThread() {
  ...
  if (!response_writer_) response_writer_.reset(new URLFetcherStringWriter);

  const int result = response_writer_->Initialize(
      base::Bind(&URLFetcherCore::DidInitializeWriter, this));
  if (result != ERR_IO_PENDING) DidInitializeWriter(result);
}
...
void URLFetcherCore::StartURLRequest() {
  ...
  // body, below.
  if (chunked_stream_) request_->set_upload(std::move(chunked_stream_));
...
// Cobalt sometimes does not have a request body for post request.
      ...
      request_->set_method(request_type_ == URLFetcher::POST  ? "POST"
      ...
      upload_progress_checker_timer_->Start(
          FROM_HERE,
          base::TimeDelta::FromMilliseconds(kUploadProgressTimerInterval), this,
...
#if defined(STARBOARD)
  request_->SetLoadTimingInfoCallback(
      base::Bind(&URLFetcherCore::GetLoadTimingInfo, base::Unretained(this)));
#endif
...
void URLFetcherCore::RetryOrCompleteUrlFetch() {
  ...
  if (response_code_ >= 500 || status_.error() == ERR_TEMPORARILY_THROTTLED) {
    ...
    if (backoff_delay < base::TimeDelta()) backoff_delay = base::TimeDelta();
...
base::TimeTicks URLFetcherCore::GetBackoffReleaseTime() {
  ...
  if (!original_url_throttler_entry_.get()) return base::TimeTicks();
...
int URLFetcherCore::WriteBuffer(scoped_refptr<DrainableIOBuffer> data) {
  while (data->BytesRemaining() > 0) {
    ...
    if (result < 0) {
      if (result != ERR_IO_PENDING) DidWriteBuffer(data, result);
      return result;
    }
...
void URLFetcherCore::InformDelegateUploadProgressInDelegateSequence(
  ...
  DCHECK(delegate_task_runner_->RunsTasksInCurrentSequence());
  if (delegate_) delegate_->OnURLFetchUploadProgress(fetcher_, current, total);
}
...
void URLFetcherCore::InformDelegateDownloadProgressInDelegateSequence(
    int64_t current, int64_t total, int64_t current_network_bytes) {
  DCHECK(delegate_task_runner_->RunsTasksInCurrentSequence());
  if (delegate_)
...
#if defined(STARBOARD)
void URLFetcherCore::GetLoadTimingInfo(const net::LoadTimingInfo& timing_info) {
  delegate_task_runner_->PostTask(
      FROM_HERE, base::Bind(&URLFetcherCore::GetLoadTimingInfoInDelegateThread,
                            this, timing_info));
}
