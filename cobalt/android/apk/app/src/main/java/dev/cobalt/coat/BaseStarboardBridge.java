...
public class BaseStarboardBridge {
    ...
    protected void onActivityDestroy(Activity activity) {
        ...
        if (mActivityHolder.get() != activity && mActivityHolder.get() != null) {
            Log.i(
                    TAG,
                    "Activity destroyed but another activity is active; skipping service cleanup.");
            return;
        }
    ...
    /**
     * Returns the absolute path to the directory where application specific files should be
     * written. May be overridden for use cases that need to segregate storage.
     */
    ...
    /**
     * Returns the absolute path to the application specific cache directory on the filesystem. May
     * be overridden for use cases that need to segregate storage.
     */
    ...
    public boolean isMicrophoneDisconnected() {
        ...
        InputManager inputManager =
                (InputManager) mAppContext.getSystemService(Context.INPUT_SERVICE);
    ...
    protected String getUserAgentAuxField() {
        ...
        try {
            if (android.os.Build.VERSION.SDK_INT < 33) {
                sb.append(
                        mAppContext.getPackageManager().getPackageInfo(packageName, 0).versionName);
            } else {
                sb.append(
                        mAppContext
                                .getPackageManager()
    ...
    AudioOutputManager getAudioOutputManager() {
        if (mAudioOutputManager == null) {
            throw new IllegalArgumentException(
                    "mAudioOutputManager cannot be null for native code");
        }
    ...
    void onRequestPermissionsResult(int requestCode, String[] permissions, int[] grantResults) {
        mAudioPermissionRequester.onRequestPermissionsResult(
        ...
        try {
            if (android.os.Build.VERSION.SDK_INT < 28) {
                ...
                return mAppContext
                        .getPackageManager()
                        .getPackageInfo(
                                GOOGLE_PLAY_SERVICES_PACKAGE, PackageManager.PackageInfoFlags.of(0))
                        .getLongVersionCode();
            }
        } catch (Exception e) {
