...
TEST_F(ActorLoginServiceImplTest, AttemptLoginDelegatesToActorLoginDelegate) {
  ...
  service_->AttemptLogin(
      client.get(), credential, false, mqls_logger(), base::TimeTicks::Now(),
...
TEST_F(ActorLoginServiceImplTest, AttemptLogin_ServiceBusy) {
  ...
  EXPECT_CALL(mock_delegate_, AttemptLogin(Eq(credential), _, _, _, _, _, _))
      .WillOnce(
          RunOnceCallback<5>(base::unexpected(ActorLoginError::kServiceBusy)));
  service_->AttemptLogin(
      client.get(), credential, false, mqls_logger(), base::TimeTicks::Now(),
...
TEST_P(ActorLoginServiceImplAttemptLoginTest, AttemptLoginResults) {
  ...
  base::test::TestFuture<LoginStatusResultOrError> future;
  EXPECT_CALL(mock_delegate_, AttemptLogin(Eq(credential), _, _, _, _, _, _))
      .WillOnce(RunOnceCallback<5>(test_case.result));
  service_->AttemptLogin(
      client.get(), credential, false, mqls_logger(), base::TimeTicks::Now(),
      /*frame_filling_started_cb=*/base::DoNothing(), future.GetCallback(),
      /*action_sequence_delegate=*/nullptr);
