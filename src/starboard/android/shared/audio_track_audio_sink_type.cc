...
// Using small size of frames help audio HAL to update new time stamp quickly
const int kMaxFramesPerRequestInTunneledMode = 768;
...
AudioTrackAudioSink::AudioTrackAudioSink(
  ...
  // TODO: support query if platform support float type for tunneled mode here
  // since now we know if tunneled mode. SbAudioSinkIsAudioSampleTypeSupported()
  // is too earlier to know if tunneled mode it is queried when AudioRenderer is
  ...
  if (tunneling_audio_session_id_ != -1) {
    ...
    max_frames_per_request_ = kMaxFramesPerRequestInTunneledMode;
    ...
    int written_frames =
        WriteData(env,
                  IncrementPointerByBytes(
                      frame_buffer_, start_position * channels_ *
                                         GetSampleSize(original_sample_type_)),
...
int AudioTrackAudioSink::HandleAudioTrackError(int error) {
  ...
  jobject j_audio_track_bridge = env->CallObjectMethodOrAbort(
      ...
      "(IIIII)Ldev/cobalt/media/AudioTrackBridge;",
      GetAudioFormatSampleType(sample_type_), sampling_frequency_hz_, channels_,
      preferred_buffer_size_, tunneling_audio_session_id_);
