...
#include "third_party/chromium/media/capture/video/video_capture_system_impl.h"
...
#include "third_party/chromium/media/capture/video/video_capture_metrics.h"
...
bool IsCaptureFormatSmaller(const media_m96::VideoCaptureFormat& format1,
                            const media_m96::VideoCaptureFormat& format2) {
...
bool IsCaptureFormatEqual(const media_m96::VideoCaptureFormat& format1,
                          const media_m96::VideoCaptureFormat& format2) {
...
void ConsolidateCaptureFormats(media_m96::VideoCaptureFormats* formats) {
  if (formats->empty())
    return;
  ...
  for (auto& format : *formats) {
    if (format.pixel_format != media_m96::PIXEL_FORMAT_Y16 &&
        format.pixel_format != media_m96::PIXEL_FORMAT_NV12)
      format.pixel_format = media_m96::PIXEL_FORMAT_I420;
  }
