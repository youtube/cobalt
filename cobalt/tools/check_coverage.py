...
def get_differential_coverage(lcov_file_path, compare_branch):
    ...
    try:
        ...
        print("Error: diff-cover command not found. Is it installed and in your PATH?", file=sys.stderr)
        sys.exit(1)
    except json.JSONDecodeError:
        print(f"Error: Could not decode JSON report from {json_report_path}", file=sys.stderr)
        sys.exit(1)
...
def main():
    parser = argparse.ArgumentParser(description="Calculate and enforce code coverage metrics.")
    parser.add_argument("--lcov-file", required=True, help="Path to the LCOV file.")
    parser.add_argument("--compare-branch", default="origin/main", help="Git branch to compare against for differential coverage.")
    parser.add_argument("--threshold", type=float, default=80.0, help="Minimum differential coverage percentage required.")
    parser.add_argument("--report-file", default="coverage_report.md", help="Output file for the Markdown report.")
    ...
    differential_coverage = get_differential_coverage(args.lcov_file, args.compare_branch)
    ...
    with open(args.report_file, 'w') as f:
        ...
        f.write(f"Coverage for new/changed code compared to `{args.compare_branch}`: **{differential_coverage:.2f}%**\n\n")
        
        if differential_coverage >= args.threshold:
            f.write(f"✅ Differential coverage of {differential_coverage:.2f}% meets the required threshold of {args.threshold:.2f}%\n")
        else:
            f.write(f"❌ Differential coverage of {differential_coverage:.2f}% is below the required threshold of {args.threshold:.2f}%\n")
