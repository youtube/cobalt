...
base::CheckedNumeric<uint32_t> ComputeAudioInputBufferSizeChecked(
  ...
  result += sizeof(media_m96::AudioInputBufferParameters);
...
base::CheckedNumeric<uint32_t> ComputeAudioOutputBufferSizeChecked(
  ...
  base::CheckedNumeric<uint32_t> result = AudioBus::CalculateMemorySize(
      parameters.channels(), parameters.frames_per_buffer());
  result += sizeof(media_m96::AudioOutputBufferParameters);
  return result;
}
...
bool AudioParameters::IsValid() const {
  return (channels_ > 0) && (channels_ <= media_m96::limits::kMaxChannels) &&
         (channel_layout_ > CHANNEL_LAYOUT_UNSUPPORTED) &&
         (sample_rate_ >= media_m96::limits::kMinSampleRate) &&
         (sample_rate_ <= media_m96::limits::kMaxSampleRate) &&
         (frames_per_buffer_ > 0) &&
         (frames_per_buffer_ <= media_m96::limits::kMaxSamplesPerPacket) &&
         (!hardware_capabilities_ ||
          ((hardware_capabilities_->min_frames_per_buffer >= 0) &&
           (hardware_capabilities_->min_frames_per_buffer <=
            media_m96::limits::kMaxSamplesPerPacket) &&
...
AudioParameters AudioParameters::UnavailableDeviceParams() {
  ...
  return media_m96::AudioParameters(
      media_m96::AudioParameters::AUDIO_FAKE, media_m96::CHANNEL_LAYOUT_STEREO,
      media_m96::AudioParameters::kAudioCDSampleRate,
      media_m96::AudioParameters::kAudioCDSampleRate / 100);
}
