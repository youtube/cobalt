...
void UpdateCastTransportStatus(
    media_m96::cast::CastTransportStatus status) {
  VLOG(1) << "Transport status: " << status;
}

void QuitLoopOnInitializationResult(media_m96::cast::OperationalStatus result) {
  CHECK(result == media_m96::cast::STATUS_INITIALIZED)
...
void WriteLogsToFileAndDestroySubscribers(
    const scoped_refptr<media_m96::cast::CastEnvironment>& cast_environment,
    std::unique_ptr<media_m96::cast::EncodingEventSubscriber>
...
void WriteStatsAndDestroySubscribers(
    const scoped_refptr<media_m96::cast::CastEnvironment>& cast_environment,
    std::unique_ptr<media_m96::cast::StatsEventSubscriber> video_stats_subscriber,
    std::unique_ptr<media_m96::cast::StatsEventSubscriber> audio_stats_subscriber,
    std::unique_ptr<media_m96::cast::ReceiverTimeOffsetEstimatorImpl> estimator) {
...
class TransportClient : public media_m96::cast::CastTransport::Client {
 public:
  explicit TransportClient(
      media_m96::cast::LogEventDispatcher* log_event_dispatcher)
      : log_event_dispatcher_(log_event_dispatcher) {}

  void OnStatusChanged(media_m96::cast::CastTransportStatus status) final {
    VLOG(1) << "Transport status: " << status;
  }
  void OnLoggingEventsReceived(
      std::unique_ptr<std::vector<media_m96::cast::FrameEvent>> frame_events,
      std::unique_ptr<std::vector<media_m96::cast::PacketEvent>> packet_events)
      final {
    DCHECK(log_event_dispatcher_);
    log_event_dispatcher_->DispatchBatchOfEvents(std::move(frame_events),
                                                 std::move(packet_events));
  }
  void ProcessRtpPacket(std::unique_ptr<media_m96::cast::Packet> packet) final {}

 private:
  media_m96::cast::LogEventDispatcher* const
...
int main(int argc, char** argv) {
  ...
  media_m96::cast::FrameSenderConfig audio_config =
      media_m96::cast::GetDefaultAudioSenderConfig();
  media_m96::cast::FrameSenderConfig video_config =
      media_m96::cast::GetDefaultVideoSenderConfig();
  ...
  // Running transport on the main thread.
  scoped_refptr<media_m96::cast::CastEnvironment> cast_environment(
      new media_m96::cast::CastEnvironment(
  ...
  std::unique_ptr<media_m96::cast::FakeMediaSource> fake_media_source(
      new media_m96::cast::FakeMediaSource(test_thread.task_runner(),
  ...
  std::unique_ptr<media_m96::cast::CastTransport> transport_sender =
      media_m96::cast::CastTransport::Create(
          cast_environment->Clock(), base::Seconds(1),
          std::make_unique<TransportClient>(cast_environment->logger()),
          std::make_unique<media_m96::cast::UdpTransportImpl>(
  ...
  std::unique_ptr<media_m96::cast::EncodingEventSubscriber> video_event_subscriber;
  std::unique_ptr<media_m96::cast::EncodingEventSubscriber> audio_event_subscriber;
  ...
  video_event_subscriber =
      std::make_unique<media_m96::cast::EncodingEventSubscriber>(
          media_m96::cast::VIDEO_EVENT, 10000);
  audio_event_subscriber =
      std::make_unique<media_m96::cast::EncodingEventSubscriber>(
          media_m96::cast::AUDIO_EVENT, 10000);
  ...
  std::unique_ptr<media_m96::cast::ReceiverTimeOffsetEstimatorImpl>
      offset_estimator(new media_m96::cast::ReceiverTimeOffsetEstimatorImpl());
  cast_environment->logger()->Subscribe(offset_estimator.get());
  std::unique_ptr<media_m96::cast::StatsEventSubscriber> video_stats_subscriber(
      new media_m96::cast::StatsEventSubscriber(media_m96::cast::VIDEO_EVENT,
                                            cast_environment->Clock(),
                                            offset_estimator.get()));
  std::unique_ptr<media_m96::cast::StatsEventSubscriber> audio_stats_subscriber(
      new media_m96::cast::StatsEventSubscriber(media_m96::cast::AUDIO_EVENT,
                                            cast_environment->Clock(),
                                            offset_estimator.get()));
  ...
  std::unique_ptr<media_m96::cast::CastSender> cast_sender =
      media_m96::cast::CastSender::Create(cast_environment, transport_sender.get());
  io_task_executor.task_runner()->PostTask(
      FROM_HERE,
      base::BindOnce(&media_m96::cast::CastSender::InitializeVideo,
  ...
  io_task_executor.task_runner()->PostTask(
      FROM_HERE,
      base::BindOnce(&media_m96::cast::CastSender::InitializeAudio,
