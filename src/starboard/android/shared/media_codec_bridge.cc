...
void MediaCodecBridge::SetVendorParameters(const std::string& key,
                                           const std::string& value) {
  JniEnvExt* env = JniEnvExt::Get();
  ScopedLocalJavaRef<jstring> j_key(
      env->NewStringStandardUTFOrAbort(key.c_str()));
  ScopedLocalJavaRef<jstring> j_value(
      env->NewStringStandardUTFOrAbort(value.c_str()));
  return JniEnvExt::Get()->CallVoidMethodOrAbort(
      j_media_codec_bridge_, "setVendorParameters",
