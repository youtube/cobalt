...
#include "third_party/chromium/media/audio/mac/audio_auhal_mac.h"
...
#include "third_party/chromium/media/audio/mac/coreaudio_dispatch_override.h"
...
static void GetAudioDeviceInfo(bool is_input,
                               media_m96::AudioDeviceNames* device_names) {
  DCHECK(AudioManager::Get()->GetTaskRunner()->BelongsToCurrentThread());
  ...
  if (!device_names->empty()) {
    ...
    device_names->push_front(media_m96::AudioDeviceName::CreateDefault());
  }
}
...
AudioOutputStream* AudioManagerMac::MakeLowLatencyOutputStream(
  ...
  if (!output_device_listener_) {
    // NOTE: Use base::BindPostTaskToCurrentDefault() to ensure the callback is always PostTask'd
