...
class CobaltPlatformBackend(android_platform_backend.AndroidPlatformBackend):
  def __init__(self, device, require_root):
    super().__init__(device, require_root)
    # self.device_ = self._device = device_utils.DeviceUtils(device.device_id)
  
  @property
  def platform_backend(self):
    return self

def run_trace(args, controller, page):
  # am start --esa commandLineArgs '--remote-allow-origins=*' < this gets mangle-quoted for some reason
  print(page)
  # _platform_backend = android_platform_backend.AndroidPlatformBackend(
  #       android_device.AndroidDevice(args.device), False)
  _platform_backend = CobaltPlatformBackend(android_device.AndroidDevice(args.device), False)

  backend = devtools_client_backend.GetDevToolsBackEndIfReady(
      devtools_port=DEVTOOLS_PORT, app_backend=_platform_backend, enable_tracing=True)
  # print(backend)
...
class AdbController:
  ...
  def __init__(self, adb_path, device_serial, command_args=None):
  ...
  def run_adb_command(self):
    ...
    full_adb_command = [self.adb_path]
    if self.device_serial:
      full_adb_command.extend(['-s', self.device_serial])
    full_adb_command.extend(self.command_args)
  ...
  def launch_cobalt(self, component, url):
    ...
    am_start_args = [
        'shell', 'am', 'start', '--esa', 'commandLineArgs', args_value,
        component
    ]
    self.command_args = am_start_args
    return self.run_adb_command() is not None

  def kill_cobalt(self, package_name):
    print(f"\nAttempting to stop/kill process: {package_name}")
    am_kill_args = ['shell', 'am', 'force-stop', package_name]
    self.command_args = am_kill_args
    return self.run_adb_command() is not None


class PerformanceMonitor:

  def __init__(self, sample_interval, duration):
    self.sample_interval = sample_interval
    self.duration = duration

  def get_cpu_core_count(self, controller):
    ...
    controller.command_args = core_cmd_args
    result = controller.run_adb_command()
    ...
    if result and result.stdout:
      try:
        ...
        if count > 0:
          ...
          controller.command_args = proc_cmd_args
          result = contoller.run_adb_command()
          if result and result.stdout:
            try:
              count = int(result.stdout.strip())
              if count > 0:
  ...
  def get_memory_usage(contoller, package_name):
    # ... (same as before) ...
    controller.timeout = 15
    meminfo_args = ['shell', 'dumpsys', 'meminfo', package_name]
    controller.command_args = meminfo_args
    result = controller.run_adb_command()
    if result and result.stdout:
  ...
  def get_cpu_usage(controller, package_name):
    # ... (same as before) ...
    top_args = ['shell', 'top', '-b', '-n', '1']
    controller.command_args = top_args
    result = controller.run_adb_command()
    if result and result.stdout:
      raw_output = result.stdout.strip()
      ...
      for line in lines:
        ...
        if package_name in line or (fields and
                                    fields[-1].endswith(package_name)):
          try:
  ...
  def wait_and_sample_performance(self, controller):
    print(
    ...
    try:
      while True:
        ...
        if current_time >= next_sample_time:
          cpu = self.get_cpu_usage(controller, COBALT_PACKAGE_NAME)
          mem = self.get_memory_usage(controller, COBALT_PACKAGE_NAME)
          ...
          if cpu is None and mem is None and len(
            ...
            if not controller.is_cobalt_running(COBALT_PACKAGE_NAME):
              print(
    ...
    self.process_performance_data(performance_data, core_count,
                                  system_mem_info)  # Pass system info
    self.save_data_to_csv(performance_data, args.output_csv)
  ...
  def process_performance_data(self, data, core_count,
                               system_mem_info):  # Added args
...
def init_cli_args():
  ...
  parser.add_argument(
      ...
      '--trace',
      default=DEFAULT_CSV_FILENAME,
      help='Boolean to capture traces or not.')
  parser.add_argument(
        '-q',
        '--quiet',
        default=3,
  ...
  return args, dev, page


def main():
  args, dev, page = init_cli_args()
  controller = AdbController(args.adb_path, args.device)
  monitor = PerformanceMonitor(args.sample_interval, args.duration)

  # --- Get System Info ---
  print("\n--- Retrieving System Info ---")
  core_count = monitor.get_cpu_core_count(controller)
  system_mem_info = monitor.get_system_memory_info(controller)
  ...
  controller.maybe_kill_cobalt()

  # --- 1. Launch Cobalt ---
  # ... (same as before) ...
  if not controller.launch_cobalt(COBALT_COMPONENT, args.url):
  ...
  controller.launch_cobalt(COBALT_COMPONENT, args.url)

  try:
    match = logcat_monitor.WaitFor(result_line_re, timeout=args.duration)
