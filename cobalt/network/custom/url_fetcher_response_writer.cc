...
URLFetcherFileWriter* URLFetcherResponseWriter::AsFileWriter() { return NULL; }
...
URLFetcherFileWriter::~URLFetcherFileWriter() { CloseAndDeleteFile(); }
...
int URLFetcherFileWriter::Initialize(CompletionOnceCallback callback) {
  ...
  if (file_path_.empty()) {
    ...
    file_task_runner_->PostTaskAndReplyWithResult(
        FROM_HERE, base::Bind(&base::CreateTemporaryFile, temp_file_path),
        base::Bind(&URLFetcherFileWriter::DidCreateTempFile,
                   weak_factory_.GetWeakPtr(), base::Owned(temp_file_path)));
  } else {
    result =
  ...
  if (result < 0) CloseAndDeleteFile();
  return result;
}

int URLFetcherFileWriter::Write(IOBuffer* buffer, int num_bytes,
                                CompletionOnceCallback callback) {
  DCHECK(file_stream_);
  DCHECK(owns_file_);
  DCHECK(!callback_);
...
URLFetcherFileWriter* URLFetcherFileWriter::AsFileWriter() { return this; }
...
void URLFetcherFileWriter::CloseAndDeleteFile() {
  if (!owns_file_) return;

  file_stream_.reset();
  DisownFile();
  file_task_runner_->PostTask(
      FROM_HERE,
      base::BindOnce(base::IgnoreResult(&base::DeleteFile), file_path_));
}
...
void URLFetcherFileWriter::DidCreateTempFile(base::FilePath* temp_file_path,
  ...
  const int result = file_stream_->Open(
      ...
      base::BindOnce(&URLFetcherFileWriter::OnIOCompleted,
                     weak_factory_.GetWeakPtr()));
  if (result != ERR_IO_PENDING) OnIOCompleted(result);
}

void URLFetcherFileWriter::OnIOCompleted(int result) {
  if (result < OK) CloseAndDeleteFile();

  if (!callback_.is_null()) std::move(callback_).Run(result);
}
