...
MediaCodecDecoder::MediaCodecDecoder(PassKey<MediaCodecDecoder>,
  ...
  MediaCodecBridge::WaitForMediaCryptoSessionCreatedCb callback;
  if (drm_system_) {
    callback = [drm_system = drm_system_](int64_t timeout_us) {
      return drm_system->WaitForMediaCryptoSessionCreated(timeout_us);
    };
  }
  media_codec_bridge_ = MediaCodecBridge::CreateAudioMediaCodecBridge(
      audio_stream_info, this, j_media_crypto, std::move(callback));
...
MediaCodecDecoder::MediaCodecDecoder(
  ...
  auto media_codec_bridge = MediaCodecBridge::CreateVideoMediaCodecBridge(
      ...
      j_output_surface, j_media_crypto, std::move(callback), color_metadata,
