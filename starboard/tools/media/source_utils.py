...
def _get_starboard_includes_and_usings(pathname):
  basename = os.path.basename(pathname)

  if (basename in [
      'media_is_audio_supported.cc', 'media_is_supported.cc',
...
def append_suffix_to_sb_function(pathname, content, suffix):
...
def create_canonical_file(source_project_root_dir, source_pathname,
  ...
  includes_and_usings = _get_starboard_includes_and_usings(source_pathname)
  ...
  with open(destination_pathname, 'w+', encoding='utf-8') as f:
    f.write(_get_copyright_header() + dedent(_CANONICAL_FILE_CONTENT).format(
        includes_and_usings, canonical_prototype, prototype, calling_statement,
...
def update_starboard_usage(source_content):
  ...
  if source_content.find('scoped_ptr') != -1:
