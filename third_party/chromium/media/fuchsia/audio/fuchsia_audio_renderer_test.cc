...
#include "third_party/chromium/media/fuchsia/audio/fuchsia_audio_renderer.h"
...
#include "third_party/chromium/media/base/renderer_client.h"
#include "third_party/chromium/media/fuchsia/cdm/fuchsia_cdm_context.h"
#include "third_party/chromium/media/fuchsia/common/passthrough_sysmem_buffer_stream.h"
#include "third_party/chromium/media/fuchsia/common/sysmem_client.h"
...
class TestStreamSink : public fuchsia::media_m96::testing::StreamSink_TestBase {
 public:
  TestStreamSink(
  ...
  const fuchsia::media_m96::AudioStreamType& stream_type() const {
    return stream_type_;
  }
  const fuchsia::media_m96::Compression* compression() const {
    return compression_.get();
  }

  std::vector<fuchsia::media_m96::StreamPacket>* received_packets() {
    return &received_packets_;
  }

  std::vector<fuchsia::media_m96::StreamPacket>* discarded_packets() {
    return &discarded_packets_;
  }

  bool received_end_of_stream() const { return received_end_of_stream_; }

  // fuchsia::media_m96::StreamSink overrides.
 ...
 private:
  fidl::Binding<fuchsia::media_m96::StreamSink> binding_;

  std::vector<zx::vmo> buffers_;
  fuchsia::media_m96::AudioStreamType stream_type_;
  std::unique_ptr<fuchsia::media_m96::Compression> compression_;

  std::vector<fuchsia::media_m96::StreamPacket> received_packets_;
  std::vector<fuchsia::media_m96::StreamPacket> discarded_packets_;
...
class TestAudioConsumer
 ...
 public:
  ...
  void UpdateStatus(absl::optional<base::TimeTicks> reference_time,
    ...
    fuchsia::media_m96::AudioConsumerStatus status;
    if (reference_time) {
      CHECK(media_time);

      fuchsia::media_m96::TimelineFunction timeline;
  ...
  // fuchsia::media_m96::AudioConsumer overrides.
  ...
  void Start(fuchsia::media_m96::AudioConsumerStartFlags flags,
    ...
    EXPECT_EQ(reference_time, fuchsia::media_m96::NO_TIMESTAMP);
    started_ = true;
  ...
  // fuchsia::media_m96::audio::VolumeControl overrides.
 ...
 private:
  ...
  fidl::Binding<fuchsia::media_m96::AudioConsumer> binding_;
  ...
  absl::optional<fuchsia::media_m96::AudioConsumerStatus> status_update_;
...
class FuchsiaAudioRendererTest
 ...
 public:
  ...
  void TestPcmStream(SampleFormat sample_format,
                     ...
                     fuchsia::media_m96::AudioSampleFormat fuchsia_sample_format,
...
void FuchsiaAudioRendererTest::CreateUninitializedRenderer() {
  fidl::InterfaceHandle<fuchsia::media_m96::AudioConsumer> audio_consumer_handle;
  audio_consumer_ =
...
TEST_P(FuchsiaAudioRendererTest, ChangeConfig) {
  ...
  EXPECT_EQ(new_stream_sink->compression()->type,
            fuchsia::media_m96::AUDIO_ENCODING_OPUS);
...
TEST_F(FuchsiaAudioRendererTest, PcmU8Stream) {
  TestPcmStream(kSampleFormatU8, 1,
                fuchsia::media_m96::AudioSampleFormat::UNSIGNED_8, 1);
}

TEST_F(FuchsiaAudioRendererTest, PcmS16Stream) {
  TestPcmStream(kSampleFormatS16, 2,
                fuchsia::media_m96::AudioSampleFormat::SIGNED_16, 2);
}

TEST_F(FuchsiaAudioRendererTest, PcmS24Stream) {
  TestPcmStream(kSampleFormatS24, 3,
                fuchsia::media_m96::AudioSampleFormat::SIGNED_24_IN_32, 4);
}

TEST_F(FuchsiaAudioRendererTest, PcmF32Stream) {
  TestPcmStream(kSampleFormatF32, 4, fuchsia::media_m96::AudioSampleFormat::FLOAT,
                4);
}
