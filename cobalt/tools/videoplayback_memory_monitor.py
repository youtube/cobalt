...
def run_adb_command(adb_path, device_serial, command_args, timeout=30, check=True, verbose=True):
    ...
    try:
        result = subprocess.run(full_adb_command, check=check, capture_output=True, text=True, timeout=timeout)
        ...
        try:
            ...
            if count > 0:
                return count
                 ...
                 result = run_adb_command(adb_path, device_serial, proc_cmd_args, timeout=10, check=False, verbose=False)
                 if result and result.stdout:
                     try:
                          count = int(result.stdout.strip())
...
def save_data_to_csv(data, filename):
    ...
    if not data: print("No performance data to save."); return
    try:
        with open(filename, 'w', newline='') as csvfile:
            writer = csv.writer(csvfile); writer.writerow(['Timestamp (s)', 'CPU (%)', 'Memory (MB)'])
            for row in data: writer.writerow([f"{row[0]:.3f}", f"{row[1]:.1f}" if row[1] is not None else '', f"{row[2]:.2f}" if row[2] is not None else ''])
...
def process_performance_data(data, core_count, system_mem_info): # Added args
    ...
    if valid_cpu:
        ...
        print(f"CPU Usage (% of 1 Core): Avg={avg_cpu:.1f}, Max={max_cpu:.1f} ({len(valid_cpu)} samples)")
        if core_count:
            # Calculate % of total system CPU capacity
            avg_cpu_total = (avg_cpu / core_count) # Already a percentage if avg_cpu is 100 = 1 core
            max_cpu_total = (max_cpu / core_count)
            print(f"CPU Usage (% of Total):  Avg={avg_cpu_total:.1f}, Max={max_cpu_total:.1f} (based on {core_count} cores)")
        else:
    ...
    if valid_mem:
        ...
        print(f"Memory Usage (MB PSS):   Avg={avg_mem:.1f}, Max={max_mem:.1f} ({len(valid_mem)} samples)")
        if system_mem_info:
             # Show Max PSS as % of Total/Available system memory
             max_mem_perc_total = (max_mem / system_mem_info['total_mb']) * 100 if system_mem_info['total_mb'] else 0
             max_mem_perc_avail = (max_mem / system_mem_info['available_mb']) * 100 if system_mem_info['available_mb'] else 0
             print(f"Max Memory as % of Sys:  {max_mem_perc_total:.1f}% (Total RAM), {max_mem_perc_avail:.1f}% (Available RAM)")
             print(f"  (System RAM: Total={system_mem_info['total_mb']:.0f}MB, Available={system_mem_info['available_mb']:.0f}MB)")
        else:
...
if __name__ == "__main__":
    ...
    else: print("Warning: Could not determine CPU core count.")

    if system_mem_info: print(f"System Memory: Total={system_mem_info['total_mb']:.0f}MB, Available={system_mem_info['available_mb']:.0f}MB (at script start)")
    else: print("Warning: Could not determine system memory info.")
    print("-----------------------------")
    ...
    if is_cobalt_running(args.adb_path, args.device, COBALT_PACKAGE_NAME):
        print(f"  Process found. Attempting to stop existing '{COBALT_PACKAGE_NAME}' process...")
    ...
    try:
        while True:
            ...
            if current_time >= next_sample_time:
                ...
                if cpu is None and mem is None and len(performance_data) > 1 and performance_data[-2][1] is None and performance_data[-2][2] is None:
            ...
            if sleep_duration <= 0.01 and time.monotonic() < end_time: time.sleep(0.01)
            elif sleep_duration > 0: time.sleep(sleep_duration)
        print("\r" + " " * 120 + "\r", end='');
        if run_completed_normally: print("  Run duration complete.")
    except KeyboardInterrupt: print("\n🛑 Sampling interrupted by user (Ctrl+C)."); run_completed_normally = False


    # --- 3. Process Performance Data ---
    process_performance_data(performance_data, core_count, system_mem_info) # Pass system info
    ...
    if not kill_cobalt(args.adb_path, args.device, COBALT_PACKAGE_NAME): print("⚠️  Final stop command may have failed (ADB Error or app already stopped).")
