...
"""Tests cancellation of scroll on Suspend."""
...
class CancelScrollWhenSuspended(black_box_tests.BlackBoxTestCase):
  """Tests cancellation of scroll on Suspend."""

  def test_cancel_scroll_when_suspended(self):
    with ThreadedWebServer(binding_address=self.GetBindingAddress()) as server:
      url = server.GetURL(
          file_name='testdata/cancel_scroll_when_suspended.html')
      with self.CreateCobaltRunner(url=url) as runner:
        ...
        actions.move_by_offset(-200, 0).pause(_SLEEP_AFTER_MOVE_TIME)
        actions.send_keys([keys.Keys.CONTROL, 'S'])
        # This action should not be cancelled after suspended.
        actions.move_by_offset(-300, 0).pause(_SLEEP_AFTER_MOVE_TIME)
        actions.release()
        actions.perform()
