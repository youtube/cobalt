...
void AudioInputStreamStarboard::ReadAudio() {
  TRACE_EVENT0("base", "AudioInputStreamStarboard::ReadAudio");
  ...
  const int buffer_size_bytes = buffer_.size() * sizeof(int16_t);
  int bytes_read = SbMicrophoneRead(microphone_, buffer_.data(), buffer_size_bytes);

  if (bytes_read > 0) {
    int frames_read = bytes_read / (params_.channels() * sizeof(int16_t));
    CHECK_LE(frames_read, params_.frames_per_buffer());
    audio_bus_->FromInterleaved<SignedInt16SampleTypeTraits>(buffer_.data(), frames_read);

    base::TimeTicks now = base::TimeTicks::Now();
    base::TimeTicks capture_time = now - buffer_duration_;
    if (!last_capture_time_.is_null() && capture_time <= last_capture_time_) {
        capture_time = last_capture_time_ + base::Microseconds(1);
    }
    last_capture_time_ = capture_time;

    callback_->OnData(audio_bus_.get(), capture_time, 1.0, {});
  }

  next_read_time_ = std::max(base::TimeTicks::Now(), next_read_time_ + buffer_duration_);
