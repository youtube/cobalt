...
TEST_F(GlobalFeaturesTest, GetExperimentConfigTypeReturnsRegular) {
  ASSERT_NE(instance_, nullptr);
  instance_->experiment_config()->SetInteger(
      variations::prefs::kVariationsCrashStreak, 0);
  EXPECT_EQ(instance_->GetExperimentConfigType(),
            ExperimentConfigType::kRegularConfig);
}

TEST_F(GlobalFeaturesTest, GetExperimentConfigTypeReturnsSafe) {
  ASSERT_NE(instance_, nullptr);
  instance_->experiment_config()->SetInteger(
...
TEST_F(GlobalFeaturesTest, GetExperimentConfigTypeReturnsEmpty) {
  ASSERT_NE(instance_, nullptr);
  instance_->experiment_config()->SetInteger(
      variations::prefs::kVariationsCrashStreak,
...
TEST_F(GlobalFeaturesTest, StoreSafeConfigIsNoOpForSafeConfig) {
  ASSERT_NE(instance_, nullptr);
  instance_->experiment_config()->SetInteger(
      variations::prefs::kVariationsCrashStreak,
      kCrashStreakSafeConfigThreshold);

  instance_->StoreSafeConfig();
  task_environment_->RunUntilIdle();

  EXPECT_FALSE(instance_->has_called_store_safe_config());
  EXPECT_TRUE(
      instance_->experiment_config()->GetDict(kSafeConfigFeatures).empty());
}

TEST_F(GlobalFeaturesTest, StoreSafeConfigIsNoOpForEmptyConfig) {
  ASSERT_NE(instance_, nullptr);
  instance_->experiment_config()->SetInteger(
      variations::prefs::kVariationsCrashStreak,
      kCrashStreakEmptyConfigThreshold);
...
TEST_F(GlobalFeaturesTest, StoreSafeConfigIsOnlyCalledOnce) {
  ASSERT_NE(instance_, nullptr);
  base::Value::Dict initial_features;
  initial_features.Set("feature1", true);
  instance_->experiment_config()->SetDict(kExperimentConfigFeatures,
  ...
  EXPECT_TRUE(instance_->has_called_store_safe_config());
  EXPECT_EQ(instance_->experiment_config()->GetDict(kSafeConfigFeatures),
            initial_features);
