...
ShellPlatformDataAura::ShellPlatformDataAura(const gfx::Size& initial_size) {
  ...
  // The FocusController wants an ActivationClient, which is owned by the root
  // window.
...
#if defined(RUN_BROWSER_TESTS)
  // The test version of the FocusClient is a TestFocusClient, which does not
  // need an ActivationClient.
  focus_client_ =
