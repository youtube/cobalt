...
void MojoCdm::OnMetricsReceived(
  ...
  DCHECK_CALLED_ON_VALID_THREAD(thread_checker_);
  if (metrics_string) {
    cdm_promise_adapter_.ResolvePromise(promise_id, metrics_string.value_or(std::string()));
  } else {
    cdm_promise_adapter_.RejectPromise(promise_id, CdmPromise::Exception::NOT_SUPPORTED_ERROR, 0,
