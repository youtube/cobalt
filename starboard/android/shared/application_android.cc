...
Java_dev_cobalt_coat_javabridge_HTMLMediaElementExtension_nativeCanPlayType(
  ...
  if (j_mime_type) {
    mime_type = Jni::GetStringStandardUTFOrAbort(env, j_mime_type);
  }
  if (j_key_system) {
    key_system = Jni::GetStringStandardUTFOrAbort(env, j_key_system);
  }
  ...
  SB_LOG(INFO) << __func__ << " (" << mime_type << ", " << key_system
               << ") --> " << ret;
  return Jni::NewStringStandardUTFOrAbort(env, ret);
}
