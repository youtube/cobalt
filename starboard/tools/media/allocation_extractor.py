...
"""Process a full Cobalt log to produce the allocator log.

The allocator log is a text file captures all allocation operations in a real
scenario and can be used by various utilities to verify allocator
implementations.
"""
...
# Extra allocation log from the full log file, clean them up, and produce a list
# with one allocation operation per item. Each allocation operation is in the
# form of:
...
def ExtractAllocations(full_log_file):
  ...
  # Now the lines are allocation records in the format of:
...
# For the allocation operations in the format of
#   a <pointer> <type> <size> <alignment>
#   f <pointer>
# Refine them to
