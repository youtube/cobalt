...
#include "third_party/chromium/media/capture/video/win/d3d_capture_test_utils.h"
#include "third_party/chromium/media/capture/video/win/sink_filter_win.h"
#include "third_party/chromium/media/capture/video/win/video_capture_device_factory_win.h"
#include "third_party/chromium/media/capture/video/win/video_capture_device_mf_win.h"
...
TEST_P(DepthCameraDeviceMFWinTest, AllocateAndStartDepthCamera) {
  ...
  VideoCaptureFormat format(gfx::Size(640, 480), 30, media_m96::PIXEL_FORMAT_Y16);
...
TEST_F(VideoCaptureDeviceMFWinTestWithDXGI, EnsureNV12SinkSubtype) {
  ...
  VideoCaptureFormat format(gfx::Size(640, 480), 30, media_m96::PIXEL_FORMAT_NV12);
  VideoCaptureParams video_capture_params;
  video_capture_params.requested_format = format;
  device_->AllocateAndStart(video_capture_params, std::move(client_));
  capture_preview_sink_->sample_callback->OnSample(nullptr);
...
TEST_F(VideoCaptureDeviceMFWinTestWithDXGI, DeliverGMBCaptureBuffers) {
  ...
  // Init capture
  VideoCaptureFormat format(expected_size, 30, media_m96::PIXEL_FORMAT_NV12);
  VideoCaptureParams video_capture_params;
  video_capture_params.requested_format = format;
  device_->AllocateAndStart(video_capture_params, std::move(client_));
