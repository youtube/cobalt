...
int UDPSocketStarboard::GetLocalAddress(IPEndPoint* address) const {
  ...
  if (!local_address_.get()) {
    ...
    if (0 != getsockname(socket_, reinterpret_cast<struct sockaddr*>(&saddr_store), &length))
      return MapSystemError(errno);

    std::unique_ptr<IPEndPoint> endpoint(new IPEndPoint());
    if (!endpoint->FromSockAddr(reinterpret_cast<struct sockaddr*>(&saddr_store), length))
      return ERR_FAILED;
#else
...
int UDPSocketStarboard::InternalRecvFrom(IOBuffer* buf,
  ...
  int bytes_transferred = recvfrom(socket_, buf->data(), buf_len, 0,
                                   reinterpret_cast<struct sockaddr*>(&saddr_store), &length);
  ...
  if (bytes_transferred >= 0) {
    ...
    if (address && !address->FromSockAddr(reinterpret_cast<struct sockaddr*>(&saddr_store), length)) {
  ...
  if (result != ERR_IO_PENDING) {
    ...
    if (result < 0 || !log_address.FromSockAddr(reinterpret_cast<struct sockaddr*>(&saddr_store), length)) {
...
int UDPSocketStarboard::InternalSendTo(IOBuffer* buf,
  ...
  if (!address || !address->ToSockAddr(reinterpret_cast<struct sockaddr*>(&saddr_store), &length)) {
...
int UDPSocketStarboard::DoBind(const IPEndPoint& address) {
  ...
  int rv = bind(socket_, reinterpret_cast<struct sockaddr*>(&saddr_store), length);
...
int UDPSocketStarboard::JoinGroup(const IPAddress& group_address) const {
  ...
  struct sockaddr_storage saddr_store = {0};
  socklen_t length = sizeof(saddr_store);
  if (!IPEndPoint(group_address, 0).ToSockAddr(reinterpret_cast<struct sockaddr*>(&saddr_store), &length)) {
    return ERR_ADDRESS_INVALID;
  }
  struct ip_mreq imreq = {0};
  if (saddr_store.ss_family == AF_INET) {
    sockaddr_in* psock_in = reinterpret_cast<struct sockaddr_in*>(&saddr_store);
    in_addr_t in_addr = *reinterpret_cast<const in_addr_t*>(&psock_in->sin_addr);
...
void UDPSocketStarboard::LocalSendBuffers() {
  ...
  struct sockaddr_storage saddr_store = {0};
  socklen_t length = sizeof(saddr_store);
  int result = remote_address_.get()->ToSockAddr(reinterpret_cast<struct sockaddr*>(&saddr_store), &length);
#else
  SbSocketAddress sb_address;
...
void UDPSocketStarboard::PostSendBuffers() {
  ...
  struct sockaddr_storage saddr_store = {0};
  socklen_t length = sizeof(saddr_store);
  DCHECK(remote_address_.get()->ToSockAddr(reinterpret_cast<struct sockaddr*>(&saddr_store), &length));
#else
  SbSocketAddress sb_address;
