...
/** Creates MediaSources for specified codec configurations */
public class ExoPlayerCodecUtil {
    @CalledByNative
    ...
    @CalledByNative
    public static ColorInfo createHdrColorInfo(int colorRange, int colorSpace, int colorTransfer,
            float primaryRChromaticityX, float primaryRChromaticityY, float primaryGChromaticityX,
            float primaryGChromaticityY, float primaryBChromaticityX, float primaryBChromaticityY,
            float whitePointChromaticityX, float whitePointChromaticityY,
            float maxMasteringLuminance, float minMasteringLuminance, int maxCll, int maxFall) {
        MediaCodecBridge.ColorInfo info = new MediaCodecBridge.ColorInfo(colorRange, colorSpace,
                colorTransfer, primaryRChromaticityX, primaryRChromaticityY, primaryGChromaticityX,
        ...
        return new ColorInfo.Builder()
                .setColorRange(info.colorRange)
                .setColorSpace(info.colorStandard)
                .setColorTransfer(info.colorTransfer)
                .setHdrStaticInfo(info.hdrStaticInfo.array())
                .build();
    }

    /** Filters software video codecs from ExoPlayer codec selection */
    public static class FilteringMediaCodecSelector implements MediaCodecSelector {
        @Override
