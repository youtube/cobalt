// Copyright 2025 The Cobalt Authors. All Rights Reserved.
//
// Licensed under the Apache License, Version 2.0 (the "License");
// you may not use this file except in compliance with the License.
// You may obtain a copy of the License at
//
//     http://www.apache.org/licenses/LICENSE-2.0
//
// Unless required by applicable law or agreed to in writing, software
// distributed under the License is distributed on an "AS IS" BASIS,
// WITHOUT WARRANTIES OR CONDITIONS OF ANY KIND, either express or implied.
// See the License for the specific language governing permissions and
// limitations under the License.

#include "cobalt/browser/h5vcc_storage/h5vcc_storage_impl.h"

#include "base/files/file.h"
#include "base/files/file_path.h"
#include "base/files/file_util.h"
#include "base/functional/bind.h"
#include "base/functional/callback.h"
#include "base/strings/stringprintf.h"
#include "base/threading/thread_restrictions.h"
#include "build/build_config.h"

#if BUILDFLAG(IS_STARBOARD)
#include "starboard/configuration_constants.h"
#include "starboard/system.h"
#endif

#if BUILDFLAG(IS_ANDROID)
#include "starboard/android/shared/starboard_bridge.h"

using starboard::android::shared::StarboardBridge;
#endif

namespace h5vcc_storage {

namespace {

constexpr char kTestFileName[] = "cache_test_file.json";
constexpr uint32_t kBufferSize = 16384;  // 16 KB

base::FilePath GetCacheDirectory() {
#if BUILDFLAG(IS_STARBOARD)
  std::string path;
  path.resize(kSbFileMaxPath);
  if (!SbSystemGetPath(kSbSystemPathCacheDirectory, path.data(),
                       kSbFileMaxPath)) {
    NOTREACHED() << "Failed to get cache directory.";
    return base::FilePath();
  }
  return base::FilePath(path.data());
#elif BUILDFLAG(IS_ANDROID)
  JNIEnv* env = base::android::AttachCurrentThread();
  StarboardBridge* starboard_bridge = StarboardBridge::GetInstance();
  return base::FilePath(starboard_bridge->GetCacheAbsolutePath(env));
#else
  NOTREACHED_NORETURN() << "Unsupported platform.";
#endif
}

}  // namespace

// TODO (b/395126160): refactor mojom implementation on Android
H5vccStorageImpl::H5vccStorageImpl(
    content::RenderFrameHost& render_frame_host,
    mojo::PendingReceiver<mojom::H5vccStorage> receiver)
    : content::DocumentService<mojom::H5vccStorage>(render_frame_host,
                                                    std::move(receiver)) {}

void H5vccStorageImpl::Create(
    content::RenderFrameHost* render_frame_host,
    mojo::PendingReceiver<mojom::H5vccStorage> receiver) {
  new H5vccStorageImpl(*render_frame_host, std::move(receiver));
}

void H5vccStorageImpl::WriteTest(uint32_t test_size,
                                 const std::string& test_string,
                                 WriteTestCallback callback) {
  base::ScopedAllowBlockingForTesting allow_blocking;
  base::FilePath cache_dir = GetCacheDirectory();

  LOG(WARNING) << "cache dir " << cache_dir;
  base::DeletePathRecursively(cache_dir);
  base::CreateDirectory(cache_dir);

  auto response = mojom::WriteTestResponse::New();
  response->error = "";
  response->bytes_written = 0;

  base::FilePath test_file_path = cache_dir.Append(kTestFileName);
  base::File test_file(test_file_path,
                       base::File::FLAG_OPEN_ALWAYS | base::File::FLAG_WRITE);

  if (!test_file.IsValid()) {
    response->error = base::StringPrintf("Error while opening ScopedFile: %s",
                                         test_file_path.value().c_str());
    std::move(callback).Run(std::move(response));
    return;
  }

  // Repeatedly write test_string to test_size bytes of write_buffer.
  std::string write_buf;
  int iterations = test_size / test_string.length();
  for (int i = 0; i < iterations; ++i) {
    write_buf.append(test_string);
  }
  write_buf.append(test_string.substr(0, test_size % test_string.length()));

  // Incremental Writes of test_data, copies SbWriteAll, using a maximum
  // kBufferSize per write.
  uint32_t total_bytes_written = 0;

  do {
    auto bytes_written = test_file.WriteAtCurrentPosNoBestEffort(
        write_buf.data() + total_bytes_written,
        std::min(kBufferSize, test_size - total_bytes_written));
    if (bytes_written <= 0) {
      base::DeleteFile(test_file_path);
      response->error = "SbWrite -1 return value error";
      std::move(callback).Run(std::move(response));
      return;
    }
    total_bytes_written += bytes_written;
  } while (total_bytes_written < test_size);

  test_file.Flush();

  response->bytes_written = total_bytes_written;
  std::move(callback).Run(std::move(response));
}

void H5vccStorageImpl::VerifyTest(uint32_t test_size,
                                  const std::string& test_string,
                                  VerifyTestCallback callback) {
  base::ScopedAllowBlockingForTesting allow_blocking;
  auto response = mojom::VerifyTestResponse::New();
  response->bytes_read = 0;
  response->verified = false;
  response->error = "";

  base::FilePath cache_dir = GetCacheDirectory();

  base::FilePath test_file_path = cache_dir.Append(kTestFileName);
  base::File test_file(test_file_path,
                       base::File::FLAG_OPEN | base::File::FLAG_READ);
  if (!test_file.IsValid()) {
    response->error = base::StringPrintf("Error while opening ScopedFile: %s",
                                         test_file_path.value().c_str());
    std::move(callback).Run(std::move(response));
    return;
  }

  // Incremental Reads of test_data, copies SbReadAll, using a maximum
  // kBufferSize per write.
  uint32_t total_bytes_read = 0;

  do {
    auto read_buffer = std::make_unique<char[]>(kBufferSize);
    auto bytes_read = test_file.ReadAtCurrentPosNoBestEffort(
        read_buffer.get(), std::min(kBufferSize, test_size - total_bytes_read));
    if (bytes_read <= 0) {
      base::DeleteFile(test_file_path);
      response->error = "SbRead -1 return value error";
      std::move(callback).Run(std::move(response));
      return;
    }

    // Verify read_buffer equivalent to a repeated test_string.
    for (auto i = 0; i < bytes_read; ++i) {
      if (read_buffer.get()[i] !=
          test_string[(total_bytes_read + i) % test_string.size()]) {
        response->error = "File test data does not match with test data string";
        std::move(callback).Run(std::move(response));
        return;
      }
    }

    total_bytes_read += bytes_read;
  } while (total_bytes_read < test_size);

  if (total_bytes_read != test_size) {
    base::DeleteFile(test_file_path);
    response->error = "File test data size does not match kTestDataSize";
    std::move(callback).Run(std::move(response));
    return;
  }

  base::DeleteFile(test_file_path);
  response->verified = true;
  response->bytes_read = total_bytes_read;
  std::move(callback).Run(std::move(response));
}

}  // namespace h5vcc_storage
