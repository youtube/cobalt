...
TEST(PROCESSORS, smt_id) {
	for (uint32_t i = 0; i < cpuinfo_get_processors_count(); i++) {
		ASSERT_EQ(i % 2, cpuinfo_get_processor(i)->smt_id);
	}
}

TEST(PROCESSORS, core) {
	for (uint32_t i = 0; i < cpuinfo_get_processors_count(); i++) {
		ASSERT_EQ(cpuinfo_get_core(i / 2), cpuinfo_get_processor(i)->core);
	}
}
...
TEST(PROCESSORS, l1i) {
	for (uint32_t i = 0; i < cpuinfo_get_processors_count(); i++) {
		ASSERT_EQ(cpuinfo_get_l1i_cache(i / 2), cpuinfo_get_processor(i)->cache.l1i);
	}
}

TEST(PROCESSORS, l1d) {
	for (uint32_t i = 0; i < cpuinfo_get_processors_count(); i++) {
		ASSERT_EQ(cpuinfo_get_l1d_cache(i / 2), cpuinfo_get_processor(i)->cache.l1d);
	}
}
...
TEST(CORES, processor_start) {
	for (uint32_t i = 0; i < cpuinfo_get_cores_count(); i++) {
		ASSERT_EQ(i * 2, cpuinfo_get_core(i)->processor_start);
	}
}

TEST(CORES, processor_count) {
	for (uint32_t i = 0; i < cpuinfo_get_cores_count(); i++) {
		ASSERT_EQ(2, cpuinfo_get_core(i)->processor_count);
	}
}
...
TEST(CORES, uarch) {
	for (uint32_t i = 0; i < cpuinfo_get_cores_count(); i++) {
		ASSERT_EQ(cpuinfo_uarch_saltwell, cpuinfo_get_core(i)->uarch);
	}
}

TEST(CORES, cpuid) {
	for (uint32_t i = 0; i < cpuinfo_get_cores_count(); i++) {
		ASSERT_EQ(UINT32_C(0x00030651), cpuinfo_get_core(i)->cpuid);
	}
}
...
TEST(ISA, rdtscp) {
	ASSERT_FALSE(cpuinfo_has_x86_rdtscp());
}
...
TEST(ISA, prefetchw) {
	ASSERT_FALSE(cpuinfo_has_x86_prefetchw());
}
...
TEST(ISA, sse4_1) {
	ASSERT_FALSE(cpuinfo_has_x86_sse4_1());
}

TEST(ISA, sse4_2) {
	ASSERT_FALSE(cpuinfo_has_x86_sse4_2());
}
...
TEST(ISA, popcnt) {
	ASSERT_FALSE(cpuinfo_has_x86_popcnt());
}
...
TEST(ISA, aes) {
	ASSERT_FALSE(cpuinfo_has_x86_aes());
}

TEST(ISA, pclmulqdq) {
	ASSERT_FALSE(cpuinfo_has_x86_pclmulqdq());
}

TEST(ISA, rdrand) {
	ASSERT_FALSE(cpuinfo_has_x86_rdrand());
}
...
TEST(L1I, processors) {
	for (uint32_t i = 0; i < cpuinfo_get_l1i_caches_count(); i++) {
		ASSERT_EQ(i * 2, cpuinfo_get_l1i_cache(i)->processor_start);
		ASSERT_EQ(2, cpuinfo_get_l1i_cache(i)->processor_count);
	}
}
...
TEST(L1D, processors) {
	for (uint32_t i = 0; i < cpuinfo_get_l1d_caches_count(); i++) {
		ASSERT_EQ(i * 2, cpuinfo_get_l1d_cache(i)->processor_start);
		ASSERT_EQ(2, cpuinfo_get_l1d_cache(i)->processor_count);
	}
}
