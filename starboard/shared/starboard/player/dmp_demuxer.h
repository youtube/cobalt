...
class DmpDemuxer {
 public:
  ...
  // Create a new dmp demuxer by extracting a track from an existing demuxer.
  DmpDemuxer(DmpDemuxer& demuxer, SbMediaType type);
  // Construct a new dmp demuxer by assigning dmp readers to each media track.
  explicit DmpDemuxer(MediaTypeToDmpReaderMap& dmp_readers);
  ~DmpDemuxer() = default;
  ...
  SbMediaAudioCodec GetAudioCodecType() const;

  SbMediaVideoCodec GetVideoCodecType() const;
