...
void ShellPermissionManager::RequestPermissions(
  ...
  if (render_frame_host->IsNestedWithinFencedFrame()) {
    std::move(callback).Run(std::vector<blink::mojom::PermissionStatus>(
        request_description.permissions.size(), blink::mojom::PermissionStatus::DENIED));
    return;
  }
...
void ShellPermissionManager::ResetPermission(PermissionType permission,
...
ShellPermissionManager::GetPermissionStatusForWorker(
  ...
  return GetPermissionStatus(permission_descriptor, worker_origin, worker_origin);
}
