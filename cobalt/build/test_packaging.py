...
def test_packaging(output_dir=None):
  if output_dir:
  ...
  with temp_dir_ctx as temp_dir:
    ...
    # Create the template and Dockerfile needed for browsertests
    template_dir = os.path.join(src_dir, 'cobalt/testing/browser_tests/tools')
    os.makedirs(template_dir, exist_ok=True)
    template_path = os.path.join(template_dir, 'run_tests.template.py')
    ...
    # Mock collect_test_artifacts.py
    collect_script_path = os.path.join(template_dir,
                                       'collect_test_artifacts.py')
    with open(collect_script_path, 'w', encoding='utf-8') as f:
      f.write("""
import argparse
import subprocess
import os
...
with tarfile.open(args.output, 'w:gz') as tar:
    # Create mock run_tests.py inside
    with open('run_tests.py', 'w') as f:
        f.write("TARGET_MAP = {'cobalt_browsertests': {}}")
    tar.add('run_tests.py')
...
""")
    ...
    if not os.path.exists(explicit_archive_path):
      print(f'FAILED: explicit archive {explicit_archive_path} not found!')
      sys.exit(1)

    print('SUCCESS: Browsertest packaging test passed!')


if __name__ == '__main__':
  parser = argparse.ArgumentParser()
  parser.add_argument(
      '--output-dir', help='Directory to store test artifacts (optional)')
  args = parser.parse_args()
  test_packaging(args.output_dir)
