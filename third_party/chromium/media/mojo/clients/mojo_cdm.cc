...
#include "third_party/chromium/media/mojo/clients/mojo_cdm.h"
...
#include "third_party/chromium/media/mojo/clients/mojo_decryptor.h"
...
void MojoCdm::SetServerCertificate(const std::vector<uint8_t>& certificate,
  ...
  if (!remote_cdm_) {
    promise->reject(media_m96::CdmPromise::Exception::INVALID_STATE_ERROR,
...
void MojoCdm::GetStatusForPolicy(HdcpVersion min_hdcp_version,
  ...
  if (!remote_cdm_) {
    promise->reject(media_m96::CdmPromise::Exception::INVALID_STATE_ERROR, 0,
                    "CDM connection lost.");
    return;
  }
