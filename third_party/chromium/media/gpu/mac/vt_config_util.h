// Copyright 2020 The Chromium Authors. All rights reserved.
// Use of this source code is governed by a BSD-style license that can be
// found in the LICENSE file.

#ifndef THIRD_PARTY_CHROMIUM_MEDIA_GPU_MAC_VT_CONFIG_UTIL_H_
#define THIRD_PARTY_CHROMIUM_MEDIA_GPU_MAC_VT_CONFIG_UTIL_H_

#include <CoreFoundation/CoreFoundation.h>
#include <CoreMedia/CoreMedia.h>

#include "third_party/chromium/media/base/video_codecs.h"
#include "third_party/chromium/media/base/video_color_space.h"
#include "third_party/chromium/media/gpu/media_gpu_export.h"
#include "third_party/abseil-cpp/absl/types/optional.h"
#include "ui/gfx/hdr_metadata.h"

namespace media_m96 {

MEDIA_GPU_EXPORT CFMutableDictionaryRef
CreateFormatExtensions(CMVideoCodecType codec_type,
                       VideoCodecProfile profile,
                       const VideoColorSpace& color_space,
                       absl::optional<gfx::HDRMetadata> hdr_metadata);

}  // namespace media_m96

#endif  // THIRD_PARTY_CHROMIUM_MEDIA_GPU_MAC_VT_CONFIG_UTIL_H_
