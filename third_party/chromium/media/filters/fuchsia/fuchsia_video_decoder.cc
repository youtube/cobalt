...
#include "third_party/chromium/media/filters/fuchsia/fuchsia_video_decoder.h"
...
#include "third_party/chromium/media/base/video_aspect_ratio.h"
#include "third_party/chromium/media/base/video_frame.h"
#include "third_party/chromium/media/base/video_util.h"
#include "third_party/chromium/media/fuchsia/cdm/fuchsia_cdm_context.h"
#include "third_party/chromium/media/fuchsia/cdm/fuchsia_decryptor.h"
#include "third_party/chromium/media/fuchsia/cdm/fuchsia_stream_decryptor.h"
#include "third_party/chromium/media/fuchsia/common/decrypting_sysmem_buffer_stream.h"
#include "third_party/chromium/media/fuchsia/common/passthrough_sysmem_buffer_stream.h"
#include "third_party/chromium/media/fuchsia/common/stream_processor_helper.h"
 ...
 public:
  ...
  scoped_refptr<VideoFrame> CreateFrame(VideoPixelFormat pixel_format,
    ...
    auto frame = VideoFrame::WrapNativeTextures(
        ...
        base::BindPostTaskToCurrentDefault(base::BindOnce(&OutputMailbox::OnFrameDestroyed,
 ...
 private:
  void OnFrameDestroyed(const gpu::SyncToken& sync_token) {
    ...
    raster_context_provider_->ContextSupport()->SignalSyncToken(
        sync_token_,
        base::BindPostTaskToCurrentDefault(base::BindOnce(&OutputMailbox::OnSyncTokenSignaled,
...
void FuchsiaVideoDecoder::Initialize(const VideoDecoderConfig& config,
  ...
  fuchsia::media_m96::StreamProcessorPtr decoder;
...
StatusCode FuchsiaVideoDecoder::InitializeSysmemBufferStream(
  ...
  *out_secure_mode = false;
  ...
  if (is_encrypted) {
    ...
    // media_m96::Decryptor interface). Otherwise (e.g. for ClearKey CDM) use
...
void FuchsiaVideoDecoder::OnStreamProcessorAllocateOutputBuffers(
    const fuchsia::media_m96::StreamBufferConstraints& output_constraints) {
  ReleaseOutputBuffers();
...
void FuchsiaVideoDecoder::OnStreamProcessorOutputFormat(
    fuchsia::media_m96::StreamOutputFormat output_format) {
