...
#define MAX_SIGNAL_NUMBER 127

int musl_sigaddset(musl_sigset_t* set, int signum) {
  if (set == nullptr || signum <= 0 || signum > MAX_SIGNAL_NUMBER) {
    errno = EINVAL;
    return -1;
  }
...
int __abi_wrap_sigaction(int signum,
  ...
  if (act) {
    ...
    for (int i = 1; i < NSIG; ++i) {
      if (i <= MAX_SIGNAL_NUMBER && musl_sigismember(&act->sa_mask, i)) {
  ...
  if (result == 0 && oldact) {
    *oldact = {};
    ...
    for (int i = 1; i < NSIG; ++i) {
      if (i <= MAX_SIGNAL_NUMBER && sigismember(&platform_oldact.sa_mask, i)) {
...
int __abi_wrap_pthread_sigmask(int how,
  ...
  if (set) {
    ...
    // musl_set
    for (int i = 1; i < NSIG; ++i) {
      if (i <= MAX_SIGNAL_NUMBER && musl_sigismember(set, i) == 1) {
  ...
  ret = pthread_sigmask(how, p_native_set, p_native_oldset);
  ...
  if (ret == 0 && oldset != NULL) {
    ...
    for (int i = 1; i < NSIG; ++i) {
      if (i <= MAX_SIGNAL_NUMBER && sigismember(&native_oldset, i) == 1) {
