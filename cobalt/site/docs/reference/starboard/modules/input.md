...
*   `kSbInputDeviceTypeGesture`
    ...
    Produces `Move` Move , `Press` Press and `Unpress` Unpress events.
...
*   `kSbInputDeviceTypeKeyboard`
    ...
    Produces `Press` Press and `Unpress` Unpress events.
...
*   `kSbInputEventTypeMove`
    ...
    Device movement. For `Mouse` Mouse and `Gesture` Gesture devices, movement
    tracks an absolute cursor position. For `TouchPad` TouchPad devices, only
    relative movement is provided.
*   `kSbInputEventTypePress`

    Key or button press. This can be a keyboard key, mouse or game controller
    button, touchscreen press, or gesture. An `Unpress` Unpress event is
    dispatched when the `Press` Press event terminates (for example, when
    releasing the key or raising the finger). The client is responsible for
    generating repeat press events.
*   `kSbInputEventTypeUnpress`

    Key or button release. The counterpart to `Press` Press , this event is sent
    when the pressed key or button is released.
*   `kSbInputEventTypeWheel`

    Wheel movement. Provides relative movement of the `Mouse` Mouse wheel.
...
Event data for `kSbEventTypeInput` kSbEventTypeInput events.
...
*   `SbKey key`
    ...
    ID space, defined by the `SbKey` SbKey enum in `key.h` key.h .
*   `wchar_t character`

    The character that corresponds to the key. For external keyboards, this
    character depends on the keyboard layout. The value is `0` 0 if there is no
...
*   `unsigned int key_modifiers`
    ...
    Key modifiers (e.g. `Ctrl` Ctrl , `Shift` Shift ) held down during this
    input event.
*   `SbInputVector position`

    The (x, y) coordinates of the persistent cursor controlled by this device.
    The value is `0` 0 if this data is not applicable. For
    `kSbInputEventTypeMove` events from `kSbInputDeviceTypeGamepad` devices,
    this field represents stick position in the range `[-1, 1]` [-1, 1] , where
    positive values indicate down and right directions.
*   `SbInputVector delta`

    The relative motion vector of this input. The value is `0` 0 if this data is
    not applicable.
*   `float pressure`

    The normalized pointer pressure in the range `[0, 1]` [0, 1] , where `0` 0
    and `1` 1 represent minimum and maximum detectable pressure. Use `NaN` NaN
    if the device does not report pressure. This value applies to mouse and
    touchscreen input events.
*   `SbInputVector size`

    The contact geometry size `(width, height)` (width, height) of the pointer,
    defining the contact area. If `(NaN, NaN)` (NaN, NaN) is specified, `(0, 0)`
    (0, 0) is used. This value applies to mouse and touchscreen input events.
*   `SbInputVector tilt`

    The tilt angle `(x, y)` (x, y) in degrees, in the range `[-90, 90]` [-90,
    90] relative to the z-axis. Positive values indicate tilt to the right (x)
    and towards the user (y). Use `(NaN, NaN)` (NaN, NaN) if the device does not
    report tilt. This value applies to mouse and touchscreen input events.
*   `const char * input_text`

    The text to input for events of type `Input` Input .
*   `bool is_composing`

    Set to `true` true if the input event is part of an ongoing composition.
