...
// TODO(bokan): |ignore_offset| is added temporarily to allow testing the
// pinch-zoom on desktop in a less strict way. Currently, touchscreen pinch-zoom
// on desktop doesn't zoom to the expected location. Until that's fixed we'll
// run tests without checking the location so we guard regressing the scale but
// once it's fixed we can remove this and run the strict tests.
// https://crbug.com/787615
function runTest(t, testCase, ignore_offset) {
          ...
          testCase.scale, centerX, centerY, t.step_func(() => {
            waitForCompositorCommit().then(t.step_func(() => {
              ...
              assert_approx_equals(
                  window.visualViewport.scale, expectedScale, kScaleEpsilon,
...
async function runAllTestCases(t, testCases) {
  for (const testCase of testCases) {
    await runTest(t, testCase, false);
  }
}

async function runAllTestCasesDesktop(t, testCases) {
  for (const testCase of testCases) {
    await runTest(t, testCase, true);
  }
}
