...
ExoPlayer::~ExoPlayer() {
  ...
  bridge_.reset();
  player_status_func_(player_, context_, kSbPlayerStateDestroyed, ticket_);
  SB_LOG(INFO) << "Exiting exoplayer destructor";
}
...
void ExoPlayer::WriteEndOfStream(SbMediaType stream_type) {
  SB_DCHECK(bridge_);
  SB_LOG(INFO) << "Called WriteEndOfStream()";
  bridge_->WriteEndOfStream(stream_type);
}
