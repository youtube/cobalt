...
StarboardRenderer::~StarboardRenderer() {
  ...
  LOG(INFO) << "Destroying StarboardRenderer.";

  base::AutoLock auto_lock(lock_);
  // Explicitly set |player_bridge_| before destroying it.
  auto player_bridge = std::move(player_bridge_);
  player_bridge.reset();
}
...
void StarboardRenderer::Initialize(MediaResource* media_resource,
  ...
  audio_stream_ = media_resource_->GetFirstStream(DemuxerStream::AUDIO);
  video_stream_ = media_resource_->GetFirstStream(DemuxerStream::VIDEO);
  ...
  DCHECK(!is_encrypted);  // TODO: Support encrypted playbacks
  */
...
void StarboardRenderer::Flush(base::OnceClosure flush_cb) {
  ...
  // The function will be called during seek, before Demuxer::Seek() is called.
  // It's possible that there are in-flight Demuxer::Read() calls posted in
  // OnDemuxerStreamRead(), we will wait until they are finished.
...
base::TimeDelta StarboardRenderer::GetMediaTime() {
  ...
  if (player_bridge_) {
    ...
    if (video_frames_decoded > last_video_frames_decoded_) {
      statistics.video_frames_decoded = video_frames_decoded - last_video_frames_decoded_;
      last_video_frames_decoded_ = video_frames_decoded;
    }

    if (video_frames_dropped > last_video_frames_dropped_) {
      statistics.video_frames_dropped = video_frames_dropped - last_video_frames_dropped_;
      last_video_frames_dropped_ = video_frames_dropped;
    }

    if (statistics.video_frames_decoded > 0 || statistics.video_frames_dropped > 0) {
      // TODO: Double check if we should report more statistics.
      // TODO: Report dropped frames for the last time at the end of playback?
      task_runner_->PostTask(FROM_HERE, base::BindOnce(&RendererClient::OnStatisticsUpdate,
...
void StarboardRenderer::CreatePlayerBridge(PipelineStatusCallback init_cb) {
...

  {
    ...
    // In the extreme case that CreatePlayerBridge() is called when a
    ...
    player_bridge_.reset(new SbPlayerBridge(
        ...
        kSbWindowInvalid, kSbDrmSystemInvalid, this, nullptr, false,
        kSbPlayerOutputModeInvalid, "", 1024 * 1024 * 8,
        "pipeline_identifier"));
...
void StarboardRenderer::OnDemuxerStreamRead(
  ...
  if (pending_flush_cb_) {
    ...
    if (!audio_read_in_progress_ && !video_read_in_progress_) {
      auto flush_cb = std::move(pending_flush_cb_).value();
      std::move(flush_cb).Run();
    }
    return;
  }
...
void StarboardRenderer::OnPlayerStatus(SbPlayerState state) {
  ...
  // In case if Stop() has been called.
  if (!player_bridge_) {
    if (state == kSbPlayerStateDestroyed) {
