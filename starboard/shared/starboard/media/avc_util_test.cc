...
TEST(AvcParameterSetsTest, CombinedSizeWithOptionalParameter) {
  ...
  ASSERT_FALSE(
      ...
      (parameter_sets_aud_first.combined_size_in_bytes() +
       kAudInAnnexB.size()));

  auto pps_last = kPpsInAnnexB + kSpsInAnnexB + kAudInAnnexB + kIdrInAnnexB;
  AvcParameterSets parameter_sets(kAnnexB, pps_last.data(), pps_last.size());
  ASSERT_FALSE(parameter_sets.combined_size_in_bytes_with_optionals() ==
               (parameter_sets.combined_size_in_bytes() + kAudInAnnexB.size()));
}
