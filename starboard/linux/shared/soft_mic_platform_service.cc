...
bool Has(const char* name) {
  ...
  return SbStringCompareAll(name, "com.google.youtube.tv.SoftMic") == 0;
}
...
void* Send(CobaltExtensionPlatformService service,
  ...
  SbMemoryCopy(message, data, length);
  message[length] = '\0';

  SB_LOG(INFO) << "Send() message: " << message;

  if (SbStringCompareAll(message, kGetMicSupport) == 0) {
  ...
  } else if (SbStringCompareAll(message, kNotifySearchActive) == 0) {
    // Process "notifySearchActive" web app message.
    SB_LOG(INFO) << "Send() kNotifySearchActive message received";
    valid_message_received = true;
  } else if (SbStringCompareAll(message, kNotifySearchInactive) == 0) {
