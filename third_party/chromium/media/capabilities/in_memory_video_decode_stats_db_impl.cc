...
#include "third_party/chromium/media/capabilities/in_memory_video_decode_stats_db_impl.h"
...
#include "third_party/chromium/media/capabilities/video_decode_stats_db_provider.h"
...
void InMemoryVideoDecodeStatsDBImpl::AppendDecodeStats(
  ...
  // Bind to avoid reentrancy.
  std::move(base::BindPostTaskToCurrentDefault(std::move(append_done_cb))).Run(true);
}
...
void InMemoryVideoDecodeStatsDBImpl::ClearStats(
  ...
  // Bind to avoid reentrancy.
  std::move(base::BindPostTaskToCurrentDefault(std::move(destroy_done_cb))).Run();
}
