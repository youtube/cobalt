...
media_m96::VideoCaptureFormats GetDeviceSupportedFormats(
    const media_m96::VideoCaptureDeviceDescriptor& descriptor) {
  media_m96::VideoCaptureFormats formats;
  ...
  if (device == nil)
    return media_m96::VideoCaptureFormats();
  for (AVCaptureDeviceFormat* format in device.formats) {
    ...
    const media_m96::VideoPixelFormat pixelFormat = [VideoCaptureDeviceAVFoundation
    ...
    for (AVFrameRateRange* frameRate in
      ...
      media_m96::VideoCaptureFormat format(
          gfx::Size(dimensions.width, dimensions.height),
          frameRate.maxFrameRate, pixelFormat);
      DVLOG(2) << descriptor.display_name() << " "
               << media_m96::VideoCaptureFormat::ToString(format);
