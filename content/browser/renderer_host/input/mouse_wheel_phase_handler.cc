...
void MouseWheelPhaseHandler::AddPhaseIfNeededAndScheduleEndEvent(
  ...
  // If the scroll is generated by a device that does not support fling gesture,
  // reset the state so that it can start the mouse_wheel_end_dispatch_timer_,
  // which will eventually generate GestureScrollEnd event.
  if (!is_fling_capable &&
