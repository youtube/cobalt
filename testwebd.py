...
def test_conn(delay):
    selenium_webdriver_module = webdriver_utils.import_selenium_module('webdriver')
    rc = selenium_webdriver_module.remote.remote_connection
    executor = rc.RemoteConnection(webdriver_url)
    executor.set_timeout(2)
    webdriver = selenium_webdriver_module.Remote(executor, COBALT_WEBDRIVER_CAPABILITIES)
    time.sleep(delay)
