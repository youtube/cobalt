...
#include "third_party/chromium/media/renderers/paint_canvas_video_renderer.h"
#include "third_party/chromium/media/renderers/shared_image_video_frame_test_utils.h"
...
scoped_refptr<VideoFrame> CreateTestY16Frame(const gfx::Size& coded_size,
  ...
  return media_m96::VideoFrame::WrapExternalData(
      media_m96::PIXEL_FORMAT_Y16, coded_size, visible_rect, visible_rect.size(),
...
void PaintCanvasVideoRendererTest::PaintRotated(
  ...
  switch (color) {
    case kNone:
      break;
    case kRed:
      media_m96::FillYUV(video_frame.get(), 76, 84, 255);
      break;
    case kGreen:
      media_m96::FillYUV(video_frame.get(), 149, 43, 21);
      break;
    case kBlue:
      media_m96::FillYUV(video_frame.get(), 29, 255, 107);
      break;
  }
  cc::PaintFlags flags;
...
TEST_F(PaintCanvasVideoRendererTest, NoTimestamp) {
  ...
  video_frame->set_timestamp(media_m96::kNoTimestamp);
...
TEST_F(PaintCanvasVideoRendererTest, CroppedFrameToRGBParallel) {
  ...
  std::unique_ptr<uint8_t, base::AlignedFreeDeleter> memory(
      static_cast<uint8_t*>(base::AlignedAlloc(
          allocation_size, media_m96::VideoFrame::kFrameAddressAlignment)));
...
TEST_F(PaintCanvasVideoRendererTest, Y16) {
  ...
  std::unique_ptr<unsigned char, base::AlignedFreeDeleter> memory(
      static_cast<unsigned char*>(base::AlignedAlloc(
          byte_size, media_m96::VideoFrame::kFrameAddressAlignment)));
...
TEST_F(PaintCanvasVideoRendererTest, CorrectFrameSizeToVisibleRect) {
  ...
  auto video_frame = media_m96::VideoFrame::WrapExternalData(
      media_m96::PIXEL_FORMAT_Y16, coded_size, gfx::Rect(visible_size),
