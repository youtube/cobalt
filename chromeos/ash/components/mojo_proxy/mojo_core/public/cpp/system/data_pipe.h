...
#ifndef CHROMEOS_ASH_COMPONENTS_MOJO_PROXY_MOJO_CORE_PUBLIC_CPP_SYSTEM_DATA_PIPE_H_
#define CHROMEOS_ASH_COMPONENTS_MOJO_PROXY_MOJO_CORE_PUBLIC_CPP_SYSTEM_DATA_PIPE_H_
...
class DataPipeProducerHandle : public Handle {
 public:
  ...
  // 2^32 bytes may result in `MOJO_LEGACY_RESULT_INVALID_ARGUMENT` unless
  // `MojoCreateDataPipeOptions::element_num_bytes` is 1.
  //
  // `bytes_written` is an out parameter that on `MOJO_LEGACY_RESULT_OK`
  // communicates how many bytes (from the beginning of `data`) were actually
  // written into the mojo data pipe (depending on the size of the internal pipe
  // buffer, this may be less than `data.size()`).
  //
  // Note that instead of passing specific `flags`, a more direct method can be
  // used instead:
  // - `MOJO_LEGACY_WRITE_DATA_FLAG_ALL_OR_NONE` => `WriteAllData`
  MojoResult WriteData(base::span<const uint8_t> data,
    ...
    // to return `MOJO_LEGACY_RESULT_INVALID_ARGUMENT` in this case (as
    // reflected in the doc comment above).
  ...
  MojoResult WriteAllData(base::span<const uint8_t> data) const {
    // Ok to ignore `bytes_written` because
    // `MOJO_LEGACY_WRITE_DATA_FLAG_ALL_OR_NONE` means that
    // `MOJO_LEGACY_RESULT_OK` will be returned only if exactly `data.size()`
    // bytes have been written (i.e. if `data.size() == bytes_written`).
    size_t bytes_written = 0;
    return WriteData(data, MOJO_LEGACY_WRITE_DATA_FLAG_ALL_OR_NONE,
                     bytes_written);
  }
  ...
  //
  // `buffer` is an out parameter that on `MOJO_LEGACY_RESULT_OK` points to a
  // mutable, pipe-owned buffer, that the caller can write into.  The caller
  // should call `EndWriteData` when they are done writing into the `buffer`.
...
class DataPipeConsumerHandle : public Handle {
 public:
  ...
  //
  // `bytes_read` is an out parameter that on `MOJO_LEGACY_RESULT_OK`
  // communicates how many bytes (at the beginning of `buffer`) were actually
  // read (depending on the size of the internal pipe buffer, this may be less
  // than `buffer.size()`).
  //
  // Note that instead of passing specific `flags`, a more direct method can be
  // used instead:
  // - `MOJO_LEGACY_READ_DATA_FLAG_DISCARD` => `DiscardData`
  MojoResult ReadData(MojoReadDataFlags flags,
    ...
    // But, for compatibility with C++ containers, the C++ APIs take `size_t`.
    //
    // Input value of `*num_bytes` is ignored in
    // `MOJO_LEGACY_READ_DATA_FLAG_QUERY` mode and otherwise is an _upper_ bound
    // on how many bytes will be read (or discarded in
    // `MOJO_LEGACY_READ_DATA_FLAG_DISCARD`).  Therefore it is okay to use
  ...
  // |MOJO_LEGACY_READ_DATA_FLAG_DISCARD| for complete documentation.
  //
  // `bytes_to_discard` is an input parameter that specifies how many bytes from
  // the pipe should be read and discarded.
  //
  // `bytes_discarded` is an output parameter that on `MOJO_LEGACY_RESULT_OK`
  // specifies how many bytes were actually discarded.
...
#endif  // CHROMEOS_ASH_COMPONENTS_MOJO_PROXY_MOJO_CORE_PUBLIC_CPP_SYSTEM_DATA_PIPE_H_
