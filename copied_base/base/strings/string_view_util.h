...
constexpr auto as_string_view(span<const char> s) {
  ...
  // C++17 compatibility: the iterators constructor was added in C++20.
  return UNSAFE_BUFFERS(std::string_view(s.data(), s.end() - s.begin()));
}
...
constexpr auto as_string_view(span<const char16_t> s) {
  ...
  // C++17 compatibility: the iterators constructor was added in C++20.
  return UNSAFE_BUFFERS(std::u16string_view(s.data(), s.end() - s.begin()));
}
constexpr auto as_string_view(span<const wchar_t> s) {
  ...
  // C++17 compatibility: the iterators constructor was added in C++20.
  return UNSAFE_BUFFERS(std::wstring_view(s.data(), s.end() - s.begin()));
}
