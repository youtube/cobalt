...
class GlobalFeaturesTest : public testing::Test {
 ...
 protected:
  ...
  void SetUp() override {
    ...
    LOG(ERROR) << "PATH: " << temp_dir_.GetPath();
    LOG(ERROR) << "IS VALID: " << temp_dir_.IsValid();
  }
};
...
TEST_F(GlobalFeaturesTest, CreatesPrefServiceOnInitialization) {
  ...
  LOG(ERROR) << "Initialization status: "
             << instance->metrics_local_state()->GetInitializationStatus();
