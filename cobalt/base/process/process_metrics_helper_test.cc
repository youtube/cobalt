...
bool WorkUntilUsageIsDetected(Thread* thread) {
  ...
  for (int i = 0; i < 10; i++) {
    ...
    EXPECT_TRUE(cpu_per_thread.is_list());
    auto& list = cpu_per_thread.GetList();
    int found_populated = 0;
    for (auto& entry_value : list) {
      EXPECT_TRUE(entry_value.is_dict());
      base::Value* id_value = entry_value.FindKey("id");
      EXPECT_NE(nullptr, id_value);
      if (id_value->GetInt() == thread->GetThreadId()) {
        int utime;
        if (!StringToInt(entry_value.FindKey("utime")->GetString(), &utime))
          return false;
        int stime;
        if (!StringToInt(entry_value.FindKey("stime")->GetString(), &stime))
          return false;
        if (utime + stime > 0) {
          return true;
        }
...
void CheckThreadUsage(const base::Value::ListStorage& list,
                      PlatformThreadId thread_id,
                      const std::string& thread_name) {
  for (auto& entry_value : list) {
    EXPECT_TRUE(entry_value.is_dict());
    auto* id_value = entry_value.FindKey("id");
    EXPECT_NE(nullptr, id_value);
    EXPECT_NE(nullptr, entry_value.FindKey("name"));
    EXPECT_NE(nullptr, entry_value.FindKey("utime"));
    EXPECT_NE(nullptr, entry_value.FindKey("stime"));
    EXPECT_NE(nullptr, entry_value.FindKey("usage_seconds"));
    EXPECT_TRUE(id_value->is_int());
    if (id_value->is_int() && id_value->GetInt() == thread_id) {
      EXPECT_EQ(thread_name, entry_value.FindKey("name")->GetString());
      EXPECT_GT(entry_value.FindKey("utime")->GetString().size(), 0);
      EXPECT_GT(entry_value.FindKey("stime")->GetString().size(), 0);
      EXPECT_GT(entry_value.FindKey("usage_seconds")->GetDouble(), 0.0);
      return;
    }
  }
...
ProcessMetricsHelper::ReadCallback GetNulloptCallback() {
  return BindOnce(
      []() -> base::Optional<std::string> { return base::nullopt; });
}

ProcessMetricsHelper::ReadCallback GetUptimeCallback() {
  return BindOnce([]() -> base::Optional<std::string> {
...
TEST_F(ProcessMetricsHelperTest, GetCumulativeCPUUsagePerThread) {
  int initial_num_threads;
  {
    base::Value cpu_per_thread =
        ProcessMetricsHelper::GetCumulativeCPUUsagePerThread();
    initial_num_threads =
        cpu_per_thread.is_list() ? cpu_per_thread.GetList().size() : 0;
  }
