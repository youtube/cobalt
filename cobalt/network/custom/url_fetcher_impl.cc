...
URLFetcherImpl::URLFetcherImpl(
    const GURL& url, RequestType request_type, URLFetcherDelegate* d,
    net::NetworkTrafficAnnotationTag traffic_annotation)
    : core_(
          new URLFetcherCore(this, url, request_type, d, traffic_annotation)) {}

URLFetcherImpl::~URLFetcherImpl() { core_->Stop(); }
...
void URLFetcherImpl::SetUploadFilePath(
  ...
  core_->SetUploadFilePath(upload_content_type, file_path, range_offset,
                           range_length, file_task_runner);
}
...
int URLFetcherImpl::GetLoadFlags() const { return core_->GetLoadFlags(); }
...
bool URLFetcherImpl::WasCached() const { return core_->WasCached(); }
...
void URLFetcherImpl::Start() { core_->Start(); }

const GURL& URLFetcherImpl::GetOriginalURL() const {
  return core_->GetOriginalURL();
}

const GURL& URLFetcherImpl::GetURL() const { return core_->GetURL(); }

Error URLFetcherImpl::GetStatus() const { return core_->GetStatus(); }

int URLFetcherImpl::GetResponseCode() const { return core_->GetResponseCode(); }
...
// static
void URLFetcherImpl::CancelAll() { URLFetcherCore::CancelAll(); }
...
// static
URLFetcherFactory* URLFetcherImpl::factory() { return g_factory; }
