...
def execute_command(
    ...
    try:
        ...
        t1 = threading.Thread(target=stream_reader, args=(process.stdout, sys.stdout, stdout_lines))
        t2 = threading.Thread(target=stream_reader, args=(process.stderr, sys.stderr, stderr_lines))
...
class CobaltCheckoutSetup:
    def __init__(self, options: argparse.Namespace):
        ...
        # Locate tracking file inside top directory parent enclosing project sources
    ...
    def _load_checkpoints(self) -> dict:
        ...
        if self.options.reset_checkpoints:
            print("Bypassing stored status data: Re-running all configuration phases.")
            return {}
        ...
        if self.checkpoint_file.exists():
            try:
                ...
                print(f"Warning: checkpoint tracking manifest parse error: {err}", file=sys.stderr)
    ...
    def _save_checkpoint(self, stage_name: str):
        ...
        try:
            ...
            print(f"Warning: failed recording validation state on storage: {err}", file=sys.stderr)
    ...
    def validate_configuration(self):
        ...
        if self.options.non_interactive:
            if not self.options.github_user:
                raise ValueError(
                    "Early Validation Failure: Parameter `--github-user` is required for "
                    "non-interactive runs when configuring fork targets."
                )

    def prompt_user(self, prompt_msg: str, default: Optional[str] = None) -> str:
        """Handles operational terminal user interaction or fails non-interactively."""
        if self.options.non_interactive:
            if default is not None:
                return default
            raise ValueError(f"Missing required parameter for automated runtime: {prompt_msg}")
    ...
    def initialize(self):
        ...
        if not (self.repo_root / ".git").is_dir():
            print("Error: Please run this script from the root of the Cobalt Git repository.", file=sys.stderr)
            sys.exit(1)
        ...
        if self.options.internal is not None:
            ...
            choice = self.prompt_user("Are you an internal Google user? (y/n)").lower()
        ...
        if not self.is_stage_completed("disable_telemetry"):
            ...
            print("Skipping Phase: disable_telemetry (Already marked complete)")
    ...
    def setup_depot_tools(self):
        """Installs, updates, and validates Google chromium repository helper systems."""
        print("\n--- Setting up depot_tools ---")
        
        # Design requirement: depot_tools must be created within top parent directory enclosing src/
        ...
        if depot_tools_dir.is_dir():
            ...
            if not self.options.non_interactive:
                input(f"depot_tools not found at {depot_tools_dir}. Press Enter to clone it there...")

            tools_dir.mkdir(parents=True, exist_ok=True)
            execute_command(
                ["git", "clone", "https://chromium.googlesource.com/chromium/tools/depot_tools.git", str(depot_tools_dir)]
            )
        ...
        try:
            ...
            print(f"Depot tools bootstrap failure: Validation was not completed via gclient.", file=sys.stderr)
            raise err

        print(f"Prepended {depot_tools_dir} to current session PATH variable.")

        if not self.options.non_interactive:
            print("\nPlease add the following to your ~/.bashrc manually if not present:")
            print(f'export PATH="{depot_tools_dir}:$PATH"')
            input("Press Enter to continue...")

    def setup_remotes(self):
        """Injects fork integration remotes dynamically while validating connections."""
        print("\n--- Adding Remotes ---")

        # Safely check for base origin/upstream _gclient remote configurations
        try:
            execute_command(["git", "remote", "get-url", "_gclient"], cwd=self.repo_root)
            print("Remote `_gclient` confirmed.")
        except subprocess.CalledProcessError:
            execute_command(
                ["git", "remote", "add", "_gclient", "git@github.com:youtube/cobalt.git"],
                cwd=self.repo_root
            )
        ...
        if self.options.github_user:
            ...
            self.github_user = self.prompt_user("Enter your GitHub username to add fork remote")
        ...
        if self.github_user:
            ...
            try:
                ...
                current_remote = process_res.stdout.strip() if process_res.returncode == 0 else ""

                if current_remote:
                    if current_remote == fork_target_url:
                        print("Remote `fork` configured correctly.")
                    else:
                        print(f"Updating `fork` URL remote configuration: {current_remote} -> {fork_target_url}")
                        execute_command(
    ...
    def setup_gclient(self):
        ...
        # Requirement: gclient workflow looks for definitions located in immediate parent folder
        parent_dir = self.repo_root.parent

        config_args = ["gclient", "config", "--name=src", "--unmanaged", "git@github.com:youtube/cobalt.git"]
        if self.is_internal:
            print("Configuring for internal user with RBE instances...")
            config_args.append('--custom-var=rbe_instance="projects/cobalt-actions-prod/instances/default_instance"')
        else:
        ...
        if gclient_file.exists():
            ...
            if "target_os" not in content:
                print("Appending target_os and target_cpu configuration parameters to .gclient file...")
                with open(gclient_file, "a") as handle:
                    handle.write("\ntarget_os = [ 'linux', 'android' ]\n")
                    handle.write("target_cpu = ['x64', 'arm', 'arm64']\n")

    def run_gclient_sync(self):
        """Fetches repository dependencies utilizing internal toolchain syncs."""
        print("\n--- Running gclient sync ---")
        try:
            rev = subprocess.check_output(["git", "rev-parse", "@"], text=True).strip()
        except subprocess.CalledProcessError:
            rev = "HEAD"
    ...
    def disable_telemetry(self):
        ...
        if self.is_internal:
            ...
            execute_command(["build_telemetry", "opt-out"], cwd=self.repo_root.parent, check=False)
    ...
    def generate_build_files(self):
        ...
        execute_command([sys.executable, str(gn_script), "-p", "linux-x64x11", "-c", "devel"], cwd=self.repo_root)
    ...
    def setup_pre_commit(self):
        ...
        if shutil.which("pre-commit"):
            try:
                execute_command(["pre-commit", "clean"], cwd=self.repo_root, check=True)
                execute_command(["pre-commit", "install", "-t", "pre-commit", "-t", "pre-push"], cwd=self.repo_root, check=True)
            except subprocess.CalledProcessError as err:
                print(f"Warning: setup failure for pre-commit routines: {err}", file=sys.stderr)
        else:
            print("Warning: `pre-commit` executable was not detected in PATH, skipping configuration.", file=sys.stderr)

    def build_targets(self):
        """Compiles workspace sources with ninja tasks integration."""
        print("\n--- Building targets ---")
        if shutil.which("autoninja"):
            execute_command(["autoninja", "-C", "out/linux-x64x11_devel", "cobalt:gn_all"], cwd=self.repo_root)
        else:
            print("Warning: `autoninja` was not detected in PATH, skipping build compilation step.", file=sys.stderr)


def main():
    parser = argparse.ArgumentParser(description="Orchestrates checkout steps for Cobalt source checkouts.")
    parser.add_argument("--non-interactive", action="store_true", help="Run non-interactively bypassing input prompts.")
    parser.add_argument("--internal", action="store_true", default=None, help="Identifies whether setup is inside internal context.")
    parser.add_argument("--no-internal", action="store_false", dest="internal", help="Identifies setup context as external user.")
    parser.add_argument("--github-user", type=str, help="User's Github identifier for adding workspace remotes.")
    parser.add_argument("--reset-checkpoints", action="store_true", help="Reset the setup state checkpoints manifest file.")
    ...
    try:
        ...
        print(f"\n❌ Process terminated with an error: {error}", file=sys.stderr)
        sys.exit(1)
