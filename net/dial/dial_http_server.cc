...
DialHttpServer::DialHttpServer(DialService* dial_service)
    : dial_service_(dial_service),
      task_runner_(base::MessageLoop()->task_runner()) {
  DCHECK(dial_service);
  DCHECK(task_runner_);

  http_server_ =
      new HttpServer(std::make_unique<TCPServerSocket>(NULL, NULL), this);
...
void DialHttpServer::Stop() {
  DCHECK_EQ(task_runner_, base::MessageLoop()->task_runner());
  http_server_ = NULL;
}
...
void DialHttpServer::OnReceivedResponse(
  ...
  if (task_runner_ != base::MessageLoop()->task_runner()) {
  ...
  if (response) {
    http_server_->Send(conn_id,
                       ...
                       response->body, response->mime_type, response->headers);
  } else {
    http_server_->Send404(conn_id);
  }
}
