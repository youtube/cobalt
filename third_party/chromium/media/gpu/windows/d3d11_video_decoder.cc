...
void D3D11VideoDecoder::DoDecode() {
  ...
  while (true) {
    ...
    media_m96::AcceleratedVideoDecoder::DecodeResult result =
    ...
    if (result == media_m96::AcceleratedVideoDecoder::kRanOutOfStreamData) {
      current_buffer_ = nullptr;
      std::move(current_decode_cb_).Run(DecodeStatus::OK);
      break;
    } else if (result == media_m96::AcceleratedVideoDecoder::kRanOutOfSurfaces) {
    ...
    } else if (result == media_m96::AcceleratedVideoDecoder::kConfigChange) {
    ...
    } else if (result == media_m96::AcceleratedVideoDecoder::kTryAgain) {
