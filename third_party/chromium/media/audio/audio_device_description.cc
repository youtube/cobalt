...
void AudioDeviceDescription::LocalizeDeviceDescriptions(
  ...
  for (auto& description : *device_descriptions) {
    if (media_m96::AudioDeviceDescription::IsDefaultDevice(description.unique_id)) {
      description.device_name =
          media_m96::AudioDeviceDescription::GetDefaultDeviceName(
              description.device_name);
    } else if (media_m96::AudioDeviceDescription::IsCommunicationsDevice(
                   description.unique_id)) {
      description.device_name =
          media_m96::AudioDeviceDescription::GetCommunicationsDeviceName(
              description.device_name);
    }
  }
}
