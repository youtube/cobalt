...
#include "third_party/chromium/media/audio/audio_output_stream_sink.h"
...
void TestMojoMediaClient::Initialize() {
  ...
  if (!audio_manager) {
    audio_manager_ = media_m96::AudioManager::CreateForTesting(
...
std::unique_ptr<Renderer> TestMojoMediaClient::CreateRenderer(
  ...
  if (!decoder_factory_) {
    decoder_factory_ = std::make_unique<media_m96::DefaultDecoderFactory>(nullptr);
  }
