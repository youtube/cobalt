...
TEST_F(CoreTest, Basic) {
  ...
  ASSERT_NE(h, MOJO_LEGACY_HANDLE_INVALID);

  ASSERT_EQ(0u, info.GetWriteMessageCallCount());
  MojoMessageHandle message;
  ASSERT_EQ(MOJO_LEGACY_RESULT_OK, core()->CreateMessage(nullptr, &message));
  ASSERT_EQ(MOJO_LEGACY_RESULT_OK, core()->WriteMessage(h, message, nullptr));
  ASSERT_EQ(1u, info.GetWriteMessageCallCount());

  ASSERT_EQ(0u, info.GetReadMessageCallCount());
  ASSERT_EQ(MOJO_LEGACY_RESULT_OK, core()->ReadMessage(h, nullptr, &message));
  ...
  ASSERT_EQ(MOJO_LEGACY_RESULT_UNIMPLEMENTED,
...
TEST_F(CoreTest, InvalidArguments) {
  ...
  // |Close()|:
  {
    ASSERT_EQ(MOJO_LEGACY_RESULT_INVALID_ARGUMENT,
              core()->Close(MOJO_LEGACY_HANDLE_INVALID));
    ASSERT_EQ(MOJO_LEGACY_RESULT_INVALID_ARGUMENT, core()->Close(10));
    ASSERT_EQ(MOJO_LEGACY_RESULT_INVALID_ARGUMENT, core()->Close(1000000000));
    ...
    ASSERT_EQ(MOJO_LEGACY_RESULT_OK, core()->Close(h));
    ASSERT_EQ(1u, info.GetCloseCallCount());
    ASSERT_EQ(MOJO_LEGACY_RESULT_INVALID_ARGUMENT, core()->Close(h));
    ASSERT_EQ(1u, info.GetCloseCallCount());
  }
  ...
  // |num_handles|.
  ASSERT_EQ(MOJO_LEGACY_RESULT_INVALID_ARGUMENT,
            core()->WriteMessage(MOJO_LEGACY_HANDLE_INVALID, 0, nullptr));
  ...
  // |num_handles|.
  {
    ASSERT_EQ(
        ...
        core()->ReadMessage(MOJO_LEGACY_HANDLE_INVALID, nullptr, nullptr));
...
TEST_F(CoreTest, MessagePipe) {
  ...
  ASSERT_EQ(MOJO_LEGACY_RESULT_OK,
            core()->CreateMessagePipe(nullptr, &h[0], &h[1]));
  // Should get two distinct, valid handles.
  ASSERT_NE(h[0], MOJO_LEGACY_HANDLE_INVALID);
  ASSERT_NE(h[1], MOJO_LEGACY_HANDLE_INVALID);
  ASSERT_NE(h[0], h[1]);
  ...
  EXPECT_EQ(MOJO_LEGACY_RESULT_OK,
            core()->QueryHandleSignalsState(h[0], &hss[0]));
  EXPECT_EQ(MOJO_LEGACY_RESULT_OK,
            core()->QueryHandleSignalsState(h[1], &hss[1]));
  ASSERT_EQ(MOJO_LEGACY_HANDLE_SIGNAL_WRITABLE, hss[0].satisfied_signals);
  ASSERT_EQ(kAllSignals, hss[0].satisfiable_signals);
  ASSERT_EQ(MOJO_LEGACY_HANDLE_SIGNAL_WRITABLE, hss[1].satisfied_signals);
  ...
  EXPECT_EQ(MOJO_LEGACY_RESULT_OK,
            mojo_legacy::Wait(mojo_legacy::Handle(h[0]),
                              MOJO_LEGACY_HANDLE_SIGNAL_READABLE, &hss[0]));
  ...
  ASSERT_EQ(MOJO_LEGACY_RESULT_OK, core()->DestroyMessage(message));
  ...
  ASSERT_EQ(MOJO_LEGACY_RESULT_OK,
            core()->SetMessageContext(message, kTestMessageContext, nullptr,
                                      nullptr, nullptr));
  ASSERT_EQ(MOJO_LEGACY_RESULT_OK,
            core()->WriteMessage(h[0], message, nullptr));

  // Close |h[0]|.
  ASSERT_EQ(MOJO_LEGACY_RESULT_OK, core()->Close(h[0]));

  // Wait for |h[1]| to learn about the other end's closure.
  EXPECT_EQ(MOJO_LEGACY_RESULT_OK,
            mojo_legacy::Wait(mojo_legacy::Handle(h[1]),
                              MOJO_LEGACY_HANDLE_SIGNAL_PEER_CLOSED, &hss[1]));

  // Check that |h[1]| is no longer writable (and will never be).
  EXPECT_EQ(MOJO_LEGACY_HANDLE_SIGNAL_READABLE |
                MOJO_LEGACY_HANDLE_SIGNAL_PEER_CLOSED,
            hss[1].satisfied_signals);
  EXPECT_FALSE(hss[1].satisfiable_signals & MOJO_LEGACY_HANDLE_SIGNAL_WRITABLE);

  // Check that |h[1]| is still readable (for the moment).
  EXPECT_EQ(MOJO_LEGACY_HANDLE_SIGNAL_READABLE |
                MOJO_LEGACY_HANDLE_SIGNAL_PEER_CLOSED,
            hss[1].satisfied_signals);
  EXPECT_TRUE(hss[1].satisfiable_signals & MOJO_LEGACY_HANDLE_SIGNAL_READABLE);

  // Discard a message from |h[1]|.
  ASSERT_EQ(MOJO_LEGACY_RESULT_OK,
            core()->ReadMessage(h[1], nullptr, &message));
  ASSERT_EQ(MOJO_LEGACY_RESULT_OK, core()->DestroyMessage(message));
  ...
  EXPECT_EQ(MOJO_LEGACY_RESULT_OK,
            core()->QueryHandleSignalsState(h[1], &hss[1]));
  EXPECT_EQ(MOJO_LEGACY_HANDLE_SIGNAL_PEER_CLOSED, hss[1].satisfied_signals);
  EXPECT_FALSE(hss[1].satisfiable_signals & MOJO_LEGACY_HANDLE_SIGNAL_READABLE);
  ...
  ASSERT_EQ(MOJO_LEGACY_RESULT_OK, core()->Close(h[1]));
}
...
TEST_F(CoreTest, MessagePipeBasicLocalHandlePassing1) {
  ...
  EXPECT_EQ(MOJO_LEGACY_RESULT_OK,
            mojo_legacy::Wait(mojo_legacy::Handle(h_passing[1]),
                              MOJO_LEGACY_HANDLE_SIGNAL_READABLE, &hss));
  ASSERT_EQ(
  ...
  ASSERT_EQ(MOJO_LEGACY_RESULT_OK,
            core()->SetMessageContext(message, 0, nullptr, nullptr, nullptr));
  ASSERT_EQ(kTestMessageContext, context);
  ASSERT_EQ(MOJO_LEGACY_RESULT_OK, MojoDestroyMessage(message_handle));

  ASSERT_EQ(MOJO_LEGACY_RESULT_OK, core()->Close(h_passing[0]));
  ASSERT_EQ(MOJO_LEGACY_RESULT_OK, core()->Close(h_passing[1]));
}
...
TEST_F(CoreTest, DataPipe) {
  ...
  ASSERT_EQ(MOJO_LEGACY_RESULT_OK, core()->CreateDataPipe(nullptr, &ph, &ch));
  // Should get two distinct, valid handles.
  ASSERT_NE(ph, MOJO_LEGACY_HANDLE_INVALID);
  ASSERT_NE(ch, MOJO_LEGACY_HANDLE_INVALID);
  ASSERT_NE(ph, ch);

  // Producer should be never-readable, but already writable.
  hss = kEmptyMojoHandleSignalsState;
  EXPECT_EQ(MOJO_LEGACY_RESULT_OK, core()->QueryHandleSignalsState(ph, &hss));
  ...
  EXPECT_EQ(MOJO_LEGACY_RESULT_OK, core()->QueryHandleSignalsState(ch, &hss));
  ...
  EXPECT_EQ(MOJO_LEGACY_RESULT_OK,
            mojo_legacy::Wait(mojo_legacy::Handle(ch),
                              MOJO_LEGACY_HANDLE_SIGNAL_READABLE, &hss));
  ...
  read_options.flags =
      MOJO_LEGACY_READ_DATA_FLAG_NONE | MOJO_LEGACY_READ_DATA_FLAG_PEEK;
  ASSERT_EQ(MOJO_LEGACY_RESULT_OK,
  ...
  elements[1] = -1;
  num_bytes = 1u;
  read_options.flags = MOJO_LEGACY_READ_DATA_FLAG_NONE;
  ASSERT_EQ(MOJO_LEGACY_RESULT_OK,
            core()->ReadData(ch, &read_options, elements, &num_bytes));
  ASSERT_EQ('A', elements[0]);
  ASSERT_EQ(-1, elements[1]);
  ...
  elements[0] = 'X';
  num_bytes = 1u;
  ASSERT_EQ(MOJO_LEGACY_RESULT_BUSY,
            core()->WriteData(ph, elements, &num_bytes, nullptr));
  ...
  ASSERT_EQ(MOJO_LEGACY_RESULT_OK, core()->EndWriteData(ph, 3u, nullptr));
  ...
  num_bytes = 0;
  read_options.flags = MOJO_LEGACY_READ_DATA_FLAG_QUERY;
  ASSERT_EQ(MOJO_LEGACY_RESULT_OK,
  ...
  num_bytes = 0;
  read_options.flags =
      MOJO_LEGACY_READ_DATA_FLAG_QUERY | MOJO_LEGACY_READ_DATA_FLAG_PEEK;
  ASSERT_EQ(MOJO_LEGACY_RESULT_INVALID_ARGUMENT,
            core()->ReadData(ch, &read_options, nullptr, &num_bytes));
  ...
  num_bytes = 10;
  read_options.flags = MOJO_LEGACY_READ_DATA_FLAG_DISCARD |
  ...
  num_bytes = 2;
  read_options.flags =
      MOJO_LEGACY_READ_DATA_FLAG_DISCARD | MOJO_LEGACY_READ_DATA_FLAG_PEEK;
  ASSERT_EQ(MOJO_LEGACY_RESULT_INVALID_ARGUMENT,
            core()->ReadData(ch, &read_options, nullptr, &num_bytes));
  ...
  ASSERT_EQ(MOJO_LEGACY_RESULT_OK, core()->EndReadData(ch, 3u, nullptr));
  ...
  // Close the producer.
  ASSERT_EQ(MOJO_LEGACY_RESULT_OK, core()->Close(ph));

  // Wait for this to get to the consumer.
  EXPECT_EQ(MOJO_LEGACY_RESULT_OK,
            mojo_legacy::Wait(mojo_legacy::Handle(ch),
                              MOJO_LEGACY_HANDLE_SIGNAL_PEER_CLOSED, &hss));
  ...
  ASSERT_EQ(MOJO_LEGACY_RESULT_OK, core()->Close(ch));
}
