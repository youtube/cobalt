...
#include "chrome/test/fuzzing/mojo_js_fuzzing/video_capture_host_fuzzer_grammar.h"
#include "chrome/test/fuzzing/mojo_js_fuzzing/video_capture_host_fuzzer_grammar.pb.h"
...
// rule.
class VideoCaptureHostInProcessFuzzer
    : public InProcessBinaryProtoFuzzer<
          domatolpm::generated::video_capture_host_fuzzer_grammar::fuzzcase> {
 public:
  using FuzzCase =
      domatolpm::generated::video_capture_host_fuzzer_grammar::fuzzcase;
  VideoCaptureHostInProcessFuzzer();
...
REGISTER_BINARY_PROTO_IN_PROCESS_FUZZER(VideoCaptureHostInProcessFuzzer)

void VideoCaptureHostInProcessFuzzer::SetUpOnMainThread() {
  ...
  CHECK(ui_test_utils::NavigateToURL(
      browser(), embedded_https_test_server().GetURL(
                     "/VideoCaptureHost_in_process_fuzzer.html")));
...
VideoCaptureHostInProcessFuzzer::VideoCaptureHostInProcessFuzzer()
...
int VideoCaptureHostInProcessFuzzer::Fuzz(const FuzzCase& fuzz_case) {
  static Environment env;
  domatolpm::Context ctx;
  CHECK(domatolpm::video_capture_host_fuzzer_grammar::handle_fuzzer(&ctx,
                                                                    fuzz_case));
