...
#include "third_party/chromium/media/audio/alsa/alsa_util.h"
...
#include "third_party/chromium/media/audio/alsa/alsa_wrapper.h"
...
int ConfigureHwParams(media_m96::AlsaWrapper* wrapper,
                      snd_pcm_t* handle,
...
int ConfigureSwParams(media_m96::AlsaWrapper* wrapper,
                      snd_pcm_t* handle,
                      snd_pcm_uframes_t frames_per_buffer,
                      snd_pcm_uframes_t frames_per_period) {
  int error = 0;
...
int SetParams(media_m96::AlsaWrapper* wrapper,
              snd_pcm_t* handle,
              snd_pcm_format_t format,
              unsigned int channels,
              unsigned int rate,
...
static snd_pcm_t* OpenDevice(media_m96::AlsaWrapper* wrapper,
...
int CloseDevice(media_m96::AlsaWrapper* wrapper, snd_pcm_t* handle) {
...
snd_pcm_t* OpenCaptureDevice(media_m96::AlsaWrapper* wrapper,
                             const char* device_name,
                             int channels,
                             int sample_rate,
                             snd_pcm_format_t pcm_format,
...
snd_pcm_t* OpenPlaybackDevice(media_m96::AlsaWrapper* wrapper,
                              const char* device_name,
                              int channels,
                              int sample_rate,
                              snd_pcm_format_t pcm_format,
...
snd_mixer_t* OpenMixer(media_m96::AlsaWrapper* wrapper,
...
void CloseMixer(media_m96::AlsaWrapper* wrapper, snd_mixer_t* mixer,
                const std::string& device_name) {
  if (!mixer)
    return;
...
snd_mixer_elem_t* LoadCaptureMixerElement(media_m96::AlsaWrapper* wrapper,
                                          snd_mixer_t* mixer) {
  if (!mixer)
    return NULL;
