...
TEST(ExtensionTest, PlayerGetNumberOfFramesToBeRendered) {
  typedef StarboardExtensionPlayerGetNumberOfFramesToBeRenderedApi ExtensionApi;
  const char* kExtensionName =
  ...
  EXPECT_STREQ(extension_api->name, kExtensionName);
  EXPECT_EQ(extension_api->version, 1u);
  EXPECT_NE(extension_api->GetNumberOfFramesToBeRendered, nullptr);
