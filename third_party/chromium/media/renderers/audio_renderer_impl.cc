...
#include "third_party/chromium/media/filters/audio_clock.h"
...
void AudioRendererImpl::ResetDecoderDone() {
  ...
  flush_cb_ = base::BindPostTaskToCurrentDefault(std::move(flush_cb_));
  FinishFlush();
}
...
void AudioRendererImpl::Initialize(DemuxerStream* stream,
  ...
  if (speech_recognition_client_) {
    speech_recognition_client_->SetOnReadyCallback(base::BindPostTaskToCurrentDefault(
...
void AudioRendererImpl::TranscribeAudio(
    scoped_refptr<media_m96::AudioBuffer> buffer) {
#if !defined(OS_ANDROID)
  DCHECK(task_runner_->BelongsToCurrentThread());
  if (speech_recognition_client_)
