...
// The 83 supported language and regional locale variants.
const char* const kCobaltSupportedLocales[] = {
...
std::string BuildLocalesArrayJs() {
  std::string js = "[";
  for (size_t i = 0; i < std::size(kCobaltSupportedLocales); ++i) {
    if (i > 0) {
      js += ", ";
    }
    js += "'";
    js += kCobaltSupportedLocales[i];
    js += "'";
  }
  js += "]";
  return js;
...
// all 83 supported locales without throwing exceptions, using V8's root UCA
// fallback.
IN_PROC_BROWSER_TEST_F(I18nBrowserTest, CollationAcrossAllSupportedLocales) {
  SetUpPage();
...
// 3. Verify Gregorian Date/Time formatting across all 83 supported locales.
...
// 4. Verify non-Gregorian calendar support for supported locales that
...
IN_PROC_BROWSER_TEST_F(I18nBrowserTest, NonGregorianCalendarSupport) {
  ...
  const std::string script = R"(
    (() => {
      ...
      // Specific supported locales with their native non-Gregorian calendars.
...
// 5. Verify number formatting across all 83 supported locales.
...
// if present on the platform or gracefully fall back to their parent language
// (e.g., fr-SN -> fr, es-CO -> es, en-AU -> en) without throwing exceptions.
IN_PROC_BROWSER_TEST_F(I18nBrowserTest, RegionalSubLocalesGracefulFallback) {
  ...
  const std::string script = R"(
    (() => {
      ...
      for (const loc of additionalRegionalSubLocales) {
        try {
          ...
          if (!dateStr || dateStr.length === 0) {
            return `Empty date for sub-locale ${loc}`;
          }
          if (!numStr || numStr.length === 0) {
            return `Empty number for sub-locale ${loc}`;
          }
          if (coll >= 0) {
            return `Invalid collation comparison for sub-locale ${loc}`;
          }
        } catch (e) {
          return `Exception in sub-locale ${loc}: ${e.message}`;
        }
      }
