...
class ByteCounterPipe : public media_m96::cast::test::PacketPipe {
 public:
  ByteCounterPipe(ByteCounter* counter) : counter_(counter) {}
  void Send(std::unique_ptr<media_m96::cast::Packet> packet) final {
...
void SetupByteCounters(std::unique_ptr<media_m96::cast::test::PacketPipe>* pipe,
                       ByteCounter* pipe_input_counter,
                       ByteCounter* pipe_output_counter) {
  media_m96::cast::test::PacketPipe* new_pipe =
      new ByteCounterPipe(pipe_input_counter);
  new_pipe->AppendToPipe(std::move(*pipe));
  new_pipe->AppendToPipe(std::unique_ptr<media_m96::cast::test::PacketPipe>(
...
int main(int argc, char** argv) {
  ...
  std::unique_ptr<media_m96::cast::test::PacketPipe> in_pipe, out_pipe;
  std::unique_ptr<media_m96::cast::test::InterruptedPoissonProcess> ipp(
      media_m96::cast::test::DefaultInterruptedPoissonProcess());

  if (network_type == "perfect") {
    // No action needed.
  } else if (network_type == "wifi") {
    in_pipe = media_m96::cast::test::WifiNetwork();
    out_pipe = media_m96::cast::test::WifiNetwork();
  } else if (network_type == "slow") {
    in_pipe = media_m96::cast::test::SlowNetwork();
    out_pipe = media_m96::cast::test::SlowNetwork();
  } else if (network_type == "bad") {
    in_pipe = media_m96::cast::test::BadNetwork();
    out_pipe = media_m96::cast::test::BadNetwork();
  } else if (network_type == "evil") {
    in_pipe = media_m96::cast::test::EvilNetwork();
    out_pipe = media_m96::cast::test::EvilNetwork();
  ...
  std::unique_ptr<media_m96::cast::test::UDPProxy> proxy(
      media_m96::cast::test::UDPProxy::Create(local_endpoint, remote_endpoint,
                                          std::move(in_pipe),
