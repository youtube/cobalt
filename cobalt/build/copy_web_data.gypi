...

{
  ...
  'outputs': [
    '<!@pymod_do_main(starboard.build.copy_data -o <(static_contents_output_data_dir)/web/<(output_dir) --outputs <(input_files))',
  ],
  'action': [
    'python',
    '<(DEPTH)/starboard/build/copy_data.py',
    '-o', '<(static_contents_output_data_dir)/web/<(output_dir)',
    '<@(input_files)',
  ],
}
