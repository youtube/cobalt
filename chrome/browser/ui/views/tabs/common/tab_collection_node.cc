...
void TabCollectionNode::SetController(
...
std::unique_ptr<views::View> TabCollectionNode::CreateViewForNode(
  ...
  switch (node_for_view->type()) {
    case Type::TABSTRIP:
      return std::make_unique<TabStripView>(node_for_view);
    case Type::PINNED:
      return std::make_unique<PinnedTabContainerView>(node_for_view);
    case Type::UNPINNED:
      return std::make_unique<UnpinnedTabContainerView>(node_for_view);
    case Type::SPLIT:
      return std::make_unique<SplitTabView>(node_for_view);
    case Type::GROUP:
      return std::make_unique<TabGroupView>(node_for_view);
    case Type::TAB:
      return std::make_unique<TabView>(node_for_view);
  }
...
TabCollectionNode::
    ~TabCollectionNode() {  // NOLINT(modernize-use-equals-default)
