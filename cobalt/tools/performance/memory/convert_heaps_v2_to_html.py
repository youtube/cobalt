...
def build_tree_for_allocator(heaps: Dict[str, Any],
  ...
  def format_node(node: Dict[str, Any], depth: int = 1) -> Dict[str, Any]:
    children_list = [
        format_node(child, depth + 1) for child in node["children"].values()
    ]
  ...
  return format_node(root, depth=1)
...
def generate_html_report(snapshots: List[Tuple[int, Dict[str, Any]]]) -> str:
  ...
  html_template = (
      "<!DOCTYPE html>\n<html lang=\"en\">\n<head>\n"
      "  <meta charset=\"UTF-8\">\n"
      "  <title>Cobalt Native C++ Heap Allocation Visualizer</title>\n"
      "  <style>\n"
      "    :root {\n"
      ...
      "      font-size: 11px;\n"
      "    }\n"
      "    .bg-root { background: hsl(0, 0%, 20%); color: #fff; }\n"
      "    .bg-cobalt { background: hsl(215, 100%, 34%); color: #fff; }\n"
      "    .bg-blink { background: hsl(180, 100%, 32%); color: #fff; }\n"
      "    .bg-v8 { background: hsl(275, 85%, 52%); color: #fff; }\n"
      "    .bg-skia { background: hsl(25, 95%, 48%); color: #fff; }\n"
      "    .bg-media { background: hsl(50, 100%, 42%); color: #fff; }\n"
      "    .bg-net { background: hsl(325, 85%, 48%); color: #fff; }\n"
      "    .bg-base { background: hsl(205, 75%, 58%); color: #fff; }\n"
      "    .bg-stl { background: hsl(0, 0%, 50%); color: #fff; }\n"
      "    .bg-diff-red { background: hsl(350, 85%, 45%); color: #fff; }\n"
      "    .bg-diff-green { background: hsl(135, 75%, 38%); color: #fff; }\n"
      "  </style>\n</head>\n<body>\n"
      "  <div class=\"header\">\n"
      "    <h1>🔥 Cobalt Native C++ Heap Allocation Visualizer\n"
      "    (malloc + PartitionAlloc)</h1>\n"
      "    <div id=\"summary-badge\" class=\"stat-badge\">Total: 0 B</div>\n"
      "  </div>\n"
      "  <div class=\"info-panel\">\n"
      "    <b>📖 Interpretation Guide:</b>\n"
      "    <div>• <b>Vertical Hierarchy:</b> Top 3 Rows (Graphite) =\n"
      "    Container &amp; Thread Entry Points &nbsp;|&nbsp; Lower Rows =\n"
      "    Application Callstacks &nbsp;|&nbsp; Bottom Row = Terminal\n"
      "    Allocation Hook</div>\n"
      "    <div>• <b>Bar Width:</b> Proportional memory footprint\n"
      "    &nbsp;|&nbsp; <b>Click Bar:</b> Zoom into subtree</div>\n"
      "    <div class=\"legend-row\">\n"
      "      <b>Color Legend:</b>\n"
      "      <span class=\"legend-tag bg-cobalt\">Cobalt / Starboard</span>\n"
      "      <span class=\"legend-tag bg-blink\">Blink Web Engine</span>\n"
      "      <span class=\"legend-tag bg-v8\">V8 Engine</span>\n"
      "      <span class=\"legend-tag bg-skia\">Skia Graphics</span>\n"
      "      <span class=\"legend-tag bg-media\">Media</span>\n"
      "      <span class=\"legend-tag bg-net\">Network/IPC</span>\n"
      "      <span class=\"legend-tag bg-base\">Base Infrastructure</span>\n"
      "      <span class=\"legend-tag bg-stl\">Standard C++ / Other</span>\n"
      "      <span class=\"legend-tag bg-diff-red\">Diff Growth</span>\n"
      "      <span class=\"legend-tag bg-diff-green\">Diff Freed</span>\n"
      "    </div>\n"
      "  </div>\n"
      "  <div class=\"controls\">\n"
      "    <label>Snapshot:\n"
      ...
      "          '<option value=\"none\">None (Single Snapshot)</option>';\n"
      ...
      "      });\n"
      "      allocSelect.value = 'all';\n"
      "    }\n"
      "    function getNodeColor(node) {\n"
      "      const name = node.name;\n"
      "      const absDepth =\n"
      "          node.absDepth !== undefined ? node.absDepth : 99;\n"
      "      if (absDepth <= 2 || name === 'Total Heap Memory' ||\n"
      "          name === 'all' || name === 'malloc' ||\n"
      "          name === 'partition_alloc') {\n"
      "        return 'hsl(0, 0%, 20%)';\n"
      "      }\n"
      "      const s = name.toLowerCase();\n"
      "      if (s.startsWith('pc:') || s.startsWith('node_')) {\n"
      "        return 'hsl(0, 0%, 15%)';\n"
      "      }\n"
      "      if (s.includes('cobalt') || s.includes('starboard')) {\n"
      "        return 'hsl(215, 100%, 34%)';\n"
      "      }\n"
      "      if (s.includes('blink') || s.includes('wtf')) {\n"
      "        return 'hsl(180, 100%, 32%)';\n"
      "      }\n"
      "      if (s.includes('v8/') || s.includes('v8::') ||\n"
      "          s.includes('js')) {\n"
      "        return 'hsl(275, 85%, 52%)';\n"
      "      }\n"
      "      if (s.includes('skia') || s.includes('skpath') ||\n"
      "          s.includes('gpu/')) {\n"
      "        return 'hsl(25, 95%, 48%)';\n"
      "      }\n"
      "      if (s.includes('media/') || s.includes('media::') ||\n"
      "          s.includes('sbplayer')) {\n"
      "        return 'hsl(50, 100%, 42%)';\n"
      "      }\n"
      "      if (s.includes('net/') || s.includes('net::') ||\n"
      "          s.includes('mojo/') || s.includes('ipcz/') ||\n"
      "          s.includes('spdy')) {\n"
      "        return 'hsl(325, 85%, 48%)';\n"
      "      }\n"
      "      if (s.includes('base/') || s.includes('base::') ||\n"
      "          s.includes('content/') || s.includes('content::')) {\n"
      "        return 'hsl(205, 75%, 58%)';\n"
      "      }\n"
      "      return 'hsl(0, 0%, 50%)';\n"
      "    }\n"
      "    function computeDiffTree(target, base) {\n"
      "      const baseChildrenMap = Object.create(null);\n"
      ...
      "      const children = [];\n"
      "      const targetChildrenMap = Object.create(null);\n"
      ...
      "        ...Object.keys(targetChildrenMap),\n"
      "        ...Object.keys(baseChildrenMap),\n"
      "      ]);\n"
      "      allChildNames.forEach(childName => {\n"
      "        const tChild = targetChildrenMap[childName];\n"
      "        const bChild = baseChildrenMap[childName];\n"
      "        children.push(computeDiffTree(tChild, bChild));\n"
      "      });\n"
      "      children.sort((a, b) =>\n"
      ...
      "        absDepth:\n"
      "            target ? target.absDepth : (base ? base.absDepth : 99),\n"
      "        children: children,\n"
      "      };\n"
      "    }\n"
      "    function render() {\n"
      ...
      "        levels[depth].push({ node, leftPct, widthPct, depth });\n"
      "        if (node.children) {\n"
      ...
      "          } else {\n"
      "            bar.style.backgroundColor = getNodeColor(item.node);\n"
      "            bar.textContent =\n"
