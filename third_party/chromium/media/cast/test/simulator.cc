...
#include "third_party/chromium/media/base/fake_single_thread_task_runner.h"
#include "third_party/chromium/media/base/media.h"
#include "third_party/chromium/media/base/video_frame.h"
#include "third_party/chromium/media/cast/cast_config.h"
#include "third_party/chromium/media/cast/cast_environment.h"
#include "third_party/chromium/media/cast/cast_sender.h"
#include "third_party/chromium/media/cast/logging/encoding_event_subscriber.h"
...
#include "third_party/chromium/media/cast/logging/raw_event_subscriber_bundle.h"
#include "third_party/chromium/media/cast/logging/simple_event_subscriber.h"
#include "third_party/chromium/media/cast/net/cast_transport.h"
#include "third_party/chromium/media/cast/net/cast_transport_config.h"
#include "third_party/chromium/media/cast/net/cast_transport_defines.h"
#include "third_party/chromium/media/cast/net/cast_transport_impl.h"
#include "third_party/chromium/media/cast/test/fake_media_source.h"
#include "third_party/chromium/media/cast/test/loopback_transport.h"
#include "third_party/chromium/media/cast/test/proto/network_simulation_model.pb.h"
#include "third_party/chromium/media/cast/test/receiver/cast_receiver.h"
#include "third_party/chromium/media/cast/test/skewed_tick_clock.h"
#include "third_party/chromium/media/cast/test/utility/audio_utility.h"
#include "third_party/chromium/media/cast/test/utility/default_config.h"
#include "third_party/chromium/media/cast/test/utility/test_util.h"
#include "third_party/chromium/media/cast/test/utility/udp_proxy.h"
#include "third_party/chromium/media/cast/test/utility/video_utility.h"

using media_m96::cast::proto::IPPModel;
using media_m96::cast::proto::NetworkSimulationModel;
using media_m96::cast::proto::NetworkSimulationModelType;
...
class EncodedVideoFrameTracker final : public RawEventSubscriber {
 public:
  ...
  scoped_refptr<media_m96::VideoFrame> PopOldestEncodedFrame() {
    CHECK(!video_frames_.empty());
    scoped_refptr<media_m96::VideoFrame> video_frame = video_frames_.front();
    video_frames_.pop();
    return video_frame;
  }
 ...
 private:
  ...
  base::queue<scoped_refptr<media_m96::VideoFrame>> video_frames_;
};
...
void AppendYuvToFile(const base::FilePath& path,
  ...
  AppendToFile(path,
               base::make_span(frame->data(media_m96::VideoFrame::kYPlane),
                               frame->stride(media_m96::VideoFrame::kYPlane) *
                                   frame->rows(media_m96::VideoFrame::kYPlane)));
  AppendToFile(path,
               base::make_span(frame->data(media_m96::VideoFrame::kUPlane),
                               frame->stride(media_m96::VideoFrame::kUPlane) *
                                   frame->rows(media_m96::VideoFrame::kUPlane)));
  AppendToFile(path,
               base::make_span(frame->data(media_m96::VideoFrame::kVPlane),
                               frame->stride(media_m96::VideoFrame::kVPlane) *
                                   frame->rows(media_m96::VideoFrame::kVPlane)));
}
...
void GotVideoFrame(GotVideoFrameOutput* metrics_output,
                   ...
                   scoped_refptr<media_m96::VideoFrame> video_frame,
  ...
  if (video_frame_tracker) {
    scoped_refptr<media_m96::VideoFrame> src_frame =
...
void RunSimulation(const base::FilePath& source_path,
  ...
  const bool use_network_simulation =
      model.type() == media_m96::cast::proto::INTERRUPTED_POISSON_PROCESS;
  ...
  media_m96::cast::proto::LogMetadata audio_metadata, video_metadata;
  media_m96::cast::FrameEventList audio_frame_events, video_frame_events;
  media_m96::cast::PacketEventList audio_packet_events, video_packet_events;
  ...
  for (size_t i = 0; i < video_frame_events.size(); ++i) {
    const media_m96::cast::proto::AggregatedFrameEvent& event =
...
bool IsModelValid(const NetworkSimulationModel& model) {
  ...
  if (type == media_m96::cast::proto::INTERRUPTED_POISSON_PROCESS) {
    if (!model.has_ipp())
      return false;
...
NetworkSimulationModel LoadModel(const base::FilePath& model_path) {
  if (base::CommandLine::ForCurrentProcess()->HasSwitch(kNoSimulation)) {
    NetworkSimulationModel model;
    model.set_type(media_m96::cast::proto::NO_SIMULATION);
    return model;
  }
  if (model_path.empty()) {
...
int main(int argc, char** argv) {
  ...
  base::FilePath media_path = cmd->GetSwitchValuePath(media_m96::cast::kLibDir);
  if (media_path.empty()) {
  ...
  base::FilePath source_path = cmd->GetSwitchValuePath(
      media_m96::cast::kSourcePath);
  base::FilePath log_output_path = cmd->GetSwitchValuePath(
      media_m96::cast::kOutputPath);
  ...
  base::FilePath metrics_output_path = cmd->GetSwitchValuePath(
      media_m96::cast::kMetricsOutputPath);
  base::FilePath yuv_output_path = cmd->GetSwitchValuePath(
      media_m96::cast::kYuvOutputPath);
  std::string sim_id = cmd->GetSwitchValueASCII(media_m96::cast::kSimulationId);

  NetworkSimulationModel model = media_m96::cast::LoadModel(
      cmd->GetSwitchValuePath(media_m96::cast::kModelPath));
  ...
  // Run.
  media_m96::cast::RunSimulation(source_path, log_output_path, metrics_output_path,
