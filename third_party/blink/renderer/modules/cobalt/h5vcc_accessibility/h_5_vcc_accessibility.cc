...
void H5vccAccessibility::NotifyTextToSpeechChange() {
  if (HasEventListeners(event_type_names::kTexttospeechchange)) {
...
void H5vccAccessibility::EnsureReceiverIsBound() {
  ...
  // receiver_.BindNewPipeAndPassRemote(task_runner) binds the receiver to the
  // Blink main thread. And it creates a Mojo pipe with two ends: Receiver end:
  // Bound to receiver_ in the renderer process in order to to handle
  // browser-to-renderer calls. Remote end: Sent to the browser process via
  // RegisterClient. remote_->RegisterClient is a Mojo call that crosses process
  // boundaries to the browser. The browser’s H5vccAccessibilityImpl stores the
  // client’s remote endpoint, allowing it to send notifications back to the
  // renderer.
  auto task_runner =
      GetExecutionContext()->GetTaskRunner(TaskType::kMiscPlatformAPI);
  remote_->RegisterClient(receiver_.BindNewPipeAndPassRemote(task_runner));
}
