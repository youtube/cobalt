...
public abstract class FeatureParam<T> {
  ...
  private static Map<Pair<String, String>, FeatureParam<?>> sParamsForTesting =
      new HashMap<>();
  ...
  /**
   * Clears all cached param values. This will give the next test a clean slate
   * so that test values can be read through. This shouldn't be used directly by
   * individual tests other than those that exercise FeatureParam subclasses.
   */
  ...
  /**
   * Use an empty sParams map for this test instead of carrying over from and to
   * other tests in the same process (batched or Robolectric).
   */
