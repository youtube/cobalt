...
const SbGlesInterface g_sb_gles_interface = {
    ...
    // Used to override the default behavior of glBindFramebuffer since the
    // backbuffer is not 0 on darwin. Ideally, we'd use
    // --wrap=glBindFramebuffer, but that is not supported on this platform.
    &eaglBindFramebuffer, &glBindRenderbuffer, &glBindTexture, &glBlendColor,
    ...
    &glGetString, &glGetTexParameterfv, &glGetTexParameteriv, &glGetUniformfv,
