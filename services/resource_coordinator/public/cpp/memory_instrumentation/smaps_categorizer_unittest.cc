...
TEST_F(SmapsCategorizerTest, ScanSmapsFile) {
  ...
  EXPECT_CALL(delegate, OnSmapsEntry(absl::string_view("/file/1"), _))
      .WillOnce([](absl::string_view name, const SmapsMetrics& metrics) {
        EXPECT_EQ(metrics.rss_kb, 296u);
        EXPECT_EQ(metrics.pss_kb, 162u);
        EXPECT_EQ(metrics.swap_kb, 4u);
      });
  
  EXPECT_CALL(delegate, OnSmapsEntry(absl::string_view(""), _))
      .WillOnce([](absl::string_view name, const SmapsMetrics& metrics) {
        EXPECT_EQ(metrics.rss_kb, 40u);
        EXPECT_EQ(metrics.pss_kb, 32u);
      });

  EXPECT_TRUE(SmapsCategorizer::ScanSmapsFile(smaps_path, &delegate));
}
