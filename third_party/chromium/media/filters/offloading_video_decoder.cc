...
#include "third_party/chromium/media/filters/offloading_video_decoder.h"
...
void OffloadingVideoDecoder::Decode(scoped_refptr<DecoderBuffer> buffer,
  ...
  DecodeCB bound_decode_cb = base::BindPostTaskToCurrentDefault(std::move(decode_cb));
  if (!offload_task_runner_) {
...
void OffloadingVideoDecoder::Reset(base::OnceClosure reset_cb) {
  ...
  base::OnceClosure bound_reset_cb = base::BindPostTaskToCurrentDefault(std::move(reset_cb));
  if (!offload_task_runner_) {
