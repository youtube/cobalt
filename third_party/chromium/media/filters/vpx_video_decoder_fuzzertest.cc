...
void OnDecodeComplete(base::OnceClosure quit_closure, media_m96::Status status) {
  std::move(quit_closure).Run();
}

void OnInitDone(base::OnceClosure quit_closure,
                bool* success_dest,
                media_m96::Status status) {
  *success_dest = status.is_ok();
  std::move(quit_closure).Run();
}

void OnOutputComplete(scoped_refptr<media_m96::VideoFrame> frame) {}
...
extern "C" int LLVMFuzzerTestOneInput(const uint8_t* data, size_t size) {
  ...
  if (rng() & 1) {
    codec = media_m96::VideoCodec::kVP8;
    // non-Alpha VP8 decoding isn't supported by VpxVideoDecoder on Linux.
    has_alpha = true;
  } else {
    codec = media_m96::VideoCodec::kVP9;
    has_alpha = rng() & 1;
  }

  auto profile = static_cast<media_m96::VideoCodecProfile>(
      rng() % media_m96::VIDEO_CODEC_PROFILE_MAX);
  auto color_space =
      media_m96::VideoColorSpace(rng() % 256, rng() % 256, rng() % 256,
                             (rng() & 1) ? gfx::ColorSpace::RangeID::LIMITED
                                         : gfx::ColorSpace::RangeID::FULL);
  auto rotation =
      static_cast<media_m96::VideoRotation>(rng() % media_m96::VIDEO_ROTATION_MAX);
  ...
  media_m96::VideoDecoderConfig config(
      codec, profile,
      has_alpha ? media_m96::VideoDecoderConfig::AlphaMode::kHasAlpha
                : media_m96::VideoDecoderConfig::AlphaMode::kIsOpaque,
      color_space, media_m96::VideoTransformation(rotation, reflection), coded_size,
      visible_rect, natural_size, media_m96::EmptyExtraData(),
      media_m96::EncryptionScheme::kUnencrypted);

  if (!config.IsValidConfig())
    return 0;

  media_m96::VpxVideoDecoder decoder;
  ...
  {
    base::RunLoop run_loop;
    auto buffer = media_m96::DecoderBuffer::CopyFrom(data, size);
