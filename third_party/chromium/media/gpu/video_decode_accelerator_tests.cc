...
#include "third_party/chromium/media/base/test_data_util.h"
...
#include "third_party/chromium/media/gpu/test/video.h"
#include "third_party/chromium/media/gpu/test/video_frame_file_writer.h"
#include "third_party/chromium/media/gpu/test/video_frame_validator.h"
#include "third_party/chromium/media/gpu/test/video_player/frame_renderer_dummy.h"
#include "third_party/chromium/media/gpu/test/video_player/frame_renderer_thumbnail.h"
#include "third_party/chromium/media/gpu/test/video_player/video_decoder_client.h"
#include "third_party/chromium/media/gpu/test/video_player/video_player.h"
#include "third_party/chromium/media/gpu/test/video_player/video_player_test_environment.h"
#include "third_party/chromium/media/gpu/test/video_test_helpers.h"
#include "third_party/chromium/media/media_buildflags.h"
#include "testing/gtest/include/gtest/gtest.h"

#if BUILDFLAG(ENABLE_DAV1D_DECODER)
#include "third_party/chromium/media/filters/dav1d_video_decoder.h"
#elif BUILDFLAG(ENABLE_LIBGAV1_DECODER)
#include "third_party/chromium/media/filters/gav1_video_decoder.h"
#endif
...
media_m96::test::VideoPlayerTestEnvironment* g_env;
  ...
  std::unique_ptr<VideoPlayer> CreateVideoPlayer(
    ...
    if (g_env->IsValidatorEnabled()) {
      ...
      if (g_env->GetValidatorType() ==
        ...
        frame_processors.push_back(media_m96::test::MD5VideoFrameValidator::Create(
        ...
        frame_processors.push_back(media_m96::test::SSIMVideoFrameValidator::Create(
 ...
 private:
  ...
  bool CreateModelFrames(const Video* video) {
    ...
    VideoDecoder::InitCB init_cb = base::BindOnce(
        [](bool* init_success, media_m96::Status result) {
    ...
    while (!encoded_data_helper->ReachEndOfStream()) {
      ...
      media_m96::VideoDecoder::DecodeCB decode_cb = base::BindOnce(
          [](bool* decode_success, media_m96::Status status) {
    ...
    media_m96::VideoDecoder::DecodeCB flush_cb = base::BindOnce(
        [](bool* flush_success, media_m96::Status status) {
...
TEST_F(VideoDecoderTest, ResetAfterFirstConfigInfo) {
  ...
  if (g_env->Video()->Codec() != media_m96::VideoCodec::kH264 &&
      g_env->Video()->Codec() != media_m96::VideoCodec::kHEVC)
    GTEST_SKIP();
...
// the media_m96::VideoDecoder interface, so the test will be skipped if --use_vd
// is not specified.
...
int main(int argc, char** argv) {
  ...
  media_m96::test::Video::SetTestDataPath(media_m96::GetTestDataPath());
  ...
  if (cmd_line->HasSwitch("help")) {
    std::cout << media_m96::test::usage_msg << "\n" << media_m96::test::help_msg;
    return 0;
  }
  ...
  auto validator_type =
      media_m96::test::VideoPlayerTestEnvironment::ValidatorType::kMD5;
  media_m96::test::FrameOutputConfig frame_output_config;
  base::FilePath::StringType output_folder = base::FilePath::kCurrentDirectory;
  bool use_legacy = false;
  bool use_vd = false;
  bool use_vd_vda = false;
  media_m96::test::DecoderImplementation implementation =
      media_m96::test::DecoderImplementation::kVD;
  ...
  for (base::CommandLine::SwitchMap::const_iterator it = switches.begin();
    ...
    if (it->first == "validator_type") {
      if (it->second == "none") {
        validator_type =
            media_m96::test::VideoPlayerTestEnvironment::ValidatorType::kNone;
      } else if (it->second == "md5") {
        validator_type =
            media_m96::test::VideoPlayerTestEnvironment::ValidatorType::kMD5;
      } else if (it->second == "ssim") {
        validator_type =
            media_m96::test::VideoPlayerTestEnvironment::ValidatorType::kSSIM;
      } else {
      ...
      if (it->second == "all") {
        frame_output_config.output_mode = media_m96::test::FrameOutputMode::kAll;
      } else if (it->second == "corrupt") {
        frame_output_config.output_mode =
            media_m96::test::FrameOutputMode::kCorrupt;
      } else {
      ...
      if (it->second == "png") {
        frame_output_config.output_format =
            media_m96::test::VideoFrameFileWriter::OutputFormat::kPNG;
      } else if (it->second == "yuv") {
        frame_output_config.output_format =
            media_m96::test::VideoFrameFileWriter::OutputFormat::kYUV;
      } else {
      ...
      use_legacy = true;
      implementation = media_m96::test::DecoderImplementation::kVDA;
    } else if (it->first == "use_vd") {
      use_vd = true;
      implementation = media_m96::test::DecoderImplementation::kVD;
    } else if (it->first == "use_vd_vda") {
      use_vd_vda = true;
      implementation = media_m96::test::DecoderImplementation::kVDVDA;
    } else {
      std::cout << "unknown option: --" << it->first << "\n"
                << media_m96::test::usage_msg;
      return EXIT_FAILURE;
    }
  }
  ...
  media_m96::test::VideoPlayerTestEnvironment* test_environment =
      media_m96::test::VideoPlayerTestEnvironment::Create(
          video_path, video_metadata_path, validator_type, implementation,
          base::FilePath(output_folder), frame_output_config);
  if (!test_environment)
    return EXIT_FAILURE;

  media_m96::test::g_env = static_cast<media_m96::test::VideoPlayerTestEnvironment*>(
