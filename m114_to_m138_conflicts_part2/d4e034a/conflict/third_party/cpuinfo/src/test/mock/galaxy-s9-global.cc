...
TEST(PROCESSORS, l2) {
	for (uint32_t i = 0; i < cpuinfo_get_processors_count(); i++) {
		switch (i) {
			...
			case 6:
			case 7:
				ASSERT_EQ(cpuinfo_get_l2_cache(4), cpuinfo_get_processor(i)->cache.l2);
				break;
		}
	}
}
...
TEST(CORES, midr) {
	for (uint32_t i = 0; i < cpuinfo_get_cores_count(); i++) {
		switch (i) {
			...
			case 2:
			case 3:
				ASSERT_EQ(UINT32_C(0x531F0020), cpuinfo_get_core(i)->midr);
				break;
			case 4:
			case 5:
			case 6:
			case 7:
				ASSERT_EQ(UINT32_C(0x410FD051), cpuinfo_get_core(i)->midr);
				break;
		}
	}
}
...
TEST(CORES, DISABLED_frequency) {
	for (uint32_t i = 0; i < cpuinfo_get_cores_count(); i++) {
		switch (i) {
			...
			case 2:
			case 3:
				ASSERT_EQ(UINT64_C(2704000000), cpuinfo_get_core(i)->frequency);
				break;
			case 4:
			case 5:
			case 6:
			case 7:
				ASSERT_EQ(UINT64_C(1794000000), cpuinfo_get_core(i)->frequency);
				break;
		}
	}
}
...
TEST(CLUSTERS, midr) {
	for (uint32_t i = 0; i < cpuinfo_get_clusters_count(); i++) {
		switch (i) {
			case 0:
				ASSERT_EQ(UINT32_C(0x531F0020), cpuinfo_get_cluster(i)->midr);
				break;
			case 1:
				ASSERT_EQ(UINT32_C(0x410FD051), cpuinfo_get_cluster(i)->midr);
				break;
		}
	}
}
...
TEST(CLUSTERS, DISABLED_frequency) {
	for (uint32_t i = 0; i < cpuinfo_get_clusters_count(); i++) {
		switch (i) {
			case 0:
				ASSERT_EQ(UINT64_C(2704000000), cpuinfo_get_cluster(i)->frequency);
				break;
			case 1:
				ASSERT_EQ(UINT64_C(1794000000), cpuinfo_get_cluster(i)->frequency);
				break;
		}
	}
}
...
TEST(L2, count) {
	ASSERT_EQ(5, cpuinfo_get_l2_caches_count());
}
...
TEST(L3, processors) {
	for (uint32_t i = 0; i < cpuinfo_get_l3_caches_count(); i++) {
		...
		ASSERT_EQ(4, cpuinfo_get_l3_cache(i)->processor_count);
	}
}
