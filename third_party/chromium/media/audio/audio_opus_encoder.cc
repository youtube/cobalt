...
AudioParameters CreateInputParams(const AudioEncoder::Options& options) {
  ...
  AudioParameters result(media_m96::AudioParameters::AUDIO_PCM_LINEAR,
                         media_m96::CHANNEL_LAYOUT_DISCRETE, options.sample_rate,
                         frames_per_buffer);
...
void AudioOpusEncoder::Initialize(const Options& options,
  ...
  output_cb_ = base::BindPostTaskToCurrentDefault(std::move(output_callback));
...
void AudioOpusEncoder::Encode(std::unique_ptr<AudioBus> audio_bus,
  ...
  current_done_cb_ = base::BindPostTaskToCurrentDefault(std::move(done_cb));
  if (!opus_encoder_) {
