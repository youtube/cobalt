...
void InitThreadLocalSequenceKey() {
  pthread_key_create(&s_thread_local_sequence_key, NULL);
  DCHECK(s_thread_local_sequence_key);
}
...
void InitThreadLocalSequenceBoolKey() {
  pthread_key_create(&s_thread_local_sequence_set_for_thread, NULL);
...
void InitThreadLocalTaskKey() {
  pthread_key_create(&s_thread_local_task_key , NULL);
  DCHECK(s_thread_local_task_key);
}
...
void InitThreadLocalTaskBoolKey() {
  pthread_key_create(&s_thread_local_task_set_for_thread, NULL);
  DCHECK(s_thread_local_task_set_for_thread);
}
