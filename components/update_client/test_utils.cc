...
int MockGetInstallationPath(int index, char* path, int path_length) {
  ...
  if (strlen(g_mock_installation_path) > 0) {
    base::strlcpy(path, g_mock_installation_path, path_length);
...
namespace update_client {
ScopedStarboardInstallationMock::ScopedStarboardInstallationMock() {
  g_mock_installation_path[0] = '\0';
  g_mock_request_roll_forward_success = true;
}
...
void ScopedStarboardInstallationMock::SetInstallationPath(const char* path) {
  base::strlcpy(g_mock_installation_path, path, sizeof(g_mock_installation_path));
}

void ScopedStarboardInstallationMock::SetRequestRollForwardSuccess(bool success) {
