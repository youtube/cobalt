...
// Maps the clear and encrypted bytes for a single subsample. All subsamples
// within a sample must use the same encryption parameters. Clear bytes always
// precede encrypted bytes in the sample.
...
// |kSbDrmStatusSuccess|. It can be `NULL` if |status| is |kSbDrmStatusSuccess|
// or if no error message is available.
//
// The |ticket| parameter matches the ticket passed to
// `SbDrmGenerateSessionUpdateRequest()`, or is |kSbDrmTicketInvalid| if the
// update request was generated by the DRM system.
//
// The |session_id| parameter must not be `NULL` when |ticket| is
// |kSbDrmTicketInvalid|, even if an error occurred during generation. This
// allows Cobalt to locate and reject the Promise associated with the
...
// subsequent encrypted samples are ready to be decoded. |drm_system| will be
// the DRM system that SbDrmUpdateSession() was called on. |context| will be the
...
// The |error_message| parameter provides additional details if |status| is not
// |kSbDrmStatusSuccess|. It can be `NULL` if |status| is |kSbDrmStatusSuccess|
// or if no error message is available. `succeeded` is whether the session was
...
// A callback that notifies the caller of `SbDrmUpdateServerCertificate()`
// whether the server certificate update succeeded.
...
//   not be NULL.
// * |initialization_data|: The data for which the session update request
//   payload is created. Must not be NULL.
...
// Updates the session with |key| from the license server response, using the
// key system of |drm_system|. This function invokes |session_updated_callback|
// with |context| and the status. The |key| parameter must not be `NULL`.
//
// |drm_system| must not be |kSbDrmSystemInvalid|. |ticket| is the opaque ID
// that distinguishes callbacks from multiple concurrent calls to
// `SbDrmUpdateSession()`, which will be passed to |session_updated_callback|
// as-is. The caller must ensure ticket uniqueness; issuing multiple calls with
// the same ticket may result in undefined behavior.
//
// Once the session updates successfully, an `SbPlayer` or `SbDecoder`
// associated with that DRM key system can decrypt encrypted samples.
//
// The |session_updated_callback| may be invoked from the current thread before
// this function returns, or from another thread. The |session_id| parameter
// must not be `NULL`.
...
//
// |drm_system| must not be |kSbDrmSystemInvalid|. |session_id| must not be
// NULL.
...
// `SbDrmUpdateServerCertificate()`. This return value must remain constant for
// the lifetime of |drm_system|.
//
...
// call invokes. Call this function only if
// `SbDrmIsServerCertificateUpdatable()` returns `true`.
//
...
// without padding or line wrapping. The size of the encoded result is written
// to |size|. For example, on Android API level 28 or later, it should return
// the result of `MediaDrm.getPropertyByteArray("metrics")`, encoded using url
// safe base64 without padding and line wrapping. On systems using Widevine CE
// CDM with oemcrypto 16 or later, it should return the metrics retrieved via
// `Cdm::getMetrics()`, encoded using url safe base64 without padding and line
// wrapping. The returned pointer should remain valid and its content should
// remain unmodified until the next time this function is called on the
// associated |drm_system| or the |drm_system| is destroyed.
//
// When the metrics is empty on supported system, it should return a non-null
// pointer with |size| set to 0.
//
// Returns `NULL` if the underlying DRM system does not support metrics, or if
// the implementation fails to retrieve them.
//
...
// Destroys |drm_system|, implicitly removing its installed keys and
// invalidating outstanding session update requests. You must destroy any
// associated `SbPlayer` or `SbDecoder` before destroying the DRM system.
//
