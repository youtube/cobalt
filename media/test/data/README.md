...
#### duplicate_track_id.mp4
A file with duplicate track IDs (audio and video tracks sharing the same ID), which violates the ISO spec. This file was generated by a fuzzer and cannot be easily reproduced with a standard command line tool. It is used to verify that the parser correctly rejects files with duplicate track IDs across different track types.
...
#### agtm-metadata-track-frag.m4s
Fragmented mp4 samples with SMPTE ST 2094-50 timed metadata track.
...
#### populated-zero-value-padding.ogg
Generated by the fuzzing tool, this file is special in that it has a discard
padding value that is populated but set to zero. Useful for ensuring that
demuxers properly handle / do not crash in this case.
