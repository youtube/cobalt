...
#include "third_party/chromium/media/filters/frame_processor.h"
...
std::unique_ptr<media_m96::StreamParser> CreateParserForTypeAndCodecs(
    const std::string& content_type,
    const std::string& codecs,
    media_m96::MediaLog* media_log) {
  std::vector<std::string> parsed_codec_ids;
  media_m96::SplitCodecs(codecs, &parsed_codec_ids);
  return media_m96::StreamParserFactory::Create(content_type, parsed_codec_ids,
                                            media_log);
}
...
void ChunkDemuxerStream::CompletePendingReadIfPossible_Locked() {
  ...
  std::move(read_cb_).Run(status, std::move(buffers));
}
#else // defined(STARBOARD)
void ChunkDemuxerStream::CompletePendingReadIfPossible_Locked() {
  lock_.AssertAcquired();
...
ChunkDemuxer::Status ChunkDemuxer::AddId(
  ...
  std::unique_ptr<media_m96::StreamParser> stream_parser(
      media_m96::StreamParserFactory::Create(std::move(audio_config)));
...
ChunkDemuxer::Status ChunkDemuxer::AddId(
  ...
  std::unique_ptr<media_m96::StreamParser> stream_parser(
      media_m96::StreamParserFactory::Create(std::move(video_config)));
  DCHECK(stream_parser);
...
void ChunkDemuxer::SetDuration(double duration) {
  ...
  if (duration == std::numeric_limits<double>::infinity()) {
    duration_td = media_m96::kInfiniteDuration;
