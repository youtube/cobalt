...
struct SbParamWrapper {
  ...
  // std::string uses its own specialized get function since it must use a
  // std::vector to store the overridden strings.
  std::string GetStringValue(
      std::vector<std::string>& overridden_strings) const;
};
...
std::string SbParamWrapper::GetStringValue(
    std::vector<std::string>& overridden_strings) const {
  SB_DCHECK(type == SbFeatureParamTypeString);
  std::string overridden_value =
      base::GetFieldTrialParamValueByFeature(*feature, param_name);
  if (!overridden_value.empty()) {
    overridden_strings.push_back(std::move(overridden_value));
    return overridden_strings.back();
  } else {
    return std::get<std::string>(default_value);
  }
}
    ...
    switch (wrapper.type) {
      ...
      case SbFeatureParamTypeString: {
        param.value.string_value =
            wrapper.GetStringValue(overridden_strings).c_str();
        break;
      }
