...
SUBTREE_SCRATCH_DIR = '/tmp/cobalt/subtree_scratch'  # Temporary directory to stage repo clones
...
def main():
    ...
    rebase_parser.add_argument(
        ...
        type=str,
        default='',
        help='Branch used to help gather local changes.')
    rebase_parser.add_argument('--scratch-dir',
                               type=str,
                               default=SUBTREE_SCRATCH_DIR,
                               help='Temporary directory to stage repo data.')
    ...
    if args.command == 'linearize':
        ...
        module_diffs = {}
        if args.deps_file:
            print('Checking out rebase_branch branch: %s...' %
                  args.rebase_branch)
            ...
            if args.reference_branch:
                print(
                    'Checking out reference branch for subtree diffs: %s...' %
                    args.reference_branch)
                try:
                    repo.git.checkout(args.reference_branch)
                    for module in modules_to_update.keys():
                        patch_file = os.path.join(args.scratch_dir, module,
                                                  'diff.patch')
                        success = convert_dep_to_squashed.eval_local_module_diffs(
                            repo, module, patch_file)
                        if success:
                            module_diffs[module] = patch_file
                            print("Created reference diff file at %s " % patch_file)
                finally:
                    repo.git.checkout(args.rebase_branch)
            return
        ...
        for module in modules_to_update.keys():
            ...
            print('Attempting to update the subtree %s.' % module_dir)
            convert_dep_to_squashed.squash_subtree(repo, module, git_source, git_rev)
