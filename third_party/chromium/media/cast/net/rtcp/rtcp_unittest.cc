...
media_m96::cast::RtcpTimeData CreateRtcpTimeData(base::TimeTicks now) {
  media_m96::cast::RtcpTimeData ret;
  ret.timestamp = now;
  media_m96::cast::ConvertTimeTicksToNtp(now, &ret.ntp_seconds, &ret.ntp_fraction);
  return ret;
}
...
TEST_F(RtcpTest, ReportReceiverEvents) {
  const RtpTimeTicks kRtpTimeStamp =
      media_m96::cast::RtpTimeTicks().Expand(UINT32_C(100));
