...
echo Linux
gcloud storage cp $GS_PATH/Linux_x64/clang-format-$CLANG_REV.tar.xz /tmp
tar xf /tmp/clang-format-$CLANG_REV.tgz -C buildtools/linux64 --strip-component=1 bin/clang-format

echo Win
gcloud storage cp $GS_PATH/Win/clang-format-$CLANG_REV.tar.xz /tmp
tar xf /tmp/clang-format-$CLANG_REV.tgz -C buildtools/win --strip-component=1 bin/clang-format.exe

echo 'Mac x64'
gcloud storage cp $GS_PATH/Mac/clang-format-$CLANG_REV.tar.xz /tmp
...
echo 'Mac arm64'
gcloud storage cp $GS_PATH/Mac_arm64/clang-format-$CLANG_REV.tgz /tmp
tar xf /tmp/clang-format-$CLANG_REV.tgz -C buildtools/mac --strip-component=1 bin/clang-format
