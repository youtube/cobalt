...
class URLFetcher {
 public:
  ...
  // multiple times if the request is retried.
...
#if defined(STARBOARD)
  // Get the response as a string when the response is expected to be large and
  ...
  virtual bool GetResponseAsLargeString(
      std::string* out_response_string) const = 0;
#endif
