...
void* UncheckedAlloc(size_t size) {
  ...
  auto ptr = chain_head->alloc_unchecked_function(chain_head, size, nullptr);
  DumpOperation('a', ptr, size);
  return ptr;
}

void UncheckedFree(void* ptr) {
  DumpOperation('f', ptr, -1);
  const AllocatorDispatch* const chain_head = GetChainHead();
...
#endif
  const allocator_shim::AllocatorDispatch* const chain_head = GetChainHead();
  void* ptr = chain_head->alloc_unchecked_function(chain_head, size, context);
  DumpOperation('a', ptr, size);
  return ptr;
}
