...
The Cobalt project uses git branches for two main purposes:
  ...
  2. To isolate `main` branch developers from risky or disruptive work.
...
continue on the `main` branch while stabilizing and finalizing a set of sources
for release.
...
### Release Timeline
  ...
  3. As bugs are discovered and feedback received from partners, fixes will be
     cherry-picked into the release branch. These cherry-picks should
  ...
  5. Once a commit on the branch is deemed to be feature-complete and stable, it
     ...
     An example stable branch name is `27.lts.stable`.

     Some example release tags are:
      - `27.lts.1` (Initial Release Candidate)
      - `27.lts.10` (First Stable Release)
      - `27.lts.20` (Second Stable Release)
...
Older release branches (Cobalt 25 and below) followed a different naming scheme
that included a minor version and a `+` suffix:
`[[Major Version](versioning.md#major-version)].[[Purpose](versioning.md#purpose)].[[Minor Version](versioning.md#minor-version)]+`
(e.g., `24.lts.1+` or `25.lts.1+`).

The `+` symbol indicated that the branch may eventually contain multiple release
updates, all greater than or equal to the specified minor version. Should another
release branch be cut from the main branch with a pre-existing (major version, purpose)
pair, the new branch would have a minor version equivalent to the most recently
released minor version, plus one. This complexity has been removed starting with
Cobalt 27 in favor of a simpler `[Major].[Purpose]` branch structure.
