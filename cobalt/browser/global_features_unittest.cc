...
class GlobalFeaturesTest : public testing::Test {
 ...
 protected:
  ...
  void SetUp() override {
    ...
    cache_dir_ = temp_dir_.GetPath().AppendASCII("cache");
    base::ScopedPathOverride cache_override(base::DIR_CACHE, cache_dir_, true,
                                            true);
  }

  void TearDown() override {
    EXPECT_TRUE(base::DeletePathRecursively(cache_dir_));
  }
};
...
TEST_F(GlobalFeaturesTest, ActiveExperimentIdsUnchangedAfterInitialization) {
  ...
  base::FilePath config_file =
      cache_dir_.Append(FILE_PATH_LITERAL("Experiment Config"));
