...
TEST(PosixSysStatvfsTest, UsageChanges) {
  ...
  ASSERT_GT(buf_before.f_bavail, 0);
  ASSERT_GT(buf_before.f_ffree, 0);

  char temp_file_path[kSbFileMaxPath];
  int result = snprintf(temp_file_path, kSbFileMaxPath, "%s/statvfs_temp.tmp",
                        cache_path);
  ASSERT_GT(result, 0);
  ASSERT_LT(result, kSbFileMaxPath);

  const size_t write_size = buf_before.f_bsize > 0 ? buf_before.f_bsize : 4096;
  ...
  struct statvfs buf_after;
  errno = 0;
  ASSERT_EQ(statvfs(cache_path, &buf_after), 0)
      << "statvfs (after) failed: " << strerror(errno);

  EXPECT_LT(buf_after.f_ffree, buf_before.f_ffree);
  EXPECT_LT(buf_after.f_bavail, buf_before.f_bavail);
