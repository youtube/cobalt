...
#include "third_party/chromium/media/filters/in_memory_url_protocol.h"
#include "third_party/chromium/media/filters/vpx_video_decoder.h"
#include "third_party/chromium/media/gpu/macros.h"
#include "third_party/chromium/media/gpu/test/video_frame_helpers.h"
...
void Video::DecodeTask(const std::vector<uint8_t> data,
  ...
  *success = false;
  ...
  media_m96::Status init_result;
  VpxVideoDecoder decoder(
      media_m96::OffloadableVideoDecoder::OffloadState::kOffloaded);
  media_m96::VideoDecoder::InitCB init_cb =
      base::BindOnce([](media_m96::Status* save_to,
                        media_m96::Status save_from) { *save_to = save_from; },
                     &init_result);
  ...
  while (av_read_frame(glue.format_context(), &packet) >= 0 &&
    ...
    if (packet.stream_index == stream_index) {
      media_m96::VideoDecoder::DecodeCB decode_cb = base::BindOnce(
          [](bool* success, media_m96::Status status) {
