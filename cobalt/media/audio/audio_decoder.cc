...
bool DecodeAudioFileData(blink::WebAudioBus* destination_bus,
  ...
  if (bus->channels() == destination_bus->NumberOfChannels() &&
      bus->frames() == destination_bus->length()) {
    size_t bytes_per_channel = bus->frames() * sizeof(float);
    for (int channel_index = 0; channel_index < bus->channels();
         ++channel_index) {
      const float* source_data = bus->channel(channel_index);
