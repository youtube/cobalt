...
class MediaCapabilitiesCacheTest : public ::testing::Test {
 protected:
  void SetUp() override {
    ...
    ON_CALL(*mock_media_capabilities_provider_, IsWidevineSupported())
        .WillByDefault(Return(true));
    ON_CALL(*mock_media_capabilities_provider_, IsCbcsSchemeSupported())
        .WillByDefault(Return(true));
    ON_CALL(*mock_media_capabilities_provider_, GetSupportedHdrTypes())
        .WillByDefault(
            Return(std::set<SbMediaTransferId>{kSbMediaTransferIdSmpteSt2084}));
    ON_CALL(*mock_media_capabilities_provider_,
            GetIsPassthroughSupported(::testing::_))
        .WillByDefault(Return(false));
    ON_CALL(*mock_media_capabilities_provider_, GetAudioConfiguration(_, _))
        .WillByDefault(Return(false));
    ON_CALL(*mock_media_capabilities_provider_, GetCodecCapabilities(_, _))
...
TEST_F(MediaCapabilitiesCacheTest, IsWidevineSupported_EnabledCache) {
  EXPECT_CALL(*mock_media_capabilities_provider_, IsWidevineSupported())
      .Times(1)
...
TEST_F(MediaCapabilitiesCacheTest, IsCbcsSchemeSupported_EnabledCache) {
  EXPECT_CALL(*mock_media_capabilities_provider_, IsCbcsSchemeSupported())
      .Times(1)
      .WillOnce(Return(true));
...
TEST_F(MediaCapabilitiesCacheTest, ClearCache_IsWidevineSupported) {
  EXPECT_CALL(*mock_media_capabilities_provider_, IsWidevineSupported())
      .Times(2)
      .WillRepeatedly(Return(true));
  EXPECT_TRUE(cache_->IsWidevineSupported());
...
TEST_F(MediaCapabilitiesCacheTest, ClearCache_IsCbcsSchemeSupported) {
  EXPECT_CALL(*mock_media_capabilities_provider_, IsCbcsSchemeSupported())
      .Times(2)
      .WillRepeatedly(Return(true));
...
TEST_F(MediaCapabilitiesCacheTest,
       ClearCache_HdrTransferCharacteristicsSupported) {
  EXPECT_CALL(*mock_media_capabilities_provider_, GetSupportedHdrTypes())
      .Times(2)
      .WillRepeatedly(
          Return(std::set<SbMediaTransferId>{kSbMediaTransferIdSmpteSt2084}));
...
// The method IsPassthroughSupported uses to store its value is different than
// the rest of the other values. Even if we call ClearCache, it should still
// retain the values that were already there.
TEST_F(MediaCapabilitiesCacheTest, ClearCache_IsPassthroughSupported) {
  EXPECT_CALL(*mock_media_capabilities_provider_,
              GetIsPassthroughSupported(kSbMediaAudioCodecAc3))
      .Times(1)
      .WillOnce(Return(true));
...
TEST_F(MediaCapabilitiesCacheTest, ClearCache_GetAudioConfiguration) {
  EXPECT_CALL(*mock_media_capabilities_provider_,
...
TEST_F(MediaCapabilitiesCacheTest, ClearCache_HasAudioDecoderFor) {
