...
#include "third_party/chromium/media/audio/audio_input_device.h"
...
ACTION_P(ReportStateChange, device) {
  static_cast<AudioInputIPCDelegate*>(device)->OnError(
      media_m96::AudioCapturerSource::ErrorCode::kUnknown);
}
...
TEST_P(AudioInputDeviceTest, CreateStream) {
  ...
  const uint32_t memory_size =
      media_m96::ComputeAudioInputBufferSize(params, kMemorySegmentCount);
