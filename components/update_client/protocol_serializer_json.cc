...
std::string ProtocolSerializerJSON::Serialize(
  ...
  if (base::IsStringUTF8AllowingNoncharacters(request.os.arch)) {
    os_node.Set("arch", request.os.arch);
  }
  if (!request.os.version.empty() &&
      base::IsStringUTF8AllowingNoncharacters(request.os.version)) {
    os_node.Set("version", request.os.version);
  }
  if (!request.os.service_pack.empty() &&
      base::IsStringUTF8AllowingNoncharacters(request.os.service_pack)) {
