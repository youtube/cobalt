...
TEST(PosixSocketSendmsgTest, SunnyDayTwoIovecs) {
  int sockets[2];
  ASSERT_EQ(0, socketpair(AF_UNIX, SOCK_STREAM, 0, sockets));
  ...
  struct iovec iov[2];
  iov[0].iov_base = const_cast<char*>(msg1);
  iov[0].iov_len = strlen(msg1);
  iov[1].iov_base = const_cast<char*>(msg2);
...
TEST(PosixSocketSendmsgTest, SunnyDayWithMultipleAncillaryData) {
  int sockets[2];
  ASSERT_EQ(0, socketpair(AF_UNIX, SOCK_STREAM, 0, sockets));
  ...
  {
    const char* message = "Here are two file descriptors!";
    struct iovec iov[1];
    iov[0].iov_base = const_cast<char*>(message);
    iov[0].iov_len = strlen(message);
    ...
    EXPECT_TRUE(close(pipe1_fds[0]) == 0);
    EXPECT_TRUE(close(pipe2_fds[0]) == 0);
  }
  ...
  EXPECT_TRUE(close(sockets[0]) == 0);
  EXPECT_TRUE(close(sockets[1]) == 0);
  EXPECT_TRUE(close(pipe1_fds[1]) == 0);
  EXPECT_TRUE(close(pipe2_fds[1]) == 0);
}
