...
#ifndef CHROMEOS_ASH_COMPONENTS_MOJO_PROXY_MOJO_CORE_PUBLIC_C_SYSTEM_MACROS_H_
#define CHROMEOS_ASH_COMPONENTS_MOJO_PROXY_MOJO_CORE_PUBLIC_C_SYSTEM_MACROS_H_
...
// Use like:
//   MOJO_LEGACY_STATIC_ASSERT(sizeof(struct Foo) == 12, "Foo has invalid
//   size");
#define MOJO_LEGACY_STATIC_ASSERT(expr, msg) static_assert(expr, msg)
...
// clang-format off
#define MOJO_LEGACY_POINTER_FIELD(type, name) \
  type name;                           \
  uint32_t : (sizeof(void*) == 4 ? 32 : 0)
// clang-format on

// Like the C++11 |alignof| operator.
#define MOJO_LEGACY_ALIGNOF(type) alignof(type)
...
#define MOJO_LEGACY_IS_STRUCT_FIELD_PRESENT(struct_pointer, field) \
...
// Use like:
//   struct MOJO_LEGACY_ALIGNAS(8) Foo { ... };
// Unlike the C++11 |alignas()|, |alignment| must be an integer. It may not be a
// type, nor can it be an expression like |MOJO_LEGACY_ALIGNOF(type)| (due to
// the non-C++11 MSVS version). This can't use alignas() in C11 mode because
// unlike the C++11 version the C11 version can't be used on struct
// declarations.
#if defined(__cplusplus)
#define MOJO_LEGACY_ALIGNAS(alignment) alignas(alignment)
#elif defined(__GNUC__)
#define MOJO_LEGACY_ALIGNAS(alignment) __attribute__((aligned(alignment)))
#elif defined(_MSC_VER)
#define MOJO_LEGACY_ALIGNAS(alignment) __declspec(align(alignment))
#else
#error "Please define MOJO_LEGACY_ALIGNAS() for your compiler."
#endif

#endif  // CHROMEOS_ASH_COMPONENTS_MOJO_PROXY_MOJO_CORE_PUBLIC_C_SYSTEM_MACROS_H_
