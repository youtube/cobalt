...
StarboardRenderer::~StarboardRenderer() {
  ...
  // that they won't access `player_bridge_` when its being destroyed.
  ...
  LOG(INFO) << "StarboardRenderer destroyed.";
}
...
void StarboardRenderer::Flush(base::OnceClosure flush_cb) {
  ...
  // OnDemuxerStreamRead(), we will wait until they are completed.
...
void StarboardRenderer::StartPlayingFrom(base::TimeDelta time) {
  ...
  LOG_IF(WARNING, time < 0) << "Potentially invalid start time " << time;
  ...
  // We cannot start playback before SbPlayerBridge is initialized.
...
base::TimeDelta StarboardRenderer::GetMediaTime() {
  ...
  if (statistics.video_frames_decoded > 0 ||
    ...
    //                    whether we should report more statistics.
