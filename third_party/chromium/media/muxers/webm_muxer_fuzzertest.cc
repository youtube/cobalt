...
static const media_m96::VideoCodec kSupportedVideoCodecs[] = {
    media_m96::VideoCodec::kVP8, media_m96::VideoCodec::kVP9, media_m96::VideoCodec::kH264};
static const media_m96::AudioCodec kSupportedAudioCodecs[] = {
    media_m96::AudioCodec::kOpus, media_m96::AudioCodec::kPCM};
...
extern "C" int LLVMFuzzerTestOneInput(const uint8_t* data, size_t size) {
  ...
  for (const auto& input_type : kVideoAudioInputTypes) {
    ...
    media_m96::WebmMuxer muxer(audio_codec, input_type.has_video,
                           input_type.has_audio,
                           std::make_unique<media_m96::LiveWebmMuxerDelegate>(
    ...
    do {
      if (input_type.has_video) {
        ...
        const auto video_frame =
            media_m96::VideoFrame::CreateBlackFrame(visible_rect);
        const auto is_key_frame = rng() % 2;
        const auto has_alpha_frame = rng() % 4;
        auto parameters = media_m96::WebmMuxer::VideoParameters(video_frame);
      ...
      if (input_type.has_audio) {
        const media_m96::ChannelLayout layout = rng() % 2
                                                ? media_m96::CHANNEL_LAYOUT_STEREO
                                                : media_m96::CHANNEL_LAYOUT_MONO;
        const int sample_rate =
            kSampleRatesInKHz[rng() % std::size(kSampleRatesInKHz)];

        const media_m96::AudioParameters params(
            media_m96::AudioParameters::AUDIO_PCM_LOW_LATENCY, layout, sample_rate,
            60 * sample_rate);
