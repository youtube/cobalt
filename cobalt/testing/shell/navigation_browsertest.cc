...
//   blink::LocalFrameToken initial_rfh_frame_token =
//   initial_rfh->GetFrameToken(); int initial_rfh_process_id =
//   initial_rfh->GetProcess()->GetID();
...
//     if (CanSameSiteMainFrameNavigationsChangeRenderFrameHosts()) {
//       // If same-site ProactivelySwapBrowsingInstance or main-frame
//       // RenderDocument is enabled, the navigation will result in a new RFH,
//       so
...
//       EXPECT_EQ(initial_rfh_process_id,
//       observer.last_initiator_process_id());
//     } else {
...
//   initial_rfh->GetFrameToken(); int initiator_process_id =
//   initial_rfh->GetProcess()->GetID();
...
//     // No response on an unreachable URL, so the ReceivedResponse event
//     should
//     // not have increased.
//     EXPECT_EQ(1u, test_ukm_recorder()
//                       .GetEntriesByName("Navigation.ReceivedResponse")
...
// // Ensure that content initiated navigations to view-sources URLs are
// blocked. IN_PROC_BROWSER_TEST_F(NavigationBrowserTest,
...
// // Ensure that content initiated navigations to googlechrome: URLs are
// blocked. IN_PROC_BROWSER_TEST_F(NavigationBrowserTest,
...
// // Ensure that closing a page by running its beforeunload handler doesn't
// hang
...
//       EXPECT_EQ(GURL(),
//       main_frame()->navigation_request()->GetReferrer().url); break;
...
//   &file_path)); ASSERT_TRUE(base::WriteFile(file_path, file_content));
...
//   // Use EvalJs and respond back to the browser process before doing the
//   actual
//   // submission. This will ensure that the process termination is guaranteed
//   to
...
//       EvalJs(
//           shell(),
//           "setTimeout(() => document.getElementById('file-form').submit(),
//           0);" "true;"));
...
//   // The expectation is that the blocked URL is present in the
//   NavigationEntry,
...
//                   net::IsolationInfo::RequestType::kMainFrame, origin,
...
// // Test that the initiator is set to the starting page when a renderer
// initiated
...
//   GURL starting_page(embedded_test_server()->GetURL("a.com",
//   "/title1.html")); url::Origin starting_page_origin; starting_page_origin =
//   starting_page_origin.Create(starting_page);
...
//   // Perform the actual navigation.
//   EXPECT_TRUE(ExecJs(main_frame()->child_at(0)->current_frame_host(),
//   script));
...
// // selected by Id, is navigated by Javascript from some starting page to
// another
// // page.
// IN_PROC_BROWSER_TEST_F(NavigationBrowserTest,
...
//     EXPECT_TRUE(
//         ExecJs(shell(), base::StringPrintf(kReplacePortNumber,
//         port_number)));
...
// // Ensure that renderer initiated navigations which have the opener
// suppressed
// // work.
// IN_PROC_BROWSER_TEST_F(NavigationBrowserTest,
...
//   blink::LocalFrameToken initiator_frame_token =
//   subframe_rfh->GetFrameToken(); int initiator_process_id =
//   subframe_rfh->GetProcess()->GetID();
...
//   EXPECT_TRUE(
//       ExecJs(shell(), JsReplace("document.getElementById('form').action =
//       $1",
//                                 form_action_url)));
...
//   EXPECT_TRUE(ExecJs(shell(), "window.open('about:blank',
//   'target_frame');")); WebContents* popup_contents =
...
//       form_contents, embedded_test_server()->GetURL("a.com",
//       "/title2.html"));
...
//   // When the navigation starts for the popup, ensure that the original page
//   has
...
// // Data URLs can have a reference fragment like any other URLs. This test
// makes
...
//     // The iframe navigation is paused. In the meantime, a pushState
//     navigation
//     // begins and ends.
...
//   // A response for the XHR request. It will be delayed until the end of all
//   the
//   // navigations.
...
//       "xhr.onabort = () =>
//       window.domAutomationController.send('xhr.onabort');" "xhr.onerror = ()
//       => window.domAutomationController.send('xhr.onerror');" "xhr.onload =
//       () => window.domAutomationController.send('xhr.onload');"
//       "xhr.send();");
...
// // Note that original issue for the bug is not applicable anymore, because
// there
...
//   // 3) In the meantime the iframe navigates elsewhere. It causes the
//   previous
//   // DocumentLoader to be replaced by the new one. Removing it may
//   // trigger the 'load' event and delete the iframe.
//   EXPECT_TRUE(
//       ExecJs(shell(), "document.querySelector('iframe').src =
//       '/title1.html'"));

//   // 4) Finish the original request.
//   iframe_response.Done();

//   // Wait for the iframe to be deleted and check the renderer process is
//   still
//   // alive.
//   int iframe_count = 1;
...
//             shell(),
//             "var iframe_count =
//             document.getElementsByTagName('iframe').length;" "iframe_count;")
//             .ExtractInt();
//   }
// }
...
// //    yield CPU to other tasks. That way the response body's data are not
// fully
// //    read when URLLoaderClient::OnComplete(..) is received.
// // 2) A script makes the document navigates elsewhere while it is still
// loading.
// //    It cancels the parser of the current document. Due to a bug, the
// document
...
//   // tasks. The goal is to ensure the response body datapipe is not fully
//   read
...
// // Renderer initiated back/forward navigation in beforeunload should not
// prevent
...
// // Ensure the renderer process doesn't send too many IPC to the browser
// process
...
// // TODO(http://crbug.com/632514): This test currently expects opener
// downloads
...
//   EXPECT_TRUE(ExecJs(popup, base::StringPrintf("window.opener.location
//   ='%s'",
//                                                data_url.spec().c_str())));
//   ASSERT_TRUE(manager.WaitForNavigationFinished());
...
// // The navigation request must be canceled and it must also cancel the
// network
...
//   // Insert enough iframes so that if sockets are not properly released:
//   there
//   // will not be enough of them to complete all navigations. As of today,
//   only 6
//   // sockets can be used simultaneously. So using 7 iframes is enough. This
//   test
//   // uses 33 as a margin.
//   EXPECT_TRUE(ExecJs(shell(), R"(
//     for(let i = 0; i<33; ++i) {
...
//                                                         "", true);
//   net::test_server::ControllableHttpResponse
//   response_2(embedded_test_server(),
//                                                         "", true);
//   ASSERT_TRUE(embedded_test_server()->Start());
...
//  private:
//   // See |NavigationCorsExemptBrowserTest::last_cors_header_value_| for
//   details. raw_ptr<std::string> last_cors_header_value_;
// };

// // ContentBrowserClient responsible for creating CorsInjectingUrlLoader.
// class CorsContentBrowserClient : public
...
//   response.WaitForRequest();
//   EXPECT_EQ(header_value,
//   response.http_request()->headers.at(kCorsHeaderName));
...
//   EXPECT_EQ(
//       url,
//       data.new_web_contents->GetPrimaryMainFrame()->GetLastCommittedURL());
// }
...
//                                      "about:srcdoc#foo"};
//   // 1. Main frame navigations to about:srcdoc and its variations are
//   blocked. for (const char* url : about_srcdoc_urls) {
...
// // browser process. It means RenderFrameHostImpl::BeginNavigation() is
// called. IN_PROC_BROWSER_TEST_F(NavigationBrowserTest,
// AboutSrcDocUsesBeginNavigation) {
//   GURL url(embedded_test_server()->GetURL("/title1.html"));
//   EXPECT_TRUE(NavigateToURL(shell(), url));
...
//   //  2) Cross-document navigation to about:srcdoc?1.
//   {
//     TestNavigationManager commit_waiter(web_contents(),
//     GURL("about:srcdoc?1")); EXPECT_TRUE(ExecJs(shell(), R"(
//       let subwindow = document.querySelector('iframe').contentWindow;
...
//   {
//     TestNavigationManager commit_waiter(web_contents(),
//     GURL("about:srcdoc?2")); EXPECT_TRUE(ExecJs(shell(), R"(
//       let subwindow = document.querySelector('iframe').contentWindow;
...
// // Make sure embedders are notified about visible URL changes in this
// scenario:
// // 1. Navigate to A.
// // 2. Navigate to B.
...
//   // This test expects a new request to be made when navigating back, which
//   is
//   // not happening with back-forward cache enabled.
//   // See BackForwardCacheBrowserTest.RestoreWhilePendingCommit which covers
//   the
...
//   // Should have the same page UKM ID in navigation as page post commit, and
//   as
//   // the top-level frame.
//   EXPECT_EQ(current_frame_host()->GetPageUkmSourceId(), nav_request_id);
...
// // Test that NavigationRequest::GetNextPageUkmSourceId returns the eventual
// // value of RenderFrameHost::GetPageUkmSourceId() --- same document
// navigation. IN_PROC_BROWSER_TEST_F(NavigationBrowserTest,
...
//   EXPECT_TRUE(
//       ExecJs(main_frame(), JsReplace("location.href = $1",
//       kFragment.spec())));
//   EXPECT_TRUE(WaitForLoadStop(web_contents()));
...
//   EXPECT_TRUE(ExecJs(sub_document_1, JsReplace("location.href = $1",
//   url_b))); EXPECT_TRUE(WaitForLoadStop(web_contents()));
...
//   // 11. Set cookie in the child document. It should be reflected on its
//   parent. EXPECT_TRUE(ExecJs(sub_document_3, "document.cookie = 'f=0';"));
...
//   EXPECT_TRUE(
//       ExecJs(shell(), JsReplace("let iframe =
...
// // requesting an history navigation, a new navigation is requested from the
// main
...
//   // 8. Ask the top-level, a.com frame to navigate the subframe to
//   about:blank. EXPECT_TRUE(ExecJs(shell(), R"(
//     document.querySelector('iframe').src = "about:blank";
//   )"));
//   EXPECT_TRUE(WaitForLoadStop(web_contents()));
//   RenderFrameHostImpl* sub_document_3 =
...
//   // 3. Set cookie in the main document. No cookies are sent when requested
//   from
//   // the data-URL.
//   EXPECT_TRUE(ExecJs(main_document, "document.cookie = 'a=0;SameSite=Lax'"));
//   EXPECT_TRUE(ExecJs(main_document, "document.cookie =
//   'b=0;SameSite=Strict'")); GURL url_response_1 =
//   https_server()->GetURL("a.com", "/response_1");
//   ExecuteScriptAsync(sub_document_1, JsReplace("fetch($1)", url_response_1));
//   response_1.WaitForRequest();
...
// // Some navigations are not allowed, such as when they fail the content
// security
// // policy, or for trying to load about:srcdoc in the main frame. These result
// in
// // us redirecting the navigation to an error page via
// // RenderFrameHostImpl::FailedNavigation().
// // Repeating the request with a different URL fragment results in attempting
// a
// // same-document navigation, but error pages do not support such navigations.
// In
// // this case treat each failed navigation request as a separate load, with
// the
// // resulting navigation being performed as a cross-document navigation. This
// is
...
// // This navigation is allowed by the browser, but the network will not be
// able
...
// // make the full attempt again, in case the network request succeeds this
// time. IN_PROC_BROWSER_TEST_F(NavigationBrowserTest,
...
//   // The navigation is okay from the browser's perspective, so
//   NavigateToURL()
//   // will return true. But the network request ultimately fails, so the
//   request
...
// // This navigation is allowed by the browser, and the request to the server
// is
// // successful, but it returns 404 error headers, and (optionally) an error
// page.
// // When another request is made for the same page but with a different
// fragment,
...
// // been loaded instead. A same-document navigation in the renderer-loaded
// error
// // page should be performed as a cross-document navigation in order to
// attempt
// // to reload the page.
// IN_PROC_BROWSER_TEST_F(NavigationBrowserTest,
//                        SameDocumentNavigationOn404ErrorPage) {
//   // This case is a non-empty 404 page. It makes different choices about
//   where
...
//     // This is another navigation to the non-existent URL, but with a
//     different
//     // fragment. We have successfully loaded content from a.com. The fact
//     that
...
//     // loading a document from the origin. HTTP responses with >400 status
//     codes
//     // are just like regular documents from the origin and we render their
//     // response body just like we would a 200 response. This is why it can
//     make
...
//   // to load the page on a same-document navigation. Since the server has
//   only
...
//     // `false` from `NavigateToURL()`) there is no server-provided document
//     to
//     // navigate within. The result should be a cross-document navigation in
//     // order to attempt to load the document at the given path from the
//     server
//     // again.
//     EXPECT_FALSE(NavigateToURL(shell(), url2));
//     EXPECT_TRUE(navigation_1.has_committed());
...
//   // This case is also an empty 404 page, but we do replaceState and
//   pushState
...
//   // The navigation to |url1| is redirected and so |url1| does not commit.
//   Then
//   // the resulting navigation to |url2| lands at the same document URL as
//   |url0|
//   // which would be a same-document navigation if there wasn't a redirect
//   // involved. But since it started as a cross-document navigation it results
//   in
...
// //    since it did not hear about the document's URL changing yet. When it
// gets
...
// // the document did not. Currently, however, the browser only considers the
// URL
...
//   // Have the renderer `history.pushState()` to `url1`, which leaves it on
//   the
...
//   {
//     // We will wait for 2 navigations: one will be the pushState() and the
//     other
...
//   // URL did not match and did a cross-document navigation. Technically the
//   same
...
// // While a document is open, state in the browser may change such that
// loading
...
//   // renderer process, a new load of the current document would get a
//   different
//   // SiteInstance.
//   GURL modified_url0 =
...
//   EXPECT_EQ(main_frame_process_host,
...
//   // normally try isolate this navigation by using a different
//   RenderProcessHost
//   // and RenderFrameHost. But since it is same-document, we want to avoid
//   that
...
//   // captures, so track how many times the handler is called using a
//   non-const
//   // static variable.
...
// // Create two windows. When the second is deleted, it initiates a navigation
// in
...
//         // Even if the initiator RenderFrameHost is gone, its policy
//         container
...
//         // Even if the initiator RenderFrameHost is gone, the navigation
//         request
...
// // A document initiates a form submission in another frame, then deletes
// itself.
...
//   // Create a 'named' iframe in the first window. This will be the target of
//   the
//   // form submission.
//   EXPECT_TRUE(ExecJs(shell(), R"(
//     new Promise(resolve => {
...
//   // submission and removing itself before the scheduled form navigation
//   occurs.
...
//           ->current_frame_host();
//   blink::LocalFrameToken initiator_frame_token =
//   initiator_rfh->GetFrameToken(); int initiator_process_id =
//   initiator_rfh->GetProcess()->GetID(); base::RunLoop loop;
//   DidStartNavigationCallback callback(
//       web_contents(), base::BindLambdaForTesting([&](NavigationHandle*
//       handle) {
//         auto* request = NavigationRequest::From(handle);
...
//   // Initiate a form submission into the first window and delete the
//   initiator. EXPECT_TRUE(WaitForLoadStop(openee_shell->web_contents()));
...
// // A document initiates a form submission in a cross-origin frame, then
// deletes
...
//   // Create a cross origin child iframe. This iframe will embed another
//   iframe,
//   // which will initiate the navigation. The only purpose of this iframe is
//   to
...
//   // Now create a grandchild iframe, which is same-origin with the parent
//   (but
...
//   blink::LocalFrameToken initiator_frame_token =
//   initiator_rfh->GetFrameToken(); int initiator_process_id =
//   initiator_rfh->GetProcess()->GetID();
...
// // Media navigations synchronously complete the time of the
// `CommitNavigation`
...
//       "Document-Policy: force-load-at-top\r\n"
//       "\r\n"
//       "<p id='text' style='position: absolute; top: 10000px;'>Some
//       text</p>");
//   response.Done();
...
//   // Both document have the same URL. Only the first sets CSP:sandbox, but
//   both
//   // are sandboxed. They get an opaque origin different from each others.
//   EXPECT_NE(current_frame_host()->GetLastCommittedOrigin(),
//   origin_committed);
// }

// // Regression test for https://crbug.com/1158306.
// // Navigate to a response, which set Content-Security-Policy: sandbox AND
// block
// // the response. The error page shouldn't set sandbox flags.
// IN_PROC_BROWSER_TEST_F(NavigationBrowserTest,
// ErrorPageFromCspSandboxResponse) {
...
//           std::unique_ptr<NavigationThrottle> {
//             return
//             std::make_unique<ShutdownThrottle>(shell()->web_contents(),
//                                                       handle);
//           }));
...
//   // Watch that ReadyToCommitNavigation() will not happen when the renderer
//   is
//   // gone.
...
//   // Important: This is a browser-initiated navigation, so the
//   NavigationRequest
//   // does not have an open connection (NavigationClient) to the renderer that
//   it
...
//   // The navigation was aborted, which means it finished but did not commit,
//   and
...
//   ExecuteScriptAsync(
//       current_frame_host(),
//       JsReplace("window.open($1, '_blank').location = 'about:blank'",
//       url_b));

//   WebContents* popup_contents =
//   new_shell_observer.GetShell()->web_contents(); TestNavigationManager
//   manager_1(popup_contents, url_b); TestNavigationManager
//   manager_2(popup_contents, GURL("about:blank"));
...
// // Make a main document, have it request a cacheable subresources. Then make
// a
// // same-site document in an iframe that serves the CSP:Sandbox header. Stop
// the
// // test server, have the sandboxed document requests the same subresource.
// The
// // request should fail. To make sure the request is actually in the cache,
// the
...
//       let iframe = document.createElement("iframe");
//       iframe.onload = resolve;
//       iframe.src = "/set-header?Content-Security-Policy: sandbox
...
// // into the parsed headers by services/network and applied there. That
// directive
...
// // policy. This serves also as a regression test for
// https://crbug.com/1150314 IN_PROC_BROWSER_TEST_F(NavigationBrowserTest,
...
// Here
// // we check that Blink reports a console message if 'plugin-type' is
// delivered
// // in a policy.
// IN_PROC_BROWSER_TEST_F(NavigationBrowserTest,
...
//   console_observer.SetPattern(
//       "The Content-Security-Policy directive 'plugin-types' has been removed
//       " "from the specification. " "If you want to block plugins, consider
//       specifying \"object-src 'none'\" " "instead.");
...
//     target_frame->GetLastCommittedURL());

//     // Verify that "about:blank" committed with the expected origin, and in
//     the
...
//     // is whether this step will hit any `cookie_url`-related NOTREACHED or
//     DwoC
//     // in RestrictedCookieManager::ValidateAccessToCookiesAt.  This
//     verification
...
//     // Verify detecting and recovering from a NetworkService crash (e.g. via
//     the
...
//       // In addition to waiting (inside SimulateNetworkServiceCrash above)
//       for
//       // getting notified about being disconnected from
//       // network::mojom::NetworkServiceTest, we also want to make sure that
//       the
//       // relevant RenderFrameHost realizes that the NetworkService has
//       crashed.
//       // Which RenderFrameHost is relevant varies from test to test, so we
//       // flush multiple frames and use
...
//  private:
//   void FlushNetworkInterfacesInOpenerChain(RenderFrameHostImpl*
//   current_frame) {
...
//         JsReplace(kScriptTemplate, image_url).c_str(),
//         target_document.c_str());
...
// // right origin, even when the initiator of the navigation is not the parent
// or
...
// //
// // In this test case there are no process swaps and the parent of the
// navigated
...
//   ASSERT_TRUE(
//       ExecJs(main_frame,
//              "grandchild_window = window.open('about:blank',
//              'grandchild')"));
//   nav_observer.Wait();

//   // Verify that the grandchild has the same origin as the main frame (*not*
//   the
...
// // GrandchildToAboutBlank_ABA_SameSite and
// GrandchildToAboutBlank_ABB_CrossSite.
// IN_PROC_BROWSER_TEST_F(SubresourceLoadingTest,
...
// // grandparent of the target frame.
// //
// // In this test case the navigation forces a process swap of the target
// frame.
...
// or
// // opener of the frame targeted by the navigation.  In the
// // TopToAboutBlank_CrossSite testcase, the top-level navigation is initiated
// by
...
//   // Verify that the main frame is the only remaining frame and that it has
//   the
...
// // URLLoaderFactory.  Note that some aspects of the current behavior (e.g.
// the
...
//   // This inserts an `iframe` element without an `src` attribute.  According
//   to
...
//   shell()->web_contents()->GetPrimaryMainFrame(); RenderFrameHost* subframe =
...
//       base::BindLambdaForTesting([&](NavigationHandle* handle) {
//         ASSERT_TRUE(handle->HasCommitted());
//         // TODO(https://crbug.com/888079) Take sandbox into account. Same
//         Origin
//         // should be true
//         EXPECT_FALSE(handle->IsSameOrigin());
//         loop.Quit();
...
// // The test below verifies that error pages have a functional
// URLLoaderFactory. IN_PROC_BROWSER_TEST_F(SubresourceLoadingTest,
// URLLoaderFactoryInErrorPage) {
//   GURL error_url(embedded_test_server()->GetURL("/close-socket"));
//   EXPECT_FALSE(NavigateToURL(shell(), error_url));
...
//   // Add a subframe that will never commit a navigation (i.e. that will be
//   stuck
...
//   // No process swaps are expected before ReadyToCommit (which will never
//   happen
//   // for a navigation to "/hung").  This test assertion double-checks that
//   the
//   // test will cover inheriting URLLoaderFactory from the
//   creator/opener/parent
//   // frame.
//   RenderFrameHost* main_frame =
//   shell()->web_contents()->GetPrimaryMainFrame(); RenderFrameHost* subframe =
//   ChildFrameAt(main_frame, 0); EXPECT_EQ(main_frame->GetProcess()->GetID(),
//   subframe->GetProcess()->GetID());

//   // Ask the parent to script the same-origin subframe and trigger some HTTP
//   // subresource loads within the subframe.
//   //
//   // This tests the functionality of the URLLoaderFactory that gets used by
//   the
...
//   // matching `request_initiator_origin_lock` (e.g. inherited from the
//   parent). VerifyImageSubresourceLoads(shell(), "ifr.contentDocument");
// }
...
//   // Open a popup window that will never commit a navigation (i.e. that will
//   be
...
//   // non-opaque origin - it requires that the URLLoaderFactory will have a
//   // matching `request_initiator_origin_lock` (e.g. inherited from the
//   opener). VerifyImageSubresourceLoads(shell(), "popup.document");

//   // TODO(https://crbug.com/1194763): Crash recovery doesn't work when there
//   is
//   // no opener.
...
// // The test below verifies that an initial empty document has a functional
// // URLLoaderFactory.  The ...WithClearedOpener testcase is a regression test
// for
...
//   // stuck on the initial empty document).  Clearing of `popup.opener` tests
//   if
...
//   // non-opaque origin - it requires that the URLLoaderFactory will have a
//   // matching `request_initiator_origin_lock` (e.g. inherited from the
//   opener). VerifyImageSubresourceLoads(popup);
...
//   // Double-check that the `popup` didn't commit any navigation and that it
//   has
//   // an opaque origin.
//   EXPECT_EQ(GURL(), popup->GetPrimaryMainFrame()->GetLastCommittedURL());
...
//   // URLLoaderFactory from the creator/opener/parent frame (because the popup
//   is
//   // in another process).
//   RenderFrameHost* opener_frame =
//       shell()->web_contents()->GetPrimaryMainFrame();
...
//   // https://crbug.com/1189838) and 2) Chrome Extensions, to inject
//   Javascript
...
//   // This tests the functionality of the URLLoaderFactory that gets used by
//   the
//   // initial empty document.  In this test, the `request_initiator` will be
//   an
...
//   void SetUpOnMainThread() override {
//     // These navigation tests require full site isolation since they test
//     races
...
//     //
//     // TODO(https://crbug.com/1222647): Enable this by default in
//     content_shell.
...
//   // The crash, if any, will manifest in the b.com renderer. Open a b.com
//   window
...
//   ASSERT_TRUE(BeginNavigateToURLFromRenderer(shell(),
...
//   // Simulates a race where a new navigation to c.com begins after the
//   browser
...
//   // If navigation queueing is enabled, the first navigation will complete
//   the
...
//   // frame case. PerformanceManager reports when a remote frame is attached
//   to
//   // a local parent, and it was previously getting confused by the fact that
//   // a `blink::RemoteFrame` with matching RemoteFrameTokens was being
//   reported
//   // as attached twice: once by the initial page loaded in the next
//   statement,
//   // and the next when the browser needs to send a `UndoCommitNavigation()`
//   to
//   // the a.com renderer.
//   ASSERT_TRUE(NavigateToURL(
...
//   // data, to preserve existing behavior (unfortunately, the latter part is
//   not
...
//     // Intentionally do not wait for script completion here. This runs an
//     event
//     // loop that pumps incoming messages, but that would cause us to process
//     // IPCs from b.com out of order (since process DidCommitNavigation has
//     been
...
//     // However, since it's not possible to wait for `remove()` to take
//     effect,
//     // the test must cheat a little and directly call the Mojo IPC that the
//     JS
...
// // Regression test for https://crbug.com/1223837. Previously, if a child
// frame
// // was in the middle of committing a navigation to a provisional frame in
// render
...
//   // Start a navigation in the second child frame that will create a
//   speculative
...
//   // render process remains live even after the second child frame is
//   detached
//   // later in this test.
...
//   // Simulates a race where the a.com renderer detaches the second child
//   frame
...
//   EXPECT_TRUE(WaitForLoadStop(web_contents));
//   // Validate that render process for b.com has handled the detach message
//   for
//   // the provisional frame that was committing. Before the fix, the render
//   // process for b.com still had the proxy for the second child frame,
//   because
...
//   // Note that since we committed a navigation, the next cross-origin
//   navigation
//   // will create a speculative RenderFrameHost (when site isolation is
//   enabled).
...
//   // However, without SiteIsolation, an earlier DidStopLoading might be
//   received
...
//   EXPECT_TRUE(IsLastCommittedEntryOfPageType(web_contents(),
//   PAGE_TYPE_NORMAL)); EXPECT_EQ(redirection_url,
//   web_contents()->GetLastCommittedURL());
// }

// class NavigationBrowserTestCredentiallessIframe : public
// NavigationBrowserTest {
//  public:
//   void SetUpCommandLine(base::CommandLine* command_line) override {
//     NavigationBrowserTest::SetUpCommandLine(command_line);
...
//   // FrameTreeNode. The RenderFrameHost, however, is updated only on
//   navigation. EXPECT_TRUE(
//       ExecJs(main_frame(),
//              "document.getElementById('test_iframe').credentialless =
//              true;"));
...
//   // attribute. The grandchild RenderFrameHost is not credentialless, since
//   its
...
//   EXPECT_EQ(false,
//             EvalJs(grandchild->current_frame_host(),
...
//   EXPECT_TRUE(ExecJs(
//       main_frame(), JsReplace("document.getElementById('test_iframe').src =
//       $1",
//                               iframe_url_2)));
//   WaitForLoadStop(web_contents());
//   EXPECT_TRUE(child->Credentialless());
...
//   // The storage key's nonce is the same for all credentialless documents in
//   the
//   // same page.
//   EXPECT_TRUE(child->current_frame_host()->storage_key().nonce().has_value());
//   EXPECT_EQ(credentialless_nonce,
...
//   EXPECT_TRUE(ExecJs(
//       child, JsReplace("document.getElementById('grandchild_iframe').src =
//       $1",
//                        iframe_url_2)));
//   WaitForLoadStop(web_contents());
//   EXPECT_TRUE(grandchild->current_frame_host()->IsCredentialless());
...
//   // Remove the 'credentialless' attribute from the iframe. This propagates
//   to
...
//   // not have the 'credentialless' attribute anymore, the grandchild document
//   is
...
//   EXPECT_EQ(true,
//             EvalJs(grandchild2->current_frame_host(),
//             "window.credentialless"));
//   EXPECT_TRUE(
...
//   GURL iframe_url_b = embedded_test_server()->GetURL("b.com",
//   "/title1.html"); EXPECT_TRUE(NavigateToURL(shell(), main_url_b));
...
// // Ensures that OpenURLParams::FromNavigationHandle translates navigation
// params
...
//   // Test that the params are translated correctly for a redirected
//   navigation. const GURL kRedirectedURL(
...
//   EXPECT_EQ(first_tab_handle->GetReferrer(),
//   second_tab_handle->GetReferrer()); EXPECT_TRUE(
...
// // such a URL is about:srcdoc. This ensures that the SiteInstance's site is
// set
...
//   // Inject a srcdoc iframe into the blank document.  This shouldn't really
//   be
...
//   // Committing an about:srcdoc navigation currently forces the
//   SiteInstance's
...
//   // path that is no longer supported, and hence this triggered a
//   NOTREACHED().
...
//   } else {
//     // When we get into this situation with strict site isolation, the site
//     URL
//     // currently used is "about:". This may be changed in the future (e.g.,
//     to
//     // an opaque ID).
...
//     pervasive_payload_url_ =
//     embedded_test_server()->GetURL(kPervasivePayload); std::string
//     pervasive_payloads_params = base::StrCat(
...
// // We may navigate the main frame, the subframe that may have an unload
// handler
...
//       case NavigateFrameType::kSubFrame:
//         // Navigating the subframe will have unload if the subframe has
//         unload. return GetUnloadFrameType() == UnloadFrameType::kSubFrame;
//     }
//   }
// };
...
//   AddUnloadHandler(DescendantRenderFrameHostImplAt(current_frame_host(),
//   {0}));
