...
#include "third_party/chromium/media/cast/sender/video_sender.h"
...
#include "third_party/chromium/media/cast/sender/fake_video_encode_accelerator_factory.h"
#include "third_party/chromium/media/cast/sender/video_frame_factory.h"
...
#include "third_party/chromium/media/video/fake_video_encode_accelerator.h"
...
class PeerVideoSender : public VideoSender {
 public:
  ...
  void ProcessFeedback(const media_m96::VideoCaptureFeedback& feedback) {
    feedback_ = feedback;
  }
...
class VideoSenderTest : public ::testing::Test {
 protected:
  ...
  scoped_refptr<media_m96::VideoFrame> GetLargeNewVideoFrame() {
    if (first_frame_timestamp_.is_null())
      first_frame_timestamp_ = testing_clock_.NowTicks();
    gfx::Size size(kWidth, kHeight);
    scoped_refptr<media_m96::VideoFrame> video_frame =
...
TEST_F(VideoSenderTest, BuiltInEncoder) {
  ...
  scoped_refptr<media_m96::VideoFrame> video_frame = GetNewVideoFrame();
...
TEST_F(VideoSenderTest, AcksCancelRetransmits) {
  ...
  transport_->SetPause(true);
  scoped_refptr<media_m96::VideoFrame> video_frame = GetLargeNewVideoFrame();
  video_sender_->InsertRawVideoFrame(video_frame, testing_clock_.NowTicks());
  RunTasks(33);
