...
void* AlsaAudioSink::ThreadEntryPoint(void* context) {
  SB_DCHECK(context);
  AlsaAudioSink* audio_sink = static_cast<AlsaAudioSink*>(context);
  audio_sink->AudioThreadFunc();
  return NULL;
}

void AlsaAudioSink::AudioThreadFunc() {
  InitializePlaybackHandle();
...
void AlsaAudioSink::StartSinkThread() {
  SB_CHECK(!SbThreadIsValid(audio_sink_thread_));
