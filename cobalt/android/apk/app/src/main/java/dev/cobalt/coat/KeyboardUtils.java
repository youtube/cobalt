 ...
 *
 * <p>Provides helper methods to identify specific key codes that require custom handling, such as
 * partner-specific fallback keys that must be intercepted before IME dispatch to allow OS-level
 * fallback.
 *
 * <p>This is a utility class with static methods and cannot be instantiated. It is owned by the
 * Cobalt Android runtime team and remains active throughout the application's lifetime.
 *
 * <p>This class is stateless and thread-safe. All methods are static and can be safely called from
 * any thread.
 */
public final class KeyboardUtils {
   ...
   *
   * <p>This is similar to the isGamepadButton defined in android.view.KeyEvent but custom for
   * Cobalt- specific key events that need to be handled before an IME dispatch.
   *
   * <p>Note: These keys must be intercepted here in Java before they are dispatched to the IME
   ...
   *
   * <p>Consequently, filtering these keys in Cobalt's native keycode conversion (e.g.
   * keyboard_code_conversion_android_cobalt.cc) is too late to allow fallback, as the key events
   * will still be consumed although they are mapped to VKEY_UNKNOWN.
   */
