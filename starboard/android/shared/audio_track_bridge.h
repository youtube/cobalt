...
class AudioTrackBridge {
 public:
  using FloatDataRef = jni_zero::ScopedJavaGlobalRef<jfloatArray>;
  using ByteDataRef = jni_zero::ScopedJavaGlobalRef<jbyteArray>;
  using AudioDataVar = std::variant<FloatDataRef, ByteDataRef>;
 ...
 private:
  ...
  const AudioDataVar j_audio_data_;
};
