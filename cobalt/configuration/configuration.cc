...
Configuration::UserOnExitStrategy Configuration::CobaltUserOnExitStrategy() {
  ...
  if (!configuration_api_) {
    return Configuration::UserOnExitStrategy::kStop;
  }
  const std::string strategy = configuration_api_->CobaltUserOnExitStrategy();
  if (strategy == kNoExit) {
    return Configuration::UserOnExitStrategy::kNoExit;
  }
  if (strategy == kSuspend) {
    return Configuration::UserOnExitStrategy::kConceal;
  }
  return Configuration::UserOnExitStrategy::kStop;
}
