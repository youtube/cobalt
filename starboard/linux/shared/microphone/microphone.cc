...
class SbMicrophoneImpl : public SbMicrophonePrivate {
 public:
  ...
  bool Open() override {
    ...
    if (error < 0) {
      SB_LOG(ERROR) << "snd_pcm_hw_params_any result: " << error;
      Close();
      return false;
    }
    ...
    if (error < 0) {
      SB_LOG(ERROR) << "snd_pcm_hw_params_set_access result: " << error;
      Close();
      return false;
    }
    ...
    if (error < 0) {
      SB_LOG(ERROR) << "snd_pcm_hw_params_set_format result: " << error;
      Close();
      return false;
    }
    ...
    if (error < 0) {
      SB_LOG(ERROR) << "snd_pcm_hw_params_set_rate_near result: " << error;
      Close();
      return false;
    }
    ...
    if (error < 0) {
      SB_LOG(ERROR) << "snd_pcm_hw_params_set_channels result: " << error;
      Close();
      return false;
    }
    ...
    if (error < 0) {
      SB_LOG(ERROR) << "snd_pcm_hw_params_set_period_size_near result: "
                    << error;
      Close();
      return false;
    }
    ...
    if (error < 0) {
      SB_LOG(ERROR) << "snd_pcm_hw_params_set_buffer_size_near result: "
                    << error;
      Close();
      return false;
    }
    ...
    if (error < 0) {
      SB_LOG(ERROR) << "snd_pcm_hw_params result: " << error;
      Close();
      return false;
    }
    ...
    if (error < 0) {
      SB_LOG(ERROR) << "snd_pcm_prepare result: " << error;
      Close();
      return false;
    }
    ...
    if (error < 0) {
      SB_LOG(ERROR) << "snd_pcm_start result: " << error;
      Close();
      return false;
    }
  ...
  int Read(void* out_audio_data, int audio_data_size) override {
    ...
    if (frames_read < 0) {
      ...
      SB_LOG(ERROR) << "snd_pcm_recover result: " << err;
      if (err >= 0) {
