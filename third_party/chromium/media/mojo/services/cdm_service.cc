...
// which then host CDMs created by the media_m96::CdmFactory provided by the
// CdmService::Client.
//
// Lifetime Note:
...
class CdmFactoryImpl final : public DeferredDestroy<mojom::CdmFactory> {
 ...
 private:
  media_m96::CdmFactory* GetCdmFactory() {
    if (!cdm_factory_) {
  ...
  std::unique_ptr<media_m96::CdmFactory> cdm_factory_;
