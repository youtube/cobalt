...
#include "third_party/chromium/media/fuchsia/audio/fake_audio_consumer.h"
...
void FakeAudioConsumer::Start(fuchsia::media_m96::AudioConsumerStartFlags flags,
                              int64_t reference_time,
                              int64_t media_time) {
  CHECK(state_ == State::kStopped);

  if (reference_time != fuchsia::media_m96::NO_TIMESTAMP) {
    reference_time_ = base::TimeTicks::FromZxTime(reference_time);
  } else {
    reference_time_ = base::TimeTicks::Now() + kMinLeadTime;
  }

  if (media_time != fuchsia::media_m96::NO_TIMESTAMP) {
...
void FakeAudioConsumer::SendPacket(fuchsia::media_m96::StreamPacket stream_packet,
                                   SendPacketCallback callback) {
  CHECK_LT(stream_packet.payload_buffer_id, num_buffers_);

  Packet packet;
  if (stream_packet.pts == fuchsia::media_m96::NO_TIMESTAMP) {
    if (media_pos_.is_min()) {
...
void FakeAudioConsumer::SendPacketNoReply(fuchsia::media_m96::StreamPacket packet) {
  NOTREACHED();
}
