...
bool VideoCaptureFormat::IsValid() const {
  return (frame_size.width() <= media_m96::limits::kMaxDimension) &&
         (frame_size.height() <= media_m96::limits::kMaxDimension) &&
         (frame_size.GetArea() >= 0) &&
         (frame_size.GetArea() <= media_m96::limits::kMaxCanvas) &&
         (frame_rate >= 0.0f) &&
         (frame_rate <= media_m96::limits::kMaxFramesPerSecond) &&
