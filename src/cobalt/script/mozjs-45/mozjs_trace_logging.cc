...
class TraceLoggingContainer {
 public:
  ...
  static TraceLoggingContainer* GetInstance() {
    return Singleton<TraceLoggingContainer, StaticMemorySingletonTraits<
                                                TraceLoggingContainer> >::get();
  }

  TraceLogging* GetLogger() { return &logger_; }
...
const char* const TraceLogging::type_name[] = {
    "IonCompile",     "IonCompile",     "IonCannon",     "IonCannon",
    "IonCannon",      "IonSideCannon",  "IonSideCannon", "IonSideCannon",
    "YarrJIT",        "YarrJIT",        "JMSafepoint",   "JMSafepoint",
    "JMNormal",       "JMNormal",       "JMCompile",     "JMCompile",
    "GarbageCollect", "GarbageCollect", "Interpreter",   "Interpreter",
    "Info",
};
...
void TraceLogging::log(Type type, const char* filename, unsigned int line) {
  ...
  switch (type) {
    ...
    case INTERPRETER_START:
      if (filename != NULL) {
        TRACE_EVENT_BEGIN2(kTraceLoggingCategory, type_name[type], "file",
                           TRACE_STR_COPY(filename), "line", line);
      } else {
...
void TraceLogging::log(const char* log) { UNREFERENCED_PARAMETER(log); }

void TraceLogging::log(Type type) { log(type, NULL, 0); }
...
void TraceLog(TraceLogging* logger, const char* log) { logger->log(log); }
