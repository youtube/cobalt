...
void AudioRendererSinkAndroid::UpdatePlaybackStatusIfPossible() {
  ...
  MediaAgency::GetInstance()->SetPlaybackStatus(audio_session_id, seek_to_time,
...
// static
void AudioRendererSinkAndroid::ConsumeFramesFunc(
    int frames_consumed,
...
// static
void AudioRendererSinkAndroid::ErrorFunc(bool capability_changed, void* context) {
  AudioRendererSinkAndroid* audio_renderer_sink =
      static_cast<AudioRendererSinkAndroid*>(context);
  SB_DCHECK(audio_renderer_sink);
  SB_DCHECK(audio_renderer_sink->render_callback_);
