...
TEST(PosixGmtimeRTests, HandlesNegativeStdTimeBeforeEpoch) {
...
TEST(PosixGmtimeRTests, HandlesNegativeDstTimeBeforeEpoch) {
...
TEST(PosixGmtimeRTests, ReturnsPointerToGivenBufferAndDoesNotOverwrite) {
  ...
  EXPECT_EQ(tm_ptr_call1->tm_gmtoff, 0);
  EXPECT_TRUE(tm_ptr_call1->tm_zone &&
              (strcmp(tm_ptr_call1->tm_zone, "UTC") == 0 ||
               strcmp(tm_ptr_call1->tm_zone, "GMT") == 0));
...
// Test gmtime_r's behavior with time_t's high value.
...
TEST(PosixGmtimeRTests, Gmtime) {
  ...
  EXPECT_TRUE(gmt_result_ptr->tm_zone &&
              (strcmp(gmt_result_ptr->tm_zone, "UTC") == 0 ||
