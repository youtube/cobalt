...
#if defined(COBALT)
    // Upstream Skia has been modified to make it such that this hack is not
    // necessary, however much has changed so this workaround is being
    // introduced instead.  This change can probably be thrown away during a
    // rebase.
    // We don't bother with looking up the texture in a cache, the incoming
    // bitmap should already be associated with a texture.
...
#if !defined(COBALT)
    // See comment in the "#if defined(COBALT)" block above.
