...
class CobaltA11yHelper extends ExploreByTouchHelper {
  ...
  // @NativeMethods
  //   interface Natives {
  //     void injectKeyEvent(WebContents webContents, int key);
  //   }
  ...
  private void maybeInjectEvent(int currentFocusedViewId) {
    ...
    switch (currentFocusedViewId) {
      case CENTER_VIEW_ID:
        ...
        break;
      case UP_VIEW_ID:
        // TODO: might not need to invoke imeAdapter.dispatchKeyEvent() in CobaltActiviy, as we only explicitly call it
        // on back key press.
        // Update: we have to manually invoke imeAdapter.dispatchKeyEvent() to make navigation work.
