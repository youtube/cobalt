...
// By default, Cobalt restarts MediaCodec after stopes/flushes during
// Reset()/Flush(). Set the following variable to > 0 to force it to
// wait during Reset()/Flush().
constexpr int64_t kForceResetDelayUsec = 0;
constexpr int64_t kForceFlushDelayUsec = 0;
...
class PlayerComponentsFactory : public starboard::shared::starboard::player::
  ...
  std::unique_ptr<VideoDecoder> CreateVideoDecoder(
    ...
    if (kForceResetDelayUsec) {
      reset_delay_usec = kForceResetDelayUsec;
      SB_LOG(INFO) << "`kForceResetDelayUsec` is set to > 0, force a delay"
                   << " of " << reset_delay_usec << "us during Reset().";
    }
    if (kForceFlushDelayUsec) {
      flush_delay_usec = kForceFlushDelayUsec;
      SB_LOG(INFO) << "`kForceFlushDelayUsec` is set to > 0, force a delay"
