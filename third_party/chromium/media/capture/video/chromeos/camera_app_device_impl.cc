...
void OnStillCaptureDone(media_m96::mojom::ImageCapture::TakePhotoCallback callback,
                        int status,
                        mojom::BlobPtr blob) {
...
ReprocessTaskQueue CameraAppDeviceImpl::GetSingleShotReprocessOptions(
    media_m96::mojom::ImageCapture::TakePhotoCallback take_photo_callback) {
...
void CameraAppDeviceImpl::SetReprocessOptions(
  ...
  for (const auto& effect : effects) {
    ReprocessTask task;
    task.effect = effect;
    task.callback = media_m96::base::BindPostTaskToCurrentDefault(
