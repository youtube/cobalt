...
`kSbSystemPathStorageDirectory` is not required to implement. Set
`sb_evergreen_compatible_lite` to `1`s in the `gyp` platform config. The
remaining is the same as the Evergreen full doc.
