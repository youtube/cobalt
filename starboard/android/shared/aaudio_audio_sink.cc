...
SbAudioSink AaudioAudioSinkType::Create(
  ...
  return Create(channels, sampling_frequency_hz, audio_sample_type,
                ...
                /*start_time=*/0, /*is_web_audio=*/false, context);
}
...
SbAudioSink AaudioAudioSinkType::Create(
  ...
  auto audio_sink = AaudioAudioSink::Create(
      ...
      frames_per_channel, callbacks, start_time, is_web_audio, context);
...
void AaudioAudioSink::SetStartTime(int64_t start_time) {
  start_time_.store(start_time, std::memory_order_relaxed);
}
