...
class PosixEpollWaitTests : public PosixEpollTest {
 protected:
  ...
  void SetUp() override {
    ...
    // NOTE: No FDs are added to epoll instance by default in SetUp.
    // Tests are responsible for adding/removing FDs they monitor.
  }
  ...
  void RemoveFromEpoll(int fd_to_remove) {
    ...
    // Best effort removal. Don't fail test if DEL fails (e.g. fd not present).
    if (epfd_ >= 0 &&
        fd_to_remove >=
            0) {  // Only attempt if epfd_ and fd_to_remove are valid
...
TEST_F(PosixEpollWaitTests, TimeoutZeroReturnsImmediately) {
  AddToEpoll(pipe_fds_[0], EPOLLIN);  // Watch something
...
TEST_F(PosixEpollWaitTests, WaitForWriteOnReadOnlyPipeEndWhenWriteEndClosed) {
  ...
  if (nfds > 0) {
    ...
    // This might be necessary for some systems to then report HUP or IN
    // correctly.
