...
pipeline () {
  ...
  if [[ "${CONFIG}" == "devel" || "${CONFIG}" == "qa" ]]; then
    custom_vars="\"custom_vars\": {'download_remoteexec_cfg': True, 'rbe_instance': 'projects/cobalt-actions-prod/instances/default_instance', 'reapi_backend_config_path': 'cobalt.star'},"
  fi
