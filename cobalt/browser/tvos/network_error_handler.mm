...
void ShowPlatformErrorDialog(content::WebContents* webContents) {
  ...
  NSString* retryButtonTitle = NSLocalizedStringWithDefaultValue(
      @"NoNetworkDialogRetry", nil, NSBundle.mainBundle, @"Retry",
      @"Retry button");
  NSString* exitButtonTitle = NSLocalizedStringWithDefaultValue(
  ...
  base::WeakPtr<content::WebContents> weakWebContents =
      webContents->GetWeakPtr();
  UIAlertAction* retryAction = [UIAlertAction
      actionWithTitle:retryButtonTitle
                style:UIAlertActionStyleDefault
              handler:^(UIAlertAction* action) {
                if (!weakWebContents) {
                  return;
                }
                weakWebContents->GetController().Reload(
                    content::ReloadType::NORMAL, /*check_for_repost*/ false);
              }];
  [alert addAction:retryAction];

  UIAlertAction* exitAction =
      [UIAlertAction actionWithTitle:exitButtonTitle
                               style:UIAlertActionStyleDefault
                             handler:^(UIAlertAction* action) {
                               exit(/*error_level*/ 0);
                             }];
