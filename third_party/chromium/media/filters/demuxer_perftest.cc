...
class DemuxerHostImpl : public media_m96::DemuxerHost {
 public:
  ...
  void OnDemuxerError(media_m96::PipelineStatus error) override {}
};

static void QuitLoopWithStatus(base::OnceClosure quit_cb,
                               media_m96::PipelineStatus status) {
  CHECK_EQ(status, media_m96::PIPELINE_OK);
...
typedef std::vector<media_m96::DemuxerStream*> Streams;
...
class StreamReader {
 public:
  StreamReader(media_m96::Demuxer* demuxer, bool enable_bitstream_converter);
 ...
 private:
  void OnReadDone(scoped_refptr<base::SingleThreadTaskRunner> task_runner,
                  ...
                  media_m96::DemuxerStream::Status status,
...
StreamReader::StreamReader(media_m96::Demuxer* demuxer,
                           bool enable_bitstream_converter) {
  std::vector<media_m96::DemuxerStream*> streams = demuxer->GetAllStreams();
  for (auto* stream : streams) {
    streams_.push_back(stream);
    end_of_stream_.push_back(false);
    last_read_timestamp_.push_back(media_m96::kNoTimestamp);
    counts_.push_back(0);
...
void StreamReader::Read() {
  ...
  CHECK(end_of_stream || timestamp != media_m96::kNoTimestamp);
...
void StreamReader::OnReadDone(
    ...
    media_m96::DemuxerStream::Status status,
    scoped_refptr<DecoderBuffer> buffer) {
  CHECK_EQ(status, media_m96::DemuxerStream::kOk);
  CHECK(buffer);
  *end_of_stream = buffer->end_of_stream();
  *timestamp = *end_of_stream ? media_m96::kNoTimestamp : buffer->timestamp();
...
int StreamReader::GetNextStreamIndexToRead() {
  ...
  for (int i = 0; i < number_of_streams(); ++i) {
    ...
    if (last_read_timestamp_[i] == media_m96::kNoTimestamp)
      return i;
