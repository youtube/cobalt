...
#define DEFINE_JNI_LIFECYCLE_HANDLER(JniName, EventType)            \
  void JNI_AppEventBridge_##JniName(JNIEnv* env, jlong timestamp) { \
    SendLifecycleEvent(EventType, timestamp);                       \
  }

DEFINE_JNI_LIFECYCLE_HANDLER(HandlePreloadEvent, kSbEventTypePreload)
DEFINE_JNI_LIFECYCLE_HANDLER(HandleBlurEvent, kSbEventTypeBlur)
DEFINE_JNI_LIFECYCLE_HANDLER(HandleFocusEvent, kSbEventTypeFocus)
DEFINE_JNI_LIFECYCLE_HANDLER(HandleConcealEvent, kSbEventTypeConceal)
DEFINE_JNI_LIFECYCLE_HANDLER(HandleRevealEvent, kSbEventTypeReveal)
DEFINE_JNI_LIFECYCLE_HANDLER(HandleFreezeEvent, kSbEventTypeFreeze)
DEFINE_JNI_LIFECYCLE_HANDLER(HandleUnfreezeEvent, kSbEventTypeUnfreeze)
DEFINE_JNI_LIFECYCLE_HANDLER(HandleStopEvent, kSbEventTypeStop)

#undef DEFINE_JNI_LIFECYCLE_HANDLER

void JNI_AppEventBridge_HandleStartEvent(
    JNIEnv* env,
    const base::android::JavaRef<jobjectArray>& j_args,
    const base::android::JavaRef<jstring>& jlink,
    jlong timestamp) {
  SbEvent event;
