...
SbDrmUpdateServerCertificate() whether the server certificate update succeeded.
...
`SbDrmGenerateSessionUpdateRequest()` SbDrmGenerateSessionUpdateRequest() was
called. The `context` parameter matches the context passed to
`SbDrmCreateSystem()` SbDrmCreateSystem() .
...
`kSbDrmStatusSuccess`. It can be `NULL` NULL if `status` is
`kSbDrmStatusSuccess` or if no error message is available.

The `ticket` parameter matches the ticket passed to
`SbDrmGenerateSessionUpdateRequest()` SbDrmGenerateSessionUpdateRequest() , or
is `kSbDrmTicketInvalid` if the update request was generated by the DRM system.

The `session_id` parameter must not be `NULL` NULL when `ticket` is
...
`SbDrmGenerateSessionUpdateRequest()` SbDrmGenerateSessionUpdateRequest() call.
...
The `error_message` parameter provides additional details if `status` is not
`kSbDrmStatusSuccess`. It can be `NULL` NULL if `status` is
`kSbDrmStatusSuccess` or if no error message is available. `succeeded` succeeded
is whether the session was successfully updated or not.
...
A handle to a DRM system used with `SbDecoder` SbDecoder or `SbPlayer` SbPlayer
.
...
Creates a new DRM system used when constructing an `SbPlayer` SbPlayer or
`SbDecoder` SbDecoder .
...
*   `update_request_callback`: A callback invoked after calling
    `SbDrmGenerateSessionUpdateRequest()` SbDrmGenerateSessionUpdateRequest() .

*   `session_updated_callback`: A callback invoked after calling
    `SbDrmUpdateSession()` SbDrmUpdateSession() .
...
*   `session_closed_callback`: A callback invoked when a session closes. If
    `NULL` NULL is passed for any of the callbacks (`update_request_callback`,
...
SbPlayer or `SbDecoder` SbDecoder before destroying the DRM system.
...
`SbDrmCreateSystem()` SbDrmCreateSystem() . It passes the `context` and a
populated request, or a `NULL` NULL `session_id` if an error occurred.
...
result of `MediaDrm.getPropertyByteArray("metrics")`
MediaDrm.getPropertyByteArray("metrics") , encoded using url safe base64 without
padding and line wrapping. On systems using Widevine CE CDM with oemcrypto 16 or
later, it should return the metrics retrieved via `Cdm::getMetrics()`
...
Returns `NULL` NULL if the underlying DRM system does not support metrics, or if
...
Returns `true` true if the server certificate of `drm_system` can be updated via
`SbDrmUpdateServerCertificate()` SbDrmUpdateServerCertificate() . This return
value must remain constant for the lifetime of `drm_system`.
...
SbDrmIsServerCertificateUpdatable() returns `true` true .
...
`context` and the status. The `key` parameter must not be `NULL` NULL .

`drm_system` must not be `kSbDrmSystemInvalid`. `ticket` is the opaque ID that
distinguishes callbacks from multiple concurrent calls to `SbDrmUpdateSession()`
SbDrmUpdateSession() , which will be passed to `session_updated_callback` as-is.
The caller must ensure ticket uniqueness; issuing multiple calls with the same
ticket may result in undefined behavior.

Once the session updates successfully, an `SbPlayer` SbPlayer or `SbDecoder`
SbDecoder associated with that DRM key system can decrypt encrypted samples.
