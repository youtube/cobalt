...
#include "third_party/chromium/media/cdm/library_cdm/clear_key_cdm/clear_key_cdm.h"
...
#include "third_party/chromium/media/cdm/library_cdm/cdm_host_proxy_impl.h"
#include "third_party/chromium/media/cdm/library_cdm/clear_key_cdm/cdm_file_io_test.h"
...
#include "third_party/chromium/media/cdm/library_cdm/clear_key_cdm/ffmpeg_cdm_audio_decoder.h"
...
static scoped_refptr<media_m96::DecoderBuffer> CopyDecoderBufferFrom(
  ...
  scoped_refptr<media_m96::DecoderBuffer> output_buffer =
      media_m96::DecoderBuffer::CopyFrom(input_buffer.data, input_buffer.data_size);
  ...
  for (uint32_t i = 0; i < input_buffer.num_subsamples; ++i) {
    subsamples.push_back(
        media_m96::SubsampleEntry(input_buffer.subsamples[i].clear_bytes,
  ...
  if (input_buffer.encryption_scheme == cdm::EncryptionScheme::kCenc) {
    output_buffer->set_decrypt_config(media_m96::DecryptConfig::CreateCencConfig(
        key_id_string, iv_string, subsamples));
  } else {
    DCHECK_EQ(input_buffer.encryption_scheme, cdm::EncryptionScheme::kCbcs);
    output_buffer->set_decrypt_config(media_m96::DecryptConfig::CreateCbcsConfig(
        key_id_string, iv_string, subsamples,
        media_m96::EncryptionPattern(input_buffer.pattern.crypt_byte_block,
...
void ConvertCdmKeysInfo(const media_m96::CdmKeysInfo& keys_info,
...
void* CreateCdmInstance(int cdm_interface_version,
  ...
  if (cdm_interface_version == CDM_10::kVersion) {
    ...
    return static_cast<CDM_10*>(
        new media_m96::ClearKeyCdm(host, key_system_string));
  }
...
static void CopyDecryptResults(media_m96::Decryptor::Status* status_copy,
                               scoped_refptr<DecoderBuffer>* buffer_copy,
                               media_m96::Decryptor::Status status,
...
cdm::Status ClearKeyCdm::InitializeAudioDecoder(
  ...
  if (!audio_decoder_) {
    audio_decoder_ =
        std::make_unique<media_m96::FFmpegCdmAudioDecoder>(cdm_host_proxy_.get());
  }
...
cdm::Status ClearKeyCdm::DecryptToMediaDecoderBuffer(
  ...
  media_m96::Decryptor::Status status = media_m96::Decryptor::kError;
  ...
  cdm_->GetCdmContext()->GetDecryptor()->Decrypt(
      media_m96::Decryptor::kVideo, std::move(buffer),
      base::BindOnce(&CopyDecryptResults, &status, decrypted_buffer));

  if (status == media_m96::Decryptor::kError)
    return cdm::kDecryptError;

  if (status == media_m96::Decryptor::kNoKey)
    return cdm::kNoKey;

  DCHECK_EQ(status, media_m96::Decryptor::kSuccess);
  return cdm::kSuccess;
}
