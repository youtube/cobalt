...
#include "third_party/chromium/media/base/mac/color_space_util_mac.h"
...
+ (media_m96::VideoPixelFormat)FourCCToChromiumPixelFormat:(FourCharCode)code {
  switch (code) {
    case kCVPixelFormatType_420YpCbCr8BiPlanarVideoRange:
      return media_m96::PIXEL_FORMAT_NV12;  // Mac fourcc: "420v".
    case kCVPixelFormatType_422YpCbCr8:
      return media_m96::PIXEL_FORMAT_UYVY;  // Mac fourcc: "2vuy".
    case kCMPixelFormat_422YpCbCr8_yuvs:
      return media_m96::PIXEL_FORMAT_YUY2;
    case kCMVideoCodecType_JPEG_OpenDML:
      return media_m96::PIXEL_FORMAT_MJPEG;  // Mac fourcc: "dmb1".
    default:
      return media_m96::PIXEL_FORMAT_UNKNOWN;
  }
}
...
- (instancetype)initWithFrameReceiver:
    (media_m96::VideoCaptureDeviceAVFoundationFrameReceiver*)frameReceiver {
  if ((self = [super init])) {
    ...
    _sampleBufferTransformer = media_m96::SampleBufferTransformer::Create();
  }
  return self;
}
...
- (BOOL)setCaptureHeight:(int)height
  ...
  _bestCaptureFormat.reset(
      media_m96::FindBestCaptureFormat([_captureDevice formats], width, height,
                                   frameRate),
      base::scoped_policy::RETAIN);
  ...
  VLOG(2) << __func__ << ": configuring '"
          << media_m96::MacFourCCToString(best_fourcc) << "' " << width << "x"
          << height << "@" << frameRate;
  ...
  if ([captureConnection
    ...
    [captureConnection
        setVideoMinFrameDuration:CMTimeMake(media_m96::kFrameRatePrecision,
                                            (int)(frameRate *
                                                  media_m96::kFrameRatePrecision))];
  }
  if ([captureConnection
          respondsToSelector:@selector(isVideoMaxFrameDurationSupported)] &&
      [captureConnection isVideoMaxFrameDurationSupported]) {
    [captureConnection
        setVideoMaxFrameDuration:CMTimeMake(media_m96::kFrameRatePrecision,
                                            (int)(frameRate *
                                                  media_m96::kFrameRatePrecision))];
  }
  return YES;
...
- (void)setScaledResolutions:(std::vector<gfx::Size>)resolutions {
  if (!base::FeatureList::IsEnabled(media_m96::kInCapturerScaling)) {
    return;
  }
  ...
  for (size_t i = 0; i < resolutions.size(); ++i) {
    ...
    auto scaledFrameTransformer = media_m96::SampleBufferTransformer::Create();
    scaledFrameTransformer->Reconfigure(
        media_m96::SampleBufferTransformer::
    ...
    const auto handler = ^(CMSampleBufferRef sampleBuffer, NSError* error) {
      {
        ...
        if (_frameReceiver) {
          if (error != nil) {
            ...
            char* baseAddress = 0;
            size_t length = 0;
            media_m96::ExtractBaseAddressAndLength(&baseAddress, &length,
                                               sampleBuffer);
...
- (void)processSample:(CMSampleBufferRef)sampleBuffer
        captureFormat:(const media_m96::VideoCaptureFormat&)captureFormat
  ...
  char* baseAddress = 0;
  size_t frameSize = 0;
  media_m96::ExtractBaseAddressAndLength(&baseAddress, &frameSize, sampleBuffer);
...
- (BOOL)processPixelBufferPlanes:(CVImageBufferRef)pixelBuffer
  ...
  const size_t numPlanes =
      media_m96::VideoFrame::NumPlanes(captureFormat.pixel_format);
  ...
  for (size_t plane = 0; plane < numPlanes; ++plane) {
    size_t bytesPerRow = media_m96::VideoFrame::RowBytes(
        plane, captureFormat.pixel_format, captureFormat.frame_size.width());
    size_t height =
        media_m96::VideoFrame::PlaneSize(captureFormat.pixel_format, plane,
  ...
  // If media_m96::VideoFrame::PlaneSize differs from the CVPixelBuffer's size then
...
- (void)processPixelBufferNV12IOSurface:(CVPixelBufferRef)pixelBuffer
                          captureFormat:
                              (const media_m96::VideoCaptureFormat&)captureFormat
                             colorSpace:(const gfx::ColorSpace&)colorSpace
                              timestamp:(const base::TimeDelta)timestamp {
  VLOG(3) << __func__;
  DCHECK_EQ(captureFormat.pixel_format, media_m96::PIXEL_FORMAT_NV12);

  IOSurfaceRef ioSurface = CVPixelBufferGetIOSurface(pixelBuffer);
  DCHECK(ioSurface);
  media_m96::CapturedExternalVideoBuffer externalBuffer =
  ...
  std::vector<media_m96::CapturedExternalVideoBuffer> scaledExternalBuffers;
  ...
  for (auto& scaledFrameTransformer : _scaledFrameTransformers) {
    ...
    media_m96::VideoCaptureFormat scaledCaptureFormat = captureFormat;
...
- (media_m96::CapturedExternalVideoBuffer)
    capturedExternalVideoBufferFromNV12IOSurface:(IOSurfaceRef)ioSurface
                                   captureFormat:
                                       (const media_m96::VideoCaptureFormat&)
                                           captureFormat
                                      colorSpace:
  ...
  return media_m96::CapturedExternalVideoBuffer(std::move(handle), captureFormat,
...
- (void)captureOutput:(AVCaptureOutput*)captureOutput
  ...
  if (logUma) {
    media_m96::LogFirstCapturedVideoFrame(_bestCaptureFormat, sampleBuffer);
  }
  ...
  if (sampleHasPixelBufferOrIsMjpeg) {
    _sampleBufferTransformer->Reconfigure(
        media_m96::SampleBufferTransformer::GetBestTransformerForNv12Output(
            sampleBuffer),
        kCVPixelFormatType_420YpCbCr8BiPlanarVideoRange,
        media_m96::GetSampleBufferSize(sampleBuffer), kPixelBufferPoolSize);
    ...
    const media_m96::VideoCaptureFormat captureFormat(
        ...
        _frameRate, media_m96::PIXEL_FORMAT_NV12);
    ...
    // space is not recognized by media_m96::GetImageBufferColorSpace(). This
  ...
  media_m96::VideoPixelFormat videoPixelFormat = [VideoCaptureDeviceAVFoundation
  ...
  if (CVPixelBufferRef pixelBuffer =
          CMSampleBufferGetImageBuffer(sampleBuffer)) {
    const gfx::ColorSpace colorSpace =
        media_m96::GetImageBufferColorSpace(pixelBuffer);
    ...
    if (CVPixelBufferGetIOSurface(pixelBuffer) &&
        videoPixelFormat == media_m96::PIXEL_FORMAT_NV12) {
      [self processPixelBufferNV12IOSurface:pixelBuffer
                              captureFormat:captureFormat
                                 colorSpace:colorSpace
                                  timestamp:timestamp];
  ...
  gfx::ColorSpace colorSpace =
      media_m96::GetFormatDescriptionColorSpace(formatDescription);
