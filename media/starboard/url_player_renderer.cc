...
void UrlPlayerRenderer::CreatePlayerBridge() {
  ...
  std::string error_message = player_bridge_->GetPlayerCreationErrorMessage();
  player_bridge_.reset();
  LOG(ERROR) << "Failed to create SbPlayerBridge: \"" << error_message << "\"";
  state_ = STATE_ERROR;
  std::move(init_cb_).Run(
      PipelineStatus(DECODER_ERROR_NOT_SUPPORTED,
                     "UrlPlayerRenderer::CreatePlayerBridge() failed - \"" +
                         error_message + "\""));
}
...
void UrlPlayerRenderer::StoreMediaTime(TimeDelta media_time) {
