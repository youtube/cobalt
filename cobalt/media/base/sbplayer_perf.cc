...
const base::TimeDelta sampling_interval =
    base::Milliseconds(20);  // base::Seconds(1);
...
class StatisticsWrapper {
 public:
  ...
  base::Statistics<double, int, 1024> audio_track_aud{
      "SbPlayer.audio_track_aud"};
...
void SbPlayerPerf::GetPlatformIndependentCPUUsage() {
  ...
  if (player_perf_extension &&
    ...
    base::Percentiles audio_track_aud_out_percentiles,
    ...
    for (auto element : map_cumulative_cpu) {
      ...
      if (element.first.compare("audio_track_aud") == 0) {
        StatisticsWrapper::GetInstance()->audio_track_aud.AddSample(
            element.second, 1);
    ...
    if (false &&
      ...
      StatisticsWrapper::GetInstance()->audio_track_aud.GetPercentiles(
          &audio_track_aud_out_percentiles);
      ...
      LOG(ERROR)
          ...
          << ";;Statistics_audio_track_aud(%)::whislo:"
          << StatisticsWrapper::GetInstance()->audio_track_aud.min()
          << ",q1:" << audio_track_aud_out_percentiles.percentile_25th
          << ",med:" << audio_track_aud_out_percentiles.median
          << ",q3:" << audio_track_aud_out_percentiles.percentile_75th
          << ",average:"
          << StatisticsWrapper::GetInstance()->audio_track_aud.average()
          << ",whishi:"
          << StatisticsWrapper::GetInstance()->audio_track_aud.max()
          << ",nsamples:"
          << StatisticsWrapper::GetInstance()
                 ->audio_track_aud.GetNumberOfSamples()
