...
FakeMediaSource::FakeMediaSource(
    ...
    : task_runner_(task_runner),
      output_audio_params_(AudioParameters::AUDIO_PCM_LINEAR,
                           media_m96::GuessChannelLayout(audio_config.channels),
...
void FakeMediaSource::Start(scoped_refptr<AudioFrameInput> audio_frame_input,
  ...
  audio_converter_ = std::make_unique<media_m96::AudioConverter>(
...
void FakeMediaSource::DecodeAudio(ScopedAVPacket packet) {
  ...
  while (frames_needed_to_scale <= audio_algo_.BufferedFrames()) {
    ...
    std::unique_ptr<media_m96::AudioBus> resampled_bus(media_m96::AudioBus::Create(
...
bool FakeMediaSource::OnNewVideoFrame(AVFrame* frame) {
  ...
  scoped_refptr<media_m96::VideoFrame> video_frame =
      VideoFrame::WrapExternalYuvData(
          media_m96::PIXEL_FORMAT_I420, size, gfx::Rect(size), size,
...
double FakeMediaSource::ProvideInput(media_m96::AudioBus* output_bus,
