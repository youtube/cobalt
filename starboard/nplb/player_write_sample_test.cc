...
const SbTimeMonotonic kDefaultWaitForDecoderStateTimeout = 5 * kSbTimeSecond;
...
class SbPlayerWriteSampleTest
 ...
 protected:
  enum DecoderEvent { kDecoderEventNeedsData, kDecoderEventError };
  ...
  // Waits for the next available decoder state. Returns |kDecoderEventError| if
  // it times out while waiting for the next decoder state.
  DecoderEvent WaitForDecoderState(
      SbTime timeout = kDefaultWaitForDecoderStateTimeout);

  // Waits for desired player state update to be sent. Returns true if it
  // receives it, otherwise returns false if the wait exceeds timeout.
  bool WaitForPlayerState(const SbPlayerState state,
                          SbTime timeout = kDefaultWaitForPlayerStateTimeout);
  ...
  bool HasReceivedPlayerState(SbPlayerState state) const {
    return received_player_states_.find(state) != received_player_states_.end();
  }

 private:
  Mutex event_queue_mutex_;
  std::deque<DecoderEvent> event_queue_;
...
SbPlayerWriteSampleTest::SbPlayerWriteSampleTest() {
  ...
  SB_LOG(INFO) << FormatString(
      ...
      "mode %s.",
      dmp_filename_.c_str(),
      output_mode_ == kSbPlayerOutputModeDecodeToTexture ? "'To Texture'"
                                                         : "'Punchout'");
}
...
void SbPlayerWriteSampleTest::TearDown() {
  ...
  ASSERT_TRUE(WaitForPlayerState(kSbPlayerStateDestroyed, 0));
...
void SbPlayerWriteSampleTest::InitializePlayer() {
  ASSERT_FALSE(destroy_player_called_);
  ASSERT_TRUE(WaitForPlayerState(kSbPlayerStateInitialized));
  Seek(0);
...
void SbPlayerWriteSampleTest::Seek(const SbTime time) {
  ...
  received_player_states_.clear();
  received_player_states_.insert(kSbPlayerStateInitialized);

  ticket_.increment();
  SbPlayerSeek2(player_, time, ticket_.load());
  ASSERT_EQ(WaitForDecoderState(), kDecoderEventNeedsData);
}
...
#if SB_API_VERSION >= SB_PUNCHOUT_MODE_HAS_INVALID_DECODE_TARGET_VERSION
    if (output_mode_ != kSbPlayerOutputModeDecodeToTexture) {
      ASSERT_TRUE(SbPlayerIsValid(player_));
      ASSERT_EQ(SbPlayerGetCurrentFrame(player_), kSbDecodeTargetInvalid);
      return;
    }
#endif  // SB_API_VERSION >= SB_PUNCHOUT_MODE_HAS_INVALID_DECODE_TARGET_VERSION
...
void SbPlayerWriteSampleTest::OnDecoderState(SbPlayer player,
                                             ...
                                             SbPlayerDecoderState state,
                                             int ticket) {
  ASSERT_EQ(player, player_);

  if (ticket != ticket_.load()) {
    // The ticket is used to filter all callbacks that do not match the ticket
    // used for the latest Seek call.
    return;
  }

  // Callbacks may be in-flight at the time that the player is destroyed by a
  // call to |SbPlayerDestroy|. In this case, the callbacks are ignored. However
  // no new callbacks are expected after receiving the player status
  ...
  switch (state) {
    case kSbPlayerDecoderStateNeedsData:
      event_queue_.push_back(kDecoderEventNeedsData);
      break;
#if SB_API_VERSION < SB_PLAYER_DEPRECATE_UNUSED_DECODER_STATES_VERSION
    ...
    case kSbPlayerDecoderStateDestroyed:
      break;
#endif  // SB_API_VERSION < SB_PLAYER_DEPRECATE_UNUSED_DECODER_STATES_VERSION
  }
}
...
void SbPlayerWriteSampleTest::OnPlayerState(SbPlayer player,
  ...
  ASSERT_NO_FATAL_FAILURE(AssertPlayerStateIsValid(state));
  received_player_states_.insert(state);
}
...
void SbPlayerWriteSampleTest::OnError(SbPlayer player,
  ...
  ScopedLock scoped_lock(event_queue_mutex_);
  event_queue_.push_back(kDecoderEventError);
  error_occurred_ = true;
}

SbPlayerWriteSampleTest::DecoderEvent
SbPlayerWriteSampleTest::WaitForDecoderState(SbTime timeout) {
  ...
  return kDecoderEventError;
}

bool SbPlayerWriteSampleTest::WaitForPlayerState(const SbPlayerState state,
                                                 SbTime timeout) {
  ...
  do {
    ...
    SbThreadSleep(kSbTimeMillisecond);
           ...
           !HasReceivedPlayerState(state));
  if (HasReceivedPlayerState(state)) {
    return true;
  }
  SB_LOG(ERROR) << FormatString(
      "Timed out while waiting for player state update %d.", state);
  return false;
}
...
void SbPlayerWriteSampleTest::WriteMultipleInputs(size_t start_index,
  ...
  while (num_inputs_to_write > 0 && start_index < GetNumBuffers()) {
    DecoderEvent event = WaitForDecoderState();
    ASSERT_EQ(event, kDecoderEventNeedsData);
    ASSERT_NO_FATAL_FAILURE(WriteSingleInput(start_index));
    ++start_index;
    --num_inputs_to_write;
  }
...
void SbPlayerWriteSampleTest::DrainOutputs() {
  ...
  ASSERT_TRUE(WaitForPlayerState(kSbPlayerStateEndOfStream));
  // We should not get any new decoder events after end of stream.
  ScopedLock lock(event_queue_mutex_);
  ASSERT_TRUE(event_queue_.empty());
}
...
void SbPlayerWriteSampleTest::AssertPlayerStateIsValid(
  ...
  // issued in this test, we should reset the |received_player_states_| member.
  ASSERT_FALSE(HasReceivedPlayerState(state));

  switch (state) {
    case kSbPlayerStateInitialized:
      // No other states have been received before getting Initialized.
      ASSERT_TRUE(received_player_states_.empty());
      break;
