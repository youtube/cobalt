...
void Unpacker::Unpack(const std::string& app_id,
                      ...
                      crx_file::VerifierFormat crx_format,
                      base::OnceCallback<void(const Result& result)> callback) {
  base::WrapRefCounted(new Unpacker(app_id, path, std::move(unzipper),
                                    std::move(callback)))
      ->Verify(pk_hash, crx_format);
}
#endif                   std::unique_ptr<Unzipper> unzipper,
                   base::OnceCallback<void(const Result& result)> callback)
    : app_id_(app_id),
      path_(path),
      unzipper_(std::move(unzipper)),
...
#endif  // BUILDFLAG(IS_STARBOARD)  if (!base::CreateNewTempDirectory(
