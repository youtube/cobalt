...
int __abi_wrap_sigaction(int signum,
  ...
  if (act) {
    ...
    sigemptyset(&platform_act.sa_mask);
    for (int i = 1; i < NSIG; ++i) {
      if (__abi_wrap_sigismember(&act->sa_mask, i)) {
        sigaddset(&platform_act.sa_mask, i);
      }
    }
  ...
  if (result == 0 && oldact) {
    *oldact = {};
    ...
    __abi_wrap_sigemptyset(&oldact->sa_mask);
    for (int i = 1; i < NSIG; ++i) {
      if (sigismember(&platform_oldact.sa_mask, i)) {
        __abi_wrap_sigaddset(&oldact->sa_mask, i);
      }
    }
...
int __abi_wrap_sigprocmask(int how,
...
int __abi_wrap_sigemptyset(musl_sigset_t* set) {
  return sigemptyset(reinterpret_cast<sigset_t*>(set));
}

int __abi_wrap_sigfillset(musl_sigset_t* set) {
  return sigfillset(reinterpret_cast<sigset_t*>(set));
}

int __abi_wrap_sigaddset(musl_sigset_t* set, int signum) {
  return sigaddset(reinterpret_cast<sigset_t*>(set), signum);
}

int __abi_wrap_sigdelset(musl_sigset_t* set, int signum) {
  return sigdelset(reinterpret_cast<sigset_t*>(set), signum);
}

int __abi_wrap_sigismember(const musl_sigset_t* set, int signum) {
...
int __abi_wrap_pthread_sigmask(int how,
                               const musl_sigset_t* set,
                               musl_sigset_t* oldset) {
  return pthread_sigmask(how, reinterpret_cast<const sigset_t*>(set),
                         reinterpret_cast<sigset_t*>(oldset));
}
