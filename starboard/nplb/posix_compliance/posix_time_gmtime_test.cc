...
#include <climits>  // For INT_MAX, INT_MIN (used in comments/reasoning)
#include <cstring>  // For strerror
#include <ctime>    // For time_t, struct tm, gmtime
...
void compare_tm(const struct tm& actual,
...
// Constant for the number of seconds in a standard day.
// Used for calculating time_t values relative to others.
static constexpr time_t kSecondsInDay = 24 * 60 * 60;

// Test with epoch time (0 seconds since Jan 1, 1970, 00:00:00 UTC).
TEST(PosixGmtimeTests, HandlesEpochTime) {
  time_t epoch_time = 0;  // Represents January 1, 1970, 00:00:00 UTC.
  ...
  compare_tm(*result_tm_ptr, expected_tm, "epoch_time");
}

// Test with a specific, known timestamp after the epoch.
TEST(PosixGmtimeTests, HandlesSpecificKnownPositiveTime) {
  ...
  compare_tm(*result_tm_ptr, expected_tm, "specific_time (2023-10-26)");
}

// Test with a negative time_t value, representing a date before the epoch.
TEST(PosixGmtimeTests, HandlesNegativeTimeBeforeEpoch) {
  // Timestamp for 1969-12-31 00:00:00 UTC (which is one full day before
  // epoch_time).
  ...
  compare_tm(*result_tm_ptr, expected_tm, "negative_time (1969-12-31)");
}

// Test the static buffer behavior: subsequent calls should return the same
// pointer and overwrite the content of the buffer.
TEST(PosixGmtimeTests, ReturnsPointerToStaticBufferAndOverwrites) {
  time_t time1 = 0;  // Epoch: Jan 1, 1970, 00:00:00 UTC.
  time_t time2 =
      kSecondsInDay;  // One day after epoch: Jan 2, 1970, 00:00:00 UTC.

  // First call to gmtime
  struct tm* tm_ptr_call1 = gmtime(&time1);
  ASSERT_NE(tm_ptr_call1, nullptr) << "gmtime(&time1) returned nullptr.";
  // Create a deep copy of the struct tm data from the first call *before* the
  // second call.
  struct tm tm_val_call1 = *tm_ptr_call1;

  // Second call to gmtime with a different time
  ...
  compare_tm(*tm_ptr_call2, expected_tm_for_time2,
  ...
  // For completeness, ensure tm_val_call1 (the copied data from the first call)
  // still matches the expected data for time1.
  ...
  compare_tm(tm_val_call1, expected_tm_for_time1,
             "copied data from first call (time1)");
}

// Test gmtime's behavior with time_t's maximum value.
// POSIX: "If the specified time cannot be converted... (e.g., if the time_t
// object has a value that is too large)... gmtime() shall return a null
// pointer." This usually means the year cannot fit in tm_year (an int).
