...
#if BUILDFLAG(IS_ANDROID) || BUILDFLAG(IS_STARBOARD) || BUILDFLAG(IS_COBALT)
  return true;
#endif
...
#if BUILDFLAG(IS_COBALT) || BUILDFLAG(IS_STARBOARD)
  // Extensions are not supported on all Starboard/embedded platforms, which may
  // generate GL_INVALID_ENUM or GL_INVALID_OPERATION during the probe above.
  // Clear any error generated by the probe and return false without failing a DCHECK.
  GLenum err = glGetError();
  return result && (err == GL_NO_ERROR);
#else
  DCHECK(glGetError() == GL_NO_ERROR);
  return result;
#endif
...
void FeatureInfo::InitializeFeatures() {
  ...
  if (is_passthrough_cmd_decoder_ &&
      (gfx::HasExtension(extensions, "GL_OES_draw_buffers_indexed") ||
       gfx::HasExtension(extensions, "GL_EXT_draw_buffers_indexed"))) {
    if (!disallowed_features_.oes_draw_buffers_indexed &&
        gl::g_current_gl_driver &&
        gl::g_current_gl_driver->fn.glColorMaskiOESFn) {
