...
def run_adb_command(adb_path, device_serial, command_args, timeout=30, check=True, verbose=True):
    ...
    try:
        ...
        if verbose and result.stderr: print(f"--> stderr (potential warnings):\n{result.stderr.strip()}")
        return result
...
def get_memory_usage(adb_path, device_serial, package_name):
    ...
    if result and result.stdout:
        ...
        # --- PRINT RAW OUTPUT ---
        print(f"DEBUG(Mem): Received dumpsys meminfo output for {package_name}:\n---\n{raw_output}\n---") # Print full output

        if "No process found" in raw_output or not raw_output:
            print("DEBUG(Mem): 'No process found' or empty output detected in dumpsys.")
            return None
        ...
        if match_pss_direct:
            ...
            if match_pss_table:
                try:
                   ...
                   print(f"DEBUG(Mem): Matched table 'TOTAL VALUE ...', PSS={total_pss} kB")
                except ValueError: pass

        if total_pss is not None:
             memory_mb = round(total_pss / 1024.0, 2) # Convert kB to MB
             print(f"DEBUG(Mem): Successfully parsed PSS -> {memory_mb} MB") # Success message
             return memory_mb
        else:
            # --- Parsing Failed ---
            print("DEBUG(Mem): Failed to parse TOTAL PSS from the output above.") # Explicit failure message
            return None # Return None *after* printing raw output and failure message

    elif result is None:
         print("DEBUG(Mem): run_adb_command failed for dumpsys meminfo (returned None). Check ADB connection/permissions.")
...
def get_cpu_usage(adb_path, device_serial, package_name):
    ...
    if result and result.stdout:
        ...
        if header_cpu_index == -1:
             print("DEBUG(CPU): Could not determine CPU% column index from header. Cannot parse CPU.")
            ...
            if package_name in line or (fields and fields[-1].endswith(package_name)):
                ...
                try:
                    ...
                    if actual_cpu_value_index < len(fields):
                        cpu_str = fields[actual_cpu_value_index].replace('%', '') # Get value from corrected index
                        ...
                        print(f"DEBUG(CPU): Actual CPU column index {actual_cpu_value_index} out of bounds for fields (len={len(fields)}) on process line.")
                        # Break here, as this line format is unexpected
                        break
                except (ValueError, IndexError) as e:
                    print(f"DEBUG(CPU): Error parsing CPU value from index {actual_cpu_value_index}: {e}")
        ...
        if not process_line_found:
            print(f"DEBUG(CPU): Package name '{package_name}' not found in any line of top output.")
        # elif header_cpu_index == -1: # Already handled
            # pass
        else: # Process line was found, but parsing failed (message printed in loop)
             print(f"DEBUG(CPU): Found process line(s) but failed to parse CPU% value from index {actual_cpu_value_index}.")

        return None # Return None if parsing failed or process not found

    elif result is None:
         print("DEBUG(CPU): run_adb_command failed for 'top' (returned None). Check ADB connection/permissions.")
    else: # result exists but stdout is empty
...
def launch_cobalt(adb_path, device_serial, component, url):
    ...
    return run_adb_command(adb_path, device_serial, am_start_args, check=True) is not None
...
def kill_cobalt(adb_path, device_serial, package_name):
    ...
    return run_adb_command(adb_path, device_serial, am_kill_args, check=False) is not None
...
def process_performance_data(data):
    ...
    if valid_cpu:
        ...
        print(f"CPU Usage (%): Avg={avg_cpu:.1f}, Max={max_cpu:.1f} ({len(valid_cpu)} samples)")
    else:
...
if __name__ == "__main__":
    ...
    parser.add_argument('--sample-interval', type=int, default=SAMPLING_INTERVAL_SECONDS, help='Interval (seconds) for CPU/Memory sampling.')
    parser.add_argument('--output-csv', default=PERFORMANCE_DATA_CSV_FILE, help='Filename for saving performance data.')
    ...
    try:
        while True:
            ...
            print(f"\r  Elapsed: {elapsed_display:.1f}s / {args.duration}s | CPU: {latest_cpu_str} | Mem: {latest_mem_str} | Rem: {remaining_display:.1f}s  ", end='')
            ...
            if sleep_duration <= 0:
                ...
                if time.monotonic() < end_time: # Only sleep if not already past end time
