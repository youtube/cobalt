...
float GetFlat(jobject input_device, int axis) {
  ...
  ScopedLocalJavaRef<jobject> motion_range(JniExt::CallObjectMethodOrAbort(
  ...
  float flat =
      JniExt::CallFloatMethodOrAbort(env, motion_range.Get(), "getFlat", "()F");
...
jobject GetDevice(GameActivityMotionEvent* android_motion_event) {
  int32_t device_id = android_motion_event->deviceId;
  JNIEnv* env = base::android::AttachCurrentThread();
  return JniExt::CallStaticObjectMethodOrAbort(
