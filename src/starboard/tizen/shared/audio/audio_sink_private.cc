...
SbAudioSinkPrivate::SbAudioSinkPrivate(
    ...
    : channels_(channels),
      ...
      destroying_(false),
      is_paused_(true) {
  SB_LOG(INFO) << "[MEDIA] SbAudioSinkPrivate : "
               << "channels " << channels << ", frequency "
               << sampling_frequency_hz << ", sample_type " << audio_sample_type
               << ", storage_type " << audio_frame_storage_type
               << ", frame_buffers " << (int)frame_buffers << ", frame_buff_sz "
               << frames_per_channel;
  ...
  //  capi_ret = audio_out_set_stream_cb(capi_audio_out_,
  //  OnCAPIAudioStreamWrite_CB, this); // not used
  //  CHECK_CAPI_AUDIO_ERROR(audio_out_set_stream_cb);

  /*
  capi_ret = audio_out_prepare(capi_audio_out_);
  CHECK_CAPI_AUDIO_ERROR(audio_out_prepare);

  capi_ret = audio_out_pause(capi_audio_out_); // to prevent underflow
  CHECK_CAPI_AUDIO_ERROR(audio_out_pause);
*/
...
SbAudioSinkPrivate::~SbAudioSinkPrivate() {
  ...
  if (SbThreadIsValid(audio_out_thread_)) {
    SB_LOG(INFO) << "[MEDIA] wait for audio sink thread exit";
    SbThreadJoin(audio_out_thread_, NULL);
  } else {
    SB_LOG(INFO) << "[MEDIA] audio sink thread is invalid. skip waiting";
  }
  ...
  if (capi_audio_out_) {
    ...
    if (ret != AUDIO_IO_ERROR_NONE) {
      SB_LOG(ERROR) << "audio_out_destroy failed (" << ret << ")";
    }
  }
}
...
void SbAudioSinkPrivate::OnCAPIAudioIOInterrupted_CB(
    ...
    void* user_data) {
  SbAudioSinkPrivate* audio_sink = (SbAudioSinkPrivate*)user_data;
  if (audio_sink) {
...
void* SbAudioSinkPrivate::AudioSinkThreadProc_CB(void* context) {
  SbAudioSinkPrivate* audio_sink = (SbAudioSinkPrivate*)context;
  if (audio_sink) {
...
void SbAudioSinkPrivate::OnCAPIAudioIOInterrupted(
    audio_io_interrupted_code_e code) {
  SB_LOG(WARNING) << "Play interrupted: audio_io_interrupted_code_e : " << code;
}

void SbAudioSinkPrivate::OnCAPIAudioStreamWrite(audio_out_h handle,
                                                size_t nbytes) {
  SB_LOG(INFO) << "[MEDIA] OnAudioStreamWrite (not used) - request " << nbytes;
}
...
void* SbAudioSinkPrivate::AudioSinkThreadProc() {
  ...
  SB_LOG(INFO) << "[MEDIA] sink thread started";
  ...
  for (;;) {
    ...
    if (is_playing) {
      buf = (uint8_t*)frame_buffers_[0] + offset_in_frames * bytes_per_frame;
      ...
      if (is_paused_) {
        ...
        is_paused_ = false;
        SB_LOG(INFO) << "[MEDIA] audio_out_resume";
      }

      bytes_written = audio_out_write(capi_audio_out_, buf, bytes_to_fill);

      if (bytes_written < 0) {
        SB_LOG(ERROR) << "[MEDIA] audio_out_write error (" << bytes_written
      ...
      if (!is_paused_) {
        ...
        is_paused_ = true;
        SB_LOG(INFO) << "[MEDIA] audio_out_pause";
      }
  ...
  SB_LOG(INFO) << "[MEDIA] sink thread exited";
  return NULL;
}
