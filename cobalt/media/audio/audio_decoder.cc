...
std::unique_ptr<blink::WebAudioBus> DecodeAudioFileData(
  ...
  out->Initialize(handler->GetNumChannels(), number_of_frames,
                  handler->GetSampleRate());

  DCHECK_EQ(static_cast<int>(out->NumberOfChannels()),
            handler->GetNumChannels());
  DCHECK_EQ(out->length(), number_of_frames);

  if (std::cmp_equal(source_bus->channels(), out->NumberOfChannels()) &&
      std::cmp_equal(source_bus->frames(), out->length())) {
    ...
    for (int channel_index = 0; channel_index < source_bus->channels();
      ...
      float* dest_data = out->ChannelData(channel_index);
    ...
    LOG(ERROR) << "AudioBus dimension mismatch during copy.\nSource Channels:"
               << source_bus->channels()
               << " Dest Channels:" << out->NumberOfChannels()
               << " Source Frames:" << source_bus->frames()
               << " Dest Frames:" << out->length();
    return nullptr;
  }
