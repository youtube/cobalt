...
TEST(CobaltProcessStateSummaryManagerTest,
     PreNativeWatchdogCrashPayloadDeserialization) {
  // Simulates a fallback payload generated purely in Java before native
  // libraries are loaded, when StartupGuard triggers a forced crash at
  // milestone 2 (PRE_LIBRARY_LOADER_INIT).
  std::vector<uint8_t> payload(kProcessStateSummaryPayloadSize, 0);
  payload[0] = kProcessStateSummaryMagic;
  payload[1] = kProcessStateSummaryVersion;
  payload[2] = 0;  // trim level
  payload[3] = kFlagStartupGuardTriggeredKill;
  ...
  // Memory peaks and uptime are 0 before native initialization
  // startup_milestones mask: (1ULL << 1) | (1ULL << 2)
  uint64_t milestones = (1ULL << 1) | (1ULL << 2);
  for (int i = 0; i < 8; ++i) {
    payload[24 + i] = static_cast<uint8_t>((milestones >> (i * 8)) & 0xFF);
  }
  payload[32] = 2;  // highest_milestone

  // Compute 32-bit PersistentHash checksum (SuperFastHash) over bytes [0..35].
  uint32_t checksum = base::PersistentHash(base::span(payload).first(36u));
  payload[36] = static_cast<uint8_t>(checksum & 0xFF);
  payload[37] = static_cast<uint8_t>((checksum >> 8) & 0xFF);
  payload[38] = static_cast<uint8_t>((checksum >> 16) & 0xFF);
  payload[39] = static_cast<uint8_t>((checksum >> 24) & 0xFF);

  std::optional<ProcessStateSnapshot> deserialized =
      CobaltProcessStateSummaryManager::DeserializeSnapshot(payload);
  ASSERT_TRUE(deserialized.has_value());
  EXPECT_EQ(deserialized->pid, 4321u);
  EXPECT_TRUE(deserialized->was_killed_by_startup_guard());
  EXPECT_FALSE(deserialized->is_startup_guard_armed());
  EXPECT_EQ(deserialized->highest_milestone, 2u);
  EXPECT_EQ(deserialized->startup_milestones, milestones);
  EXPECT_EQ(deserialized->peak_rss_kb, 0u);
  EXPECT_EQ(deserialized->peak_pmf_kb, 0u);
  EXPECT_EQ(deserialized->peak_v8_code_kb, 0u);
  EXPECT_EQ(deserialized->uptime_sec, 0u);
}
