...
std::unique_ptr<AudioTrackAudioSink> AudioTrackAudioSink::Create(
    ...
    bool is_web_audio,
    void* context) {
  auto bridge = AudioTrackBridge::Create(
      kSbMediaAudioCodingTypePcm, sample_type, channels, sampling_frequency_hz,
      preferred_buffer_size, tunnel_mode_audio_session_id,
  ...
  return std::make_unique<AudioTrackAudioSink>(
      PassKey<AudioTrackAudioSink>(), std::move(bridge), type, channels,
      sampling_frequency_hz, sample_type, frame_buffers, frames_per_channel,
      preferred_buffer_size, update_source_status_func, consume_frames_func,
      error_func, start_media_time, tunnel_mode_audio_session_id, is_web_audio,
      context);
}
...
SbAudioSink AudioTrackAudioSinkType::Create(
  ...
  std::unique_ptr<AudioTrackAudioSink> audio_sink = AudioTrackAudioSink::Create(
