...
#include "cobalt/shell/browser/renderer_host/shell_render_widget_host_view_mac_delegate.h"
...
void ShellWebContentsViewDelegate::ShowContextMenu(
  ...
  if (params.media_type == blink::mojom::ContextMenuDataMediaType::kNone &&
    ...
    MakeContextMenuItem(@"Back", ShellContextMenuItemBackTag, menu,
                        back_menu_enabled, delegate);

    BOOL forward_menu_enabled =
        web_contents_->GetController().CanGoForward() ? YES : NO;
    MakeContextMenuItem(@"Forward", ShellContextMenuItemForwardTag, menu,
                        forward_menu_enabled, delegate);

    MakeContextMenuItem(@"Reload", ShellContextMenuItemReloadTag, menu, YES,
                        delegate);
  ...
  if (has_link) {
    MakeContextMenuItem(@"Open In New Window", ShellContextMenuItemOpenLinkTag,
                        menu, YES, delegate);
  ...
  if (params_.is_editable) {
    ...
    MakeContextMenuItem(@"Cut", ShellContextMenuItemCutTag, menu,
                        cut_menu_enabled, delegate);

    BOOL copy_menu_enabled =
        (params_.edit_flags & ContextMenuDataEditFlags::kCanCopy) ? YES : NO;
    MakeContextMenuItem(@"Copy", ShellContextMenuItemCopyTag, menu,
                        copy_menu_enabled, delegate);

    BOOL paste_menu_enabled =
        (params_.edit_flags & ContextMenuDataEditFlags::kCanPaste) ? YES : NO;
    MakeContextMenuItem(@"Paste", ShellContextMenuItemPasteTag, menu,
                        paste_menu_enabled, delegate);

    BOOL delete_menu_enabled =
        (params_.edit_flags & ContextMenuDataEditFlags::kCanDelete) ? YES : NO;
    MakeContextMenuItem(@"Delete", ShellContextMenuItemDeleteTag, menu,
                        delete_menu_enabled, delegate);

    NSMenuItem* separator = [NSMenuItem separatorItem];
    [menu addItem:separator];
  } else if (has_selection) {
    MakeContextMenuItem(@"Copy", ShellContextMenuItemCopyTag, menu, YES,
                        delegate);
  ...
  MakeContextMenuItem(@"Inspect", ShellContextMenuItemInspectTag, menu, YES,
                      delegate);
  ...
  [NSMenu popUpContextMenu:menu withEvent:clickEvent forView:parent_view];
}
