...
class FeatureList {
 ...
 public:
  // Initialize the FeatureList instance with the
  // received features and parameters from the Cobalt side.
  // When this function is completed, |is_initialized_| will
  // be set to true.
  ...
  // Check to see if the given SbFeature is enabled
  // or not. If the given SbFeature does not exist
  // in the FeatureList, the app will crash.
  //
  // An example of using this function would look like:
  // starboard::FeatureList::IsEnabled(starboard::features::kFeatureFoo);
  static bool IsEnabled(const SbFeature& feature);

  // Template function to retrieve a parameter based
  // on the value type of the parameter.
  //
  // An example of using this function would look like:
  // starboard::features::kFeatureParamBoo.Get();
  template <typename T>
  static T GetParam(const SbFeatureParamExt<T>& param);

  template <>
  bool GetParam(const SbFeatureParamExt<bool>& param);
  template <>
  int GetParam(const SbFeatureParamExt<int>& param);
  template <>
  double GetParam(const SbFeatureParamExt<double>& param);
  template <>
  std::string GetParam(const SbFeatureParamExt<std::string>& param);
  template <>
  int64_t GetParam(const SbFeatureParamExt<int64_t>& param);

 private:
  FeatureList() {}
  ~FeatureList() = default;

  // Retrieve the singleton instance of the FeatureList. There
  // can only be one instance of features and parameters
  // under the starboard level. We ensure that we use the
  // same instance with this function.
  ...
  // Helper function to check if the singleton instance of the
  // FeatureList is initialized. Called by other FeatureList class
  // functions. If the FeatureList is accessed before it is
  // initialized, the app will crash.
  ...
  // Starboard features will be stored in an std::unordered_map,
  // where the keys are the string representations of the
  // features, and the values are the associated boolean value
  // of the feature.
  std::unordered_map<std::string, bool> features_;

  // Starboard feature parameters will be stored in a 2D
  // std::unordered_map, where the outer map's keys will be
  // the string of the feature associated with the parameter. Each key will be
  // associated with an inner std::unordered_map whose key-value pairing is the
  // string representation of the specified parameter and the associated type
  // and value of that parameter, respectively.
  std::unordered_map<std::string,
                     std::unordered_map<std::string, ParamTypeAndValue>>
      params_;

  // When InitializeStarboardFeatures() is completed, this value will
  // be set to true.
...
struct SbFeatureParamExt : public SbFeatureParam {
  ...
  // Function used to retrieve the parameter value for a given param.
  // Outside code will call this function, which will then call the
  // corresponding FeatureList::GetParam function.
