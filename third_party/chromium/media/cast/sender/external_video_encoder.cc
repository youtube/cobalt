...
// to encode media_m96::VideoFrames and emit media_m96::cast::EncodedFrames.  All
...
class ExternalVideoEncoder::VEAClientImpl final
 ...
 public:
  VEAClientImpl(
      ...
      : cast_environment_(cast_environment),
        ...
        codec_profile_(media_m96::VIDEO_CODEC_PROFILE_UNKNOWN),
  ...
  void Initialize(const gfx::Size& frame_size,
    ...
    const media_m96::VideoEncodeAccelerator::Config config(
        media_m96::PIXEL_FORMAT_I420, frame_size, codec_profile,
        media_m96::Bitrate::ConstantBitrate(start_bit_rate));
  ...
  void EncodeVideoFrame(
    ...
    if (free_input_buffer_index_.empty()) {
      if (!allocate_input_buffer_in_progress_ &&
        ...
        const size_t buffer_size = media_m96::VideoFrame::AllocationSize(
            media_m96::PIXEL_FORMAT_I420, frame_coded_size_);
    ...
    scoped_refptr<media_m96::VideoFrame> frame = video_frame;
    if (video_frame->coded_size() != frame_coded_size_ ||
        video_frame->storage_type() !=
            media_m96::VideoFrame::StorageType::STORAGE_SHMEM) {
      ...
      if (!frame || !media_m96::I420CopyWithPadding(*video_frame, frame.get())) {
      ...
      frame->AddDestructionObserver(media_m96::base::BindPostTaskToCurrentDefault(base::BindOnce(
 ...
 protected:
  ...
  void AllocateOutputBuffers(size_t size) {
    ...
    for (size_t i = 0; i < kOutputBufferCount; ++i) {
      ...
      video_encode_accelerator_->UseOutputBitstreamBuffer(
          media_m96::BitstreamBuffer(static_cast<int32_t>(i),
  ...
  // buffers.  Package the result in a media_m96::cast::EncodedFrame and post it
  ...
  void BitstreamBufferReady(int32_t bitstream_buffer_id,
    ...
    if (bitstream_buffer_id < 0 ||
      ...
      VLOG(1) << "BitstreamBufferReady(): invalid bitstream_buffer_id="
              << bitstream_buffer_id;
      NotifyError(media_m96::VideoEncodeAccelerator::kPlatformFailureError);
      return;
    }
    ...
    if (!key_frame_encountered_) {
      ...
      if (frame_duration > base::TimeDelta()) {
        ...
        if (metadata.key_frame || key_frame_quantizer_parsable_) {
          if (codec_profile_ == media_m96::VP8PROFILE_ANY) {
            quantizer = ParseVpxHeaderQuantizer(
                reinterpret_cast<const uint8_t*>(encoded_frame->data.data()),
                encoded_frame->data.size());
          } else if (codec_profile_ == media_m96::H264PROFILE_MAIN) {
        ...
        if (quantizer >= 0) {
          const double max_quantizer =
              codec_profile_ == media_m96::VP8PROFILE_ANY
 ...
 private:
  ...
  ~VEAClientImpl() final {
    ...
    // According to the media_m96::VideoEncodeAccelerator interface, Destroy()
  ...
  std::unique_ptr<media_m96::VideoEncodeAccelerator> video_encode_accelerator_;
...
void ExternalVideoEncoder::OnCreateVideoEncodeAccelerator(
  ...
  switch (video_config.codec) {
    case CODEC_VIDEO_VP8:
      codec_profile = media_m96::VP8PROFILE_ANY;
      break;
    case CODEC_VIDEO_H264:
      codec_profile = media_m96::H264PROFILE_MAIN;
      break;
...
bool QuantizerEstimator::CanExamineFrame(const VideoFrame& frame) {
  DCHECK_EQ(8, VideoFrame::PlaneHorizontalBitsPerPixel(frame.format(),
                                                       VideoFrame::kYPlane));
  return media_m96::IsYuvPlanar(frame.format()) &&
