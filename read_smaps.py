...
VmaTag = namedtuple('VmaTag', ['start', 'end', 'name'])
...
def read_smap(args):
  ...
  pid_match = re.search(r'/proc/(\d+)/smaps', args.smaps_file)
  if pid_match:
    pid = pid_match.group(1)
    vma_tag_files = glob.glob(f'/tmp/cobalt_vma_tags_{pid}_*')
    for vma_tag_file in vma_tag_files:
      try:
        with open(vma_tag_file, encoding='utf-8') as f:
          for line in f:
            parts = line.strip().split()
            if len(parts) == 3:
              vma_tags.append(
                  VmaTag(
                      int(parts[0], 16), int(parts[1], 16), parts[2]))
  ...
  for ls in takeuntil(lines, lambda x: x.startswith('VmFlags:')):
    ...
    start_addr = int('0x' + addr_range[0], 16)
    end_addr = int('0x' + addr_range[1], 16)

    if key == '<anon>':
      for tag in vma_tags:
        if start_addr >= tag.start and end_addr <= tag.end:
          key = f'<anon:{tag.name}>'
          break
    ...
    if args.aggregate_android:
      key = re.sub(r'[@\-\.\w]*\.(hyb|vdex|odex|art|jar|oat)[\]]*$', r'<\g<1>>', key) 
    ...
    if (end_addr - start_addr) / 1024 != d.size:
