...
void LogCaptureDeviceMetrics(
    base::span<const media_m96::VideoCaptureDeviceInfo> devices_info) {
  for (const auto& device : devices_info) {
    base::flat_set<media_m96::VideoPixelFormat> supported_pixel_formats;
    base::flat_set<gfx::Size, FrameSizeCompare> resolutions;
    for (const auto& format : device.supported_formats) {
      VLOG(2) << "Device supports "
              << media_m96::VideoPixelFormatToString(format.pixel_format) << " at "
              << format.frame_size.ToString() << " ("
              << static_cast<int>(ResolutionNameFromSize(format.frame_size))
              << ")";
      media_m96::VideoPixelFormat pixel_format = format.pixel_format;
      ...
      if (inserted) {
        base::UmaHistogramEnumeration(
            ...
            media_m96::VideoPixelFormat::PIXEL_FORMAT_MAX);
      }
