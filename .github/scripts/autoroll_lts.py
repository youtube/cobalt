  ...
  for line in get_commits(args.source_branch, args.target_branch,
    ...
    if args.identifier_type == 'pr':
      ...
      if match:
        ...
        # Skip if PR in skip list.
        if any(
            skip_sha.startswith(sha)
            for skip_sha in _SKIP_LIST.get(args.target_branch, [])):
          continue

        # Skip if PR in target branch.
        if pr_num in target_change_ids:
          continue

        # Skip if PR in autoroll branch.
        if pr_num in autoroll_change_ids:
      ...
      if match:
        ...
        # Skip if commit in skip list.
        if any(
            skip_sha.startswith(sha)
            for skip_sha in _SKIP_LIST.get(args.target_branch, [])):
          continue

        # Skip if commit in target branch.
        if any(
            commit_hash.startswith(sha) for commit_hash in target_change_ids):
          continue

        # Skip if commit in autoroll branch.
        if any(
